...
export const Main = (props: Props) => {
  ...
  const [homebrewSourcebooks, setHomebrewSourcebooks] = useState<Sourcebook[]>(
    props.homebrewSourcebooks,
  );
  const [hiddenSourcebookIDs, setHiddenSourcebookIDs] = useState<string[]>(
    props.hiddenSourcebookIDs,
  );
  ...
  const persistHiddenSourcebookIDs = (ids: string[]) => {
    return props.dataService.saveHiddenSettingIds(ids).then(setHiddenSourcebookIDs, err => {
      console.error(err);
      notify.error({
  ...
  const exportHeroImage = (hero: Hero) => {
    ...
    document
      .querySelectorAll(`[id^=hero-sheet-${hero.id}-page]`)
      .forEach(elem => pageIds.push(elem.id));
  ...
  const exportHeroPdf = (hero: Hero, resolution: 'standard' | 'high') => {
    ...
    Utils.wait(500).then(() => {
      ...
      Utils.elementsToPdf(pageIds, name, options.classicSheetPageSize, resolution).then(() =>
        setSpinning(false),
      );
    });
  };
  ...
  const exportStandardAbilities = () => {
    ...
    Utils.wait(500).then(() => {
      const pageIds: string[] = [];
      document
        .querySelectorAll('[id^=hero-sheet-standard-abilities-page-abilities]')
        .forEach(elem => pageIds.push(elem.id));

      Utils.elementsToPdf(pageIds, 'Standard Abilities', options.classicSheetPageSize, 'high').then(
        () => setSpinning(false),
      );
    });
  };
    ...
    setDrawer(
      <MinionSlotModal
        ...
        updateSlot={slot => {
          ...
          copy.state.controlledSlots = copy.state.controlledSlots.map(s =>
            s.id === slot.id ? slot : s,
          );
  ...
  const createLibraryElement = (
    ...
    persistHomebrewSourcebooks(sourcebooks).then(() =>
      navigation.goToLibraryEdit(kind, sourcebook.id, id),
    );
  };

  const moveLibraryElement = (
    kind: SourcebookElementKind,
    sourcebookID: string,
    element: Element,
  ) => {
    ...
    switch (kind) {
      case 'adventure':
        sourceSourcebook = SourcebookLogic.getAdventureSourcebook(
          sourcebooks,
          element as Adventure,
        );
        break;
      case 'ancestry':
      ...
      case 'class':
        ...
        break;
      case 'complication':
        sourceSourcebook = SourcebookLogic.getComplicationSourcebook(
          sourcebooks,
          element as Complication,
        );
        break;
      case 'culture':
      ...
      case 'domain':
        ...
        break;
      case 'encounter':
        sourceSourcebook = SourcebookLogic.getEncounterSourcebook(
          sourcebooks,
          element as Encounter,
        );
        break;
      case 'imbuement':
        sourceSourcebook = SourcebookLogic.getImbuementSourcebook(
          sourcebooks,
          element as Imbuement,
        );
        break;
      case 'item':
      ...
      case 'kit':
        ...
        break;
      case 'monster-group':
        sourceSourcebook = SourcebookLogic.getMonsterGroupSourcebook(
          sourcebooks,
          element as MonsterGroup,
        );
        break;
      case 'montage':
        ...
        break;
      case 'negotiation':
        sourceSourcebook = SourcebookLogic.getNegotiationSourcebook(
          sourcebooks,
          element as Negotiation,
        );
        break;
      case 'perk':
      ...
      case 'subclass':
        ...
        break;
      case 'tactical-map':
        sourceSourcebook = SourcebookLogic.getTacticalMapSourcebook(
          sourcebooks,
          element as TacticalMap,
        );
        break;
      case 'terrain':
    ...
    switch (kind) {
      ...
      case 'complication':
        ...
        sourceSourcebook.complications = sourceSourcebook.complications.filter(
          x => x.id !== element.id,
        );
        break;
      case 'culture':
      ...
      case 'monster-group':
        ...
        sourceSourcebook.monsterGroups = sourceSourcebook.monsterGroups.filter(
          x => x.id !== element.id,
        );
        break;
      case 'montage':
      ...
      case 'negotiation':
        ...
        sourceSourcebook.negotiations = sourceSourcebook.negotiations.filter(
          x => x.id !== element.id,
        );
        break;
      case 'perk':
      ...
      case 'tactical-map':
        ...
        sourceSourcebook.tacticalMaps = sourceSourcebook.tacticalMaps.filter(
          x => x.id !== element.id,
        );
        break;
      case 'terrain':
  ...
  const saveLibraryElement = (
    ...
    if (sourcebook) {
      switch (kind) {
        case 'adventure':
          sourcebook.adventures = sourcebook.adventures.map(x =>
            x.id === element.id ? element : x,
          ) as Adventure[];
          break;
        case 'ancestry':
          sourcebook.ancestries = sourcebook.ancestries.map(x =>
            x.id === element.id ? element : x,
          ) as Ancestry[];
          break;
        case 'career':
          sourcebook.careers = sourcebook.careers.map(x =>
            x.id === element.id ? element : x,
          ) as Career[];
          break;
        case 'class':
          sourcebook.classes = sourcebook.classes.map(x =>
            x.id === element.id ? element : x,
          ) as HeroClass[];
          break;
        case 'complication':
          sourcebook.complications = sourcebook.complications.map(x =>
            x.id === element.id ? element : x,
          ) as Complication[];
          break;
        case 'culture':
          sourcebook.cultures = sourcebook.cultures.map(x =>
            x.id === element.id ? element : x,
          ) as Culture[];
          break;
        case 'domain':
          sourcebook.domains = sourcebook.domains.map(x =>
            x.id === element.id ? element : x,
          ) as Domain[];
          break;
        case 'encounter':
          sourcebook.encounters = sourcebook.encounters.map(x =>
            x.id === element.id ? element : x,
          ) as Encounter[];
          break;
        case 'imbuement':
          sourcebook.imbuements = sourcebook.imbuements.map(x =>
            x.id === element.id ? element : x,
          ) as Imbuement[];
          break;
        case 'item':
          sourcebook.items = sourcebook.items.map(x =>
            x.id === element.id ? element : x,
          ) as Item[];
          break;
        case 'kit':
          ...
          break;
        case 'monster-group':
          sourcebook.monsterGroups = sourcebook.monsterGroups.map(x =>
            x.id === element.id ? element : x,
          ) as MonsterGroup[];
          break;
        case 'montage':
          sourcebook.montages = sourcebook.montages.map(x =>
            x.id === element.id ? element : x,
          ) as Montage[];
          break;
        case 'negotiation':
          sourcebook.negotiations = sourcebook.negotiations.map(x =>
            x.id === element.id ? element : x,
          ) as Negotiation[];
          break;
        case 'perk':
          sourcebook.perks = sourcebook.perks.map(x =>
            x.id === element.id ? element : x,
          ) as Perk[];
          break;
        case 'project':
          sourcebook.projects = sourcebook.projects.map(x =>
            x.id === element.id ? element : x,
          ) as Project[];
          break;
        case 'subclass':
          sourcebook.subclasses = sourcebook.subclasses.map(x =>
            x.id === element.id ? element : x,
          ) as SubClass[];
          break;
        case 'tactical-map':
          sourcebook.tacticalMaps = sourcebook.tacticalMaps.map(x =>
            x.id === element.id ? element : x,
          ) as TacticalMap[];
          break;
        case 'terrain':
          sourcebook.terrain = sourcebook.terrain.map(x =>
            x.id === element.id ? element : x,
          ) as Terrain[];
          break;
        case 'title':
          sourcebook.titles = sourcebook.titles.map(x =>
            x.id === element.id ? element : x,
          ) as Title[];
          break;
      }
  ...
  const importLibraryElement = (
    ...
    switch (kind) {
      case 'adventure':
        ...
        sourcebook.adventures = Collections.sort<Element>(
      ...
      case 'ancestry':
        ...
        sourcebook.ancestries = Collections.sort<Element>(
      ...
      case 'career':
        ...
        sourcebook.careers = Collections.sort<Element>(
          sourcebook.careers,
          item => item.name,
        ) as Career[];
        break;
      case 'class':
        ...
        sourcebook.classes = Collections.sort<Element>(
          sourcebook.classes,
          item => item.name,
        ) as HeroClass[];
        break;
      case 'complication':
        ...
        sourcebook.complications = Collections.sort<Element>(
      ...
      case 'culture':
        ...
        sourcebook.cultures = Collections.sort<Element>(
          sourcebook.cultures,
          item => item.name,
        ) as Culture[];
        break;
      case 'domain':
        ...
        sourcebook.domains = Collections.sort<Element>(
          sourcebook.domains,
          item => item.name,
        ) as Domain[];
        break;
      case 'encounter':
        ...
        sourcebook.encounters = Collections.sort<Element>(
      ...
      case 'imbuement':
        ...
        sourcebook.imbuements = Collections.sort<Element>(
      ...
      case 'monster-group':
        ...
        sourcebook.monsterGroups = Collections.sort<Element>(
      ...
      case 'montage':
        ...
        sourcebook.montages = Collections.sort<Element>(
          sourcebook.montages,
          item => item.name,
        ) as Montage[];
        break;
      case 'negotiation':
        ...
        sourcebook.negotiations = Collections.sort<Element>(
      ...
      case 'project':
        ...
        sourcebook.projects = Collections.sort<Element>(
          sourcebook.projects,
          item => item.name,
        ) as Project[];
        break;
      case 'subclass':
        ...
        sourcebook.subclasses = Collections.sort<Element>(
      ...
      case 'tactical-map':
        ...
        sourcebook.tacticalMaps = Collections.sort<Element>(
      ...
      case 'terrain':
        ...
        sourcebook.terrain = Collections.sort<Element>(
          sourcebook.terrain,
          item => item.name,
        ) as Terrain[];
        break;
      case 'title':
        ...
        sourcebook.titles = Collections.sort<Element>(
          sourcebook.titles,
          item => item.name,
        ) as Title[];
        break;
  ...
  const exportLibraryElementImage = (category: string, element: Element) => {
    ...
    const pageIds: string[] = [];
    document
      .querySelectorAll(`[id^=${category.toLowerCase()}-${element.id}-page]`)
      .forEach(elem => pageIds.push(elem.id));

    Utils.exportImage(pageIds, name);
  };

  const exportLibraryElementPdf = (
    category: string,
    element: Element,
  ...
  const startEncounter = async (encounter: Encounter) => {
    const copy = SessionLogic.startEncounter(
      encounter,
  ...
  const showAbout = () => {
    setDrawer(
      <AboutModal options={options} setOptions={persistOptions} onClose={() => setDrawer(null)} />,
    );
  };
  ...
  const showRoll = (hero?: Hero) => {
    setDrawer(<RollModal hero={hero} onClose={() => setDrawer(null)} />);
  };
  ...
  const onSelectFollower = (follower: Follower) => {
    setDrawer(<FollowerModal follower={follower} onClose={() => setDrawer(null)} />);
  };

  const onSelectCharacteristic = (characteristic: Characteristic, hero: Hero) => {
    setDrawer(
      <RollModal characteristics={[characteristic]} hero={hero} onClose={() => setDrawer(null)} />,
    );
  };

  const onSelectFeature = (feature: Feature, hero: Hero) => {
    const sourcebooks = SourcebookLogic.getSourcebooks(homebrewSourcebooks).filter(sb =>
      hero.settingIDs.includes(sb.id),
    );
  ...
  return (
    <ErrorBoundary name="main">
      <Routes>
        <Route path="/" element={<MainLayout drawer={drawer} setDrawer={setDrawer} />}>
          <Route
            index={true}
            element={
              <WelcomePage
          ...
          <Route path="hero">
            ...
            <Route
              ...
              element={
                <HeroViewPage
                  ...
                  showComplication={complication =>
                    onSelectLibraryElement(complication, 'complication')
                  }
            ...
            />
            <Route path="edit/:heroID" element={<Navigate to="start" replace={true} />} />
            <Route
          ...
          <Route path="library">
            <Route index={true} element={<Navigate to="ancestry" replace={true} />} />
            <Route
              path=":kind/:elementID?"
              element={
                <LibraryListPage
                  ...
                  createElement={(kind, sourcebookID, element) =>
                    createLibraryElement(kind, sourcebookID, element)
                  }
          ...
          <Route path="session">
            <Route index={true} element={<Navigate to="director" replace={true} />} />
            <Route
              path="director"
              element={
                <SessionDirectorPage
