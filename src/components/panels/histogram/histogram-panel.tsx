    ...
    <ErrorBoundary>
      <div className={props.onSelect ? 'histogram-panel clickable' : 'histogram-panel'}>
        {data.map(v => (
          <div
            key={v.x}
            className={v.x === props.selected ? 'bar-section selected' : 'bar-section'}
            onClick={() => onSelect(v.x)}
          >
            <div style={{ height: `${height * (mode - v.value)}px` }} />
            {v.value > 0 ? (
              <div className="bar" style={{ height: `${height * v.value}px` }} />
            ) : null}
            <div className="label">{props.getLabel ? props.getLabel(v.x) : v.x}</div>
            {props.showPercentages && props.values.length > 0 ? (
              <div className="percentage">{Math.round((100 * v.value) / props.values.length)}%</div>
            ) : null}
          </div>
        ))}
      </div>
    ...
    <ErrorBoundary>
      <div className={props.onSelect ? 'histogram-panel clickable' : 'histogram-panel'}>
        {sortedData.map(v => (
          <div
            key={v.key}
            className={v.key === props.selected ? 'bar-section selected' : 'bar-section'}
