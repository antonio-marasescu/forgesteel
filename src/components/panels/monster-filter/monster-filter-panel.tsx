...
export const MonsterFilterPanel = (props: Props) => {
  ...
  const keywords = Collections.distinct(
    props.monsters.flatMap(m => m.keywords),
    k => k,
  ).sort();
  const maxSize =
    Collections.max(
      props.monsters.map(m => m.size.value),
      x => x,
    ) || 1;
  const maxLevel =
    Collections.max(
      props.monsters.map(m => m.level),
      x => x,
    ) || 1;
  const maxEV =
    Collections.max(
      props.monsters.map(m => m.encounterValue),
      x => x,
    ) || 0;
      ...
      <div className="monster-filter-panel">
        <Space orientation="vertical" style={{ width: '100%' }}>
          ...
          <Toggle
            ...
            value={props.monsterFilter.size.length > 0}
            onChange={value => setFilterSize(value ? [1, 2] : [])}
          />
          {props.monsterFilter.size.length > 0 ? (
            <>
              ...
              <Field
                label="Size"
                value={`${Math.min(...props.monsterFilter.size)} to ${Math.max(...props.monsterFilter.size)}`}
              />
            </>
          ) : null}
          <Toggle
            label="Filter by level"
            value={props.monsterFilter.level.length > 0}
            onChange={value => setFilterLevel(value ? [1, 2] : [])}
          />
          {props.monsterFilter.level.length > 0 ? (
            <>
              ...
              <Field
                label="Level"
                value={`${Math.min(...props.monsterFilter.level)} to ${Math.max(...props.monsterFilter.level)}`}
              />
            </>
          ) : null}
          {props.includeEVFilter ? (
            <>
              <Toggle
                label="Filter by EV"
                value={props.monsterFilter.ev.length > 0}
                onChange={value => setFilterEV(value ? [1, 10] : [])}
              />
              {props.monsterFilter.ev.length > 0 ? (
                <>
                  ...
                  <Field
                    label="EV"
                    value={`${Math.min(...props.monsterFilter.ev)} to ${Math.max(...props.monsterFilter.ev)}`}
                  />
                </>
              ) : null}
            </>
