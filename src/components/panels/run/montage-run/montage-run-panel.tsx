...
export const MontageRunPanel = (props: Props) => {
    ...
    return (
      <div key={challenge.id} className="montage-challenge">
        <Flex align="center" justify="space-between" gap={10}>
          <div className="status">{getIcons()}</div>
  ...
  const successes = Collections.sum(montage.sections, s =>
    Collections.sum(s.challenges, c => c.successes),
  );
  const failures = Collections.sum(montage.sections, s =>
    Collections.sum(s.challenges, c => c.failures),
  );
    ...
    <ErrorBoundary>
      <div className="montage-run-panel" id={montage.id}>
        ...
        <StatsRow>
          ...
          />
          <Field orientation="vertical" label="Outcome" value={outcome} />
        </StatsRow>
        ...
        <div>
          ...
          <Field
            label="Total Success"
            value={<Markdown text={montage.outcomes.totalSuccess} useSpan={true} />}
          />
          <Field
            label="Partial Success"
            value={<Markdown text={montage.outcomes.partialSuccess} useSpan={true} />}
          />
          <Field
            label="Total Failure"
            value={<Markdown text={montage.outcomes.totalFailure} useSpan={true} />}
          />
        </div>
      </div>
    </ErrorBoundary>
