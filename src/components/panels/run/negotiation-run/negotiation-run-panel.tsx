  ...
  return (
    <ErrorBoundary>
      <div className="negotiation-run-panel" id={negotiation.id}>
        ...
        />
        {negotiation.languages.length > 0 ? (
          <Field label="Languages" value={negotiation.languages.join(', ')} />
        ) : null}
        <div className="negotiation-content">
          <div>
            ...
            {props.negotiation.motivations.map((t, n) => (
              <Field
                key={n}
                label={t.trait}
                value={t.description || NegotiationLogic.getMotivationDescription(t.trait)}
              />
            ))}
          ...
          <div>
            ...
            {props.negotiation.pitfalls.map((t, n) => (
              <Field
                key={n}
                label={t.trait}
                value={t.description || NegotiationLogic.getPitfallDescription(t.trait)}
              />
            ))}
          ...
          <div>
            ...
            <Field
              highlight={negotiation.interest === 5}
              label="5"
              value={
                <Markdown text={props.negotiation.outcomes[5] || 'Yes, and...'} useSpan={true} />
              }
            />
            <Field
              highlight={negotiation.interest === 4}
              label="4"
              value={<Markdown text={props.negotiation.outcomes[4] || 'Yes'} useSpan={true} />}
            />
            <Field
              highlight={negotiation.interest === 3}
              label="3"
              value={
                <Markdown text={props.negotiation.outcomes[3] || 'Yes, but...'} useSpan={true} />
              }
            />
            <Field
              highlight={negotiation.interest === 2}
              label="2"
              value={
                <Markdown text={props.negotiation.outcomes[2] || 'No, but...'} useSpan={true} />
              }
            />
            <Field
              highlight={negotiation.interest === 1}
              label="1"
              value={<Markdown text={props.negotiation.outcomes[1] || 'No'} useSpan={true} />}
            />
            <Field
              highlight={negotiation.interest === 0}
              label="0"
              value={
                <Markdown text={props.negotiation.outcomes[0] || 'No, and...'} useSpan={true} />
              }
            />
          </div>
        </div>
