...
export const TerrainFilterPanel = (props: Props) => {
  ...
  const maxLevel =
    Collections.max(
      props.terrain.map(t => t.level),
      x => x,
    ) || 1;
  const maxEV =
    Collections.max(
      props.terrain.map(t => t.encounterValue),
      x => x,
    ) || 0;
    ...
    <ErrorBoundary>
      <div className="terrain-filter-panel">
        <Space orientation="vertical" style={{ width: '100%' }}>
          ...
          <Toggle
            label="By level"
            value={props.terrainFilter.level.length > 0}
          ...
          {props.terrainFilter.level.length > 0 ? (
            <>
              ...
              <Field
                label="Level"
                value={`${Math.min(...props.terrainFilter.level)} to ${Math.max(...props.terrainFilter.level)}`}
              />
            </>
          ) : null}
          <Toggle
            label="By EV"
            value={props.terrainFilter.ev.length > 0}
          ...
          {props.terrainFilter.ev.length > 0 ? (
            <>
              ...
              <Field
                label="EV"
                value={`${Math.min(...props.terrainFilter.ev)} to ${Math.max(...props.terrainFilter.ev)}`}
              />
            </>
          ) : null}
        </Space>
