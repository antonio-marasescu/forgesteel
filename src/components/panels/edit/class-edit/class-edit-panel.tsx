...
export const ClassEditPanel = (props: Props) => {
  ...
  const getClassEditSection = () => {
    ...
    const toggleCharacteristic = (index: number, characteristic: Characteristic) => {
      ...
      if (copy.primaryCharacteristicsOptions[index].includes(characteristic)) {
        copy.primaryCharacteristicsOptions[index] = copy.primaryCharacteristicsOptions[
          index
        ].filter(ch => ch !== characteristic);
      } else {
    ...
    const moveCharacteristicSet = (index: number, direction: 'up' | 'down') => {
      const copy = Utils.copy(heroClass);
      copy.primaryCharacteristicsOptions = Collections.move(
        copy.primaryCharacteristicsOptions,
        index,
        direction,
      );
      setHeroClass(copy);
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        <NumberSpin min={0} value={heroClass.subclassCount} onChange={setSubclassCount} />
        <HeaderText
          extra={<Button type="text" icon={<PlusOutlined />} onClick={addCharacteristicSet} />}
        >
        ...
        {heroClass.primaryCharacteristicsOptions.map((o, n) => (
          ...
          >
            <Space orientation="vertical" style={{ width: '100%' }}>
              <Toggle
                label={Characteristic.Might}
                value={o.includes(Characteristic.Might)}
                onChange={() => toggleCharacteristic(n, Characteristic.Might)}
              />
              <Toggle
                label={Characteristic.Agility}
                value={o.includes(Characteristic.Agility)}
                onChange={() => toggleCharacteristic(n, Characteristic.Agility)}
              />
              <Toggle
                label={Characteristic.Reason}
                value={o.includes(Characteristic.Reason)}
                onChange={() => toggleCharacteristic(n, Characteristic.Reason)}
              />
              <Toggle
                label={Characteristic.Intuition}
                value={o.includes(Characteristic.Intuition)}
                onChange={() => toggleCharacteristic(n, Characteristic.Intuition)}
              />
              <Toggle
                label={Characteristic.Presence}
                value={o.includes(Characteristic.Presence)}
                onChange={() => toggleCharacteristic(n, Characteristic.Presence)}
              />
        ...
        {heroClass.featuresByLevel.map(lvl => (
          <div key={lvl.level}>
            ...
            <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              {lvl.features.length === 0 ? <Empty /> : null}
            </Space>
          </div>
        ))}
      </>
  ...
  const getClassAbilitiesEditSection = () => {
    ...
    return (
      <>
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addAbility} />}>
          Abilities
        </HeaderText>
        <Space orientation="vertical" style={{ width: '100%' }}>
          {heroClass.abilities.map(a => (
            ...
            >
              <AbilityEditPanel ability={a} onChange={changeAbility} />
            </Expander>
          ))}
          {heroClass.abilities.length === 0 ? <Empty /> : null}
        </Space>
      </>
    );
  };
    ...
    return (
      <>
        ...
        <Space orientation="vertical" style={{ width: '100%' }}>
          ...
          {heroClass.subclasses.length === 0 ? <Empty /> : null}
          <Drawer
            open={drawerOpen}
            closeIcon={null}
    ...
    <ErrorBoundary>
      <Select
        options={[
          ...
          ...heroClass.subclasses.map(sc => ({
            label: `Subclass: ${sc.name || 'Unnamed Subclass'}`,
            value: sc.id,
          })),
        ]}
