...
export const CareerEditPanel = (props: Props) => {
  ...
  const getFeaturesEditSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        {career.features.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
  ...
  const getIncitingIncidentsSection = () => {
    ...
    const moveIncident = (e: Element, direction: 'up' | 'down') => {
      const copy = Utils.copy(career);
      const index = copy.incitingIncidents.options.findIndex(o => o.id === e.id);
      copy.incitingIncidents.options = Collections.move(
        copy.incitingIncidents.options,
        index,
        direction,
      );
      setCareer(copy);
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addIncident} />}>
          Inciting Incidents
        </HeaderText>
        {career.incitingIncidents.options.map(o => (
          ...
          >
            <ElementEditPanel element={o} onChange={changeIncident} />
          </Expander>
        ))}
        {career.incitingIncidents.options.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
