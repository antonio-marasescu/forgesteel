...
export const PlotEditPanel = (props: Props) => {
  ...
  const linkTargets = parentPlot.plots.filter(
    p => !plot.links.map(x => x.plotID).includes(p.id) && !upstreamIDs.includes(p.id),
  );
  ...
  const getContentSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        {plot.content.map(c => {
          ...
          switch (c.contentType) {
            ...
            case 'reference': {
              switch (c.type) {
                case 'encounter': {
                  const element = SourcebookLogic.getEncounters(props.sourcebooks).find(
                    e => e.id === c.contentID,
                  );
                  if (element) {
                    name = element.name;
                ...
                case 'montage': {
                  const element = SourcebookLogic.getMontages(props.sourcebooks).find(
                    m => m.id === c.contentID,
                  );
                  if (element) {
                    name = element.name;
                ...
                case 'negotiation': {
                  const element = SourcebookLogic.getNegotiations(props.sourcebooks).find(
                    n => n.id === c.contentID,
                  );
                  if (element) {
                    name = element.name;
                ...
                case 'tactical-map': {
                  const element = SourcebookLogic.getTacticalMaps(props.sourcebooks).find(
                    tm => tm.id === c.contentID,
                  );
                  if (element) {
                    name = element.name;
                    tag = 'Tactical Map';
        ...
        {plot.content.length === 0 ? <Empty text="No content" /> : null}
      </Space>
    );
  };
  ...
  const getLinksSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              <Popover
                ...
                content={
                  <Space orientation="vertical" style={{ width: '100%', padding: '0 5px' }}>
                    {linkTargets.map(p => (
                      <Button key={p.id} type="text" block={true} onClick={() => addLink(p)}>
        ...
        {plot.links.length === 0 ? <Empty text="No links" /> : null}
      </Space>
    );
  };
    ...
    <ErrorBoundary>
      <div className="plot-edit-panel">
        <Tabs
          ...
          tabBarExtraContent={
            <Flex gap={5}>
              <Button icon={<PlusOutlined />} onClick={() => props.onAddAfter(plot.id)}>
                Add Point After
              </Button>
              ...
              <Space orientation="vertical" style={{ width: '100%' }}>
                <Select
                  ...
                  options={[
                    ...
                    value: opt,
                    label: (
                      <div className="ds-text">{Format.capitalize(opt.split('-').join(' '))}</div>
                    ),
                  }))}
                  value={referenceType}
                ...
                <Divider />
                {Collections.sort(elements, e => e.element.name).map(e => (
                  <SelectablePanel
                    key={e.element.id}
                    onSelect={() => addContentReference(e.type, e.element.id)}
                  >
                    <HeaderText tags={[Format.capitalize(e.type.split('-').join(' '))]}>
                      {e.element.name}
                    </HeaderText>
