...
export const NegotiationEditPanel = (props: Props) => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        <Select
          style={{ width: '100%' }}
          options={[
            AttitudeType.Trusting,
            AttitudeType.Friendly,
            AttitudeType.Open,
            AttitudeType.Neutral,
            AttitudeType.Suspicious,
            AttitudeType.Hostile,
          ].map(a => ({ value: a, label: a, desc: NegotiationLogic.getAttitudeDescription(a) }))}
        ...
        <Select
          ...
          options={SourcebookLogic.getLanguages(props.sourcebooks).map(l => ({
            value: l.name,
            label: l.name,
            desc: l.description,
          }))}
  ...
  const getNegotiationMotivationsSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addMotivation} />}>
          Motivations
        </HeaderText>
          ...
          >
            ...
            <Space orientation="vertical" style={{ width: '100%' }}>
              <Select
                ...
                placeholder="Trait"
                options={[
                  NegotiationTrait.Benevolence,
                  NegotiationTrait.Discovery,
                  NegotiationTrait.Freedom,
                  NegotiationTrait.Greed,
                  NegotiationTrait.HigherAuthority,
                  NegotiationTrait.Justice,
                  NegotiationTrait.Legacy,
                  NegotiationTrait.Peace,
                  NegotiationTrait.Power,
                  NegotiationTrait.Protection,
                  NegotiationTrait.Revelry,
                  NegotiationTrait.Vengeance,
                ].map(nt => ({
                  label: nt,
                  value: nt,
                  desc: NegotiationLogic.getMotivationDescription(nt),
                }))}
                optionRender={option => (
              ...
              <MarkdownEditor
                ...
                value={m.description}
                onChange={value => setMotivationDescription(n, value)}
              />
            </Space>
          </Expander>
        ))}
        {negotiation.motivations.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
  ...
  const getNegotiationPitfallsSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addPitfall} />}>
          Pitfalls
        </HeaderText>
            ...
            <Space orientation="vertical" style={{ width: '100%' }}>
              <Select
                ...
                options={[
                  ...
                  label: nt,
                  value: nt,
                  desc: NegotiationLogic.getPitfallDescription(nt),
                }))}
                optionRender={option => (
                  <Field label={option.data.label} value={option.data.desc} />
                )}
              ...
              <MarkdownEditor
                placeholder="Description"
                value={p.description}
                onChange={value => setPitfallDescription(n, value)}
              />
            </Space>
          </Expander>
        ))}
        {negotiation.pitfalls.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
