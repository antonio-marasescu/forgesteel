...
export const MonsterGroupEditPanel = (props: Props) => {
  ...
  const getInformationEditSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addInformation} />}>
          Information
        </HeaderText>
        {monsterGroup.information.map(i => (
          ...
          >
            <ElementEditPanel element={i} onChange={changeInformation} />
          </Expander>
        ))}
        {monsterGroup.information.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
  ...
  const getMaliceEditSection = () => {
    ...
    const moveMaliceFeature = (
      feature: FeatureMalice | FeatureMaliceAbility,
      direction: 'up' | 'down',
    ) => {
      const copy = Utils.copy(monsterGroup);
      const index = copy.malice.findIndex(f => f.id === feature.id);
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <HeaderText
          extra={<Button type="text" icon={<PlusOutlined />} onClick={addMaliceFeature} />}
        >
          Malice
        </HeaderText>
        ...
        {monsterGroup.malice.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
  ...
  const getMonstersEditSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        {monsterGroup.monsters.length === 0 ? <Empty /> : null}
        <Flex gap={10}>
  ...
  const getMonstersCustomizationSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addAddOn} />}>
          Customizations
        </HeaderText>
        ...
        {monsterGroup.addOns.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
    ...
    <ErrorBoundary>
      <Select
        options={[
          ...
          ...monsterGroup.monsters.map(m => ({
            label: `Monster: ${m.name || 'Unnamed Monster'}`,
            value: m.id,
          })),
        ]}
