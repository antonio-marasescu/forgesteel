...
export const EncounterEditPanel = (props: Props) => {
  ...
  const [terrainFilter, setTerrainFilter] = useState<TerrainFilter>(
    FactoryLogic.createTerrainFilter(),
  );
  ...
  const getMonstersSection = () => {
    ...
    const warnings = [];
    const statblocks = Collections.distinct(
      encounter.groups.flatMap(g => g.slots).map(s => s.monsterID),
      s => s,
    ).length;
    if (statblocks > 6) {
      warnings.push(
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        {warnings}
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addGroup} />}>
          Monsters
        </HeaderText>
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <Flex align="center" justify="space-between" gap={10}>
          <Toggle
            label="Specify an encounter objective"
            value={!!encounter.objective}
            onChange={value => setObjective(value ? FactoryLogic.createEncounterObjective() : null)}
          />
          <Popover
            trigger="click"
            content={
              ...
              >
                {[
                  ...
                  <Button key={o.id} block={true} onClick={() => setObjective(o)}>
                    {o.name}
                  </Button>
                ))}
              </div>
        ...
        {encounter.objective ? (
          <>
            ...
            <HeaderText>Description</HeaderText>
            <MarkdownEditor
              value={encounter.objective.description}
              onChange={setObjectiveDescription}
            />
            <HeaderText>Difficulty Modifier</HeaderText>
            <MarkdownEditor
              value={encounter.objective.difficultyModifier}
              onChange={setObjectiveDifficultyModifier}
            />
            <HeaderText>Success Condition</HeaderText>
            <MarkdownEditor
              value={encounter.objective.successCondition}
              onChange={setObjectiveSuccessCondition}
            />
            <HeaderText>Failure Condition</HeaderText>
            <MarkdownEditor
              value={encounter.objective.failureCondition}
              onChange={setObjectiveFailureCondition}
            />
            <HeaderText>Victories</HeaderText>
            <MarkdownEditor
              value={encounter.objective.victories}
              onChange={setObjectiveVictories}
            />
          </>
        ) : null}
      </Space>
  ...
  const getNotesSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addNote} />}>
          Notes
        </HeaderText>
        {encounter.notes.map(i => (
          ...
          >
            <ElementEditPanel element={i} onChange={changeNote} />
          </Expander>
        ))}
        {encounter.notes.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
  ...
  const getMaliceSection = () => {
    ...
    const removeMaliceFeatureIds = (ids: string[]) => {
      const copy = Utils.copy(encounter);
      copy.hiddenMaliceFeatures = [
        ...copy.hiddenMaliceFeatures,
        ...ids.filter(id => !copy.hiddenMaliceFeatures.includes(id)),
      ];
  ...
  const getMonsterListSection = () => {
    ...
    const groups = Collections.sort(
      props.sourcebooks
        .flatMap(sb => sb.monsterGroups)
        .filter(g =>
          g.monsters.some(
            m =>
              m.role.organization !== MonsterOrganizationType.Retainer &&
              MonsterLogic.matches(m, monsterFilter),
          ),
        ),
      g => g.name,
    );
            ...
            <Space orientation="vertical" style={{ width: '100%' }}>
              {Collections.sort(
                ...
                <MonsterListItem
                  ...
                  monster={m}
                  monsterGroup={
                    SourcebookLogic.getMonsterGroup(props.sourcebooks, m.id) as MonsterGroup
                  }
                  encounter={encounter}
  ...
  const getTerrainListSection = () => {
    ...
    const terrains = Collections.sort(
      allTerrains.filter(m => TerrainLogic.matches(m, terrainFilter)),
      t => t.name,
    );
  ...
  const getDifficultySection = () => {
    ...
    const difficulty = EncounterDifficultyLogic.getDifficulty(
      strength,
      props.options,
      props.heroes,
    );
  ...
  return (
    <ErrorBoundary>
      <div className="encounter-group-panel">
          ...
          extra={
            <Flex>
              <Button
                ...
                type="text"
                icon={
                  editing ? <EditFilled style={{ color: 'rgb(22, 119, 255)' }} /> : <EditOutlined />
                }
              ...
              <DangerButton
                ...
                mode="clear"
                label="Delete Group"
                onConfirm={() => props.deleteGroup(props.group)}
              />
            </Flex>
          }
        >
        ...
        EncounterDifficultyLogic.getGroupStrength(props.group, props.sourcebooks) <
          EncounterDifficultyLogic.getHeroValue(props.options.heroLevel) ? (
          <Alert
            type="warning"
            showIcon={true}
        ...
        {props.group.slots.length > 0 &&
        EncounterDifficultyLogic.getGroupStrength(props.group, props.sourcebooks) >
          EncounterDifficultyLogic.getHeroValue(props.options.heroLevel) * 2 ? (
          <Alert
            type="warning"
            showIcon={true}
...
const MonsterSlotPanel = (props: MonsterSlotPanelProps) => {
  ...
  const monster = EncounterLogic.getCustomizedMonster(
    props.slot.monsterID,
    props.slot.customization,
    props.sourcebooks,
  );
        ...
        <Space orientation="vertical" style={{ width: '100%' }}>
          ...
          {monsterGroup.addOns.length > 0 ? (
            <Select
              ...
              placeholder="Select"
              mode="multiple"
              options={Collections.sort(monsterGroup.addOns, a => a.name).map(a => ({
                value: a.id,
                label: a.name,
                feature: a,
                cost: a.data.cost,
              ...
              optionRender={option => (
                <FeaturePanel
                  feature={option.data.feature}
                  options={props.options}
                  cost={option.data.cost}
                  mode={PanelMode.Full}
                />
              )}
          ...
          {originalMonster.role.organization === MonsterOrganizationType.Elite ||
          originalMonster.role.organization === MonsterOrganizationType.Leader ? (
            <Toggle
              label="Turn this monster into a Solo"
              value={props.slot.customization.convertToSolo}
              onChange={value => props.setSlotConvertToSolo(props.group.id, props.slot.id, value)}
            />
          ) : null}
            ...
            if (item) {
              return (
                <Flex align="center">
                  ...
                  <DangerButton
                    mode="icon"
                    onConfirm={() => props.removeItem(props.group.id, props.slot.id, itemID)}
                  />
                </Flex>
              );
            }
          ...
          <Select
            ...
            options={SourcebookLogic.getItems(props.sourcebooks).map(i => ({
              value: i.id,
              label: <Field label={i.name} value={i.description} />,
              data: i,
            }))}
          ...
          <DropdownButton
            ...
            items={[
              ...props.encounter.groups
                ...
                .map(g => ({
                  key: g.id,
                  label: <div className="ds-text centered-text">{g.name}</div>,
                })),
        ...
        <div className="slot-row">
            ...
            <Flex align="center" justify="space-between">
              ...
              <Flex align="center">
                <Button
                  ...
                  onClick={() => props.showMonster(monster, monsterGroup)}
                />
                <Button
                  type="text"
                  title="Customize"
                  icon={showCustomize ? <EditFilled /> : <EditOutlined />}
          ...
          <div className="actions">
            <NumberSpin
              ...
              format={value =>
                (value * MonsterLogic.getRoleMultiplier(monster.role.organization)).toString()
              }
  ...
  return <div className="slot-row">Unknown monster</div>;
};
...
const TerrainSlotPanel = (props: TerrainSlotPanelProps) => {
  ...
  const terrain = SourcebookLogic.getTerrains(props.sourcebooks).find(
    t => t.id === props.slot.terrainID,
  );
      ...
      return (
        <Space orientation="vertical" style={{ width: '100%' }}>
          ...
          <Select
            ...
            placeholder="Select"
            mode="multiple"
            options={Collections.sort(terrain.upgrades, a => a.label).map(a => ({
              value: a.id,
              label: a.label,
              cost: a.cost,
            }))}
            optionRender={option => (
              <Flex align="center" gap={8}>
                <div className="ds-text">{option.data.label}</div>
                <Pill>+{option.data.cost} EV</Pill>
              </Flex>
            )}
            ...
            <Flex align="center" justify="space-between">
              ...
              <Flex align="center">
                <Button
                  ...
                  title="Show stat block"
                  icon={<InfoCircleOutlined />}
                  onClick={() => props.showTerrain(terrain, props.slot.upgradeIDs)}
                />
  ...
  return <div className="terrain-row">Unknown terrain</div>;
};
...
const MonsterListItem = (props: MonsterListItemProps) => {
  const { attributes, listeners, setNodeRef } = useDraggable({
    id: props.monster.id,
    data: { type: 'monster', element: props.monster },
  });
  ...
  if (props.monsterGroup && props.showMonster) {
    showBtn = (
      <Button
        type="text"
        icon={<InfoCircleOutlined />}
        onClick={() => props.showMonster!(props.monster, props.monsterGroup!)}
      />
    );
  }
    ...
    if (props.encounter.groups.length === 0) {
      addBtn = (
        <Button
          type="text"
          icon={<PlusOutlined />}
          onClick={() => props.addMonster!(props.monster, null)}
        />
      );
    } else {
      addBtn = (
            ...
            <Space orientation="vertical">
              {props.encounter.groups.map((group, n) => (
                <Button
                  ...
                  type="text"
                  block={true}
                  onClick={() => props.addMonster!(props.monster, group.id)}
                >
                  Group {n + 1}
                </Button>
              ))}
...
const TerrainListItem = (props: TerrainListItemProps) => {
  const { attributes, listeners, setNodeRef } = useDraggable({
    id: props.terrain.id,
    data: { type: 'terrain', element: props.terrain },
  });
  ...
  if (props.showTerrain) {
    showBtn = (
      <Button
        type="text"
        icon={<InfoCircleOutlined />}
        onClick={() => props.showTerrain!(props.terrain, [])}
      />
    );
  }
  ...
  if (props.addTerrain) {
    addBtn = (
      <Button
        type="text"
        icon={<PlusOutlined />}
        onClick={() => props.addTerrain!(props.terrain)}
      />
    );
  }
