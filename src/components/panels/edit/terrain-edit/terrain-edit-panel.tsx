...
export const TerrainEditPanel = (props: Props) => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        <Select
          style={{ width: '100%' }}
          options={[
            TerrainCategory.SupernaturalObject,
            TerrainCategory.Environmental,
            TerrainCategory.Fieldwork,
            TerrainCategory.Mechanism,
            TerrainCategory.PowerFixture,
            TerrainCategory.SiegeEngine,
        ...
        <NumberSpin min={1} max={10} value={terrain.level} onChange={setLevel} />
        ...
        <Select
          ...
          options={[
            ...
            MonsterRoleType.Mount,
            MonsterRoleType.Support,
          ].map(type => ({
            label: type,
            value: type,
            desc: MonsterLogic.getRoleTypeDescription(type),
          }))}
        ...
        <NumberSpin
          min={1}
          value={terrain.encounterValue}
          steps={[1, 10]}
          onChange={setEncounterValue}
        />
        <TextInput placeholder="Area" allowClear={true} value={terrain.area} onChange={setArea} />
        <HeaderText>Stamina</HeaderText>
        <NumberSpin
          label="Base Stamina"
          min={0}
          value={terrain.stamina.base}
          steps={[1, 10]}
          onChange={setStaminaBase}
        />
        <NumberSpin
          label="Per Square"
          min={0}
          value={terrain.stamina.perSquare}
          steps={[1, 10]}
          onChange={setStaminaPerSquare}
        />
        ...
        {typeof terrain.size === 'string' ? (
          <TextInput placeholder="Size" allowClear={true} value={terrain.size} onChange={setSize} />
        ) : (
          <>
  ...
  const getTerrainDamageSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addDamageMod} />}>
          Damage Modifiers
        </HeaderText>
        ...
        {terrain.damageMods.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
  ...
  const getTerrainSectionsSection = () => {
    ...
    const moveSectionContent = (
      ...
      const copy = Utils.copy(terrain);
      copy.sections[sectionIndex].content = Collections.move(
        copy.sections[sectionIndex].content,
        contentIndex,
        direction,
      );
      setTerrain(copy);
    ...
    const setSectionContentFeature = (
      sectionIndex: number,
      contentIndex: number,
      value: FeatureText | FeatureAbility,
    ) => {
      const copy = Utils.copy(terrain);
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addSection} />}>
          Sections
        </HeaderText>
                ...
                <Expander
                  ...
                  extra={[
                    <Button
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        moveSectionContent(sectionIndex, contentIndex, 'up');
                      }}
                    />,
                    <Button
                      key="down"
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        moveSectionContent(sectionIndex, contentIndex, 'down');
                      }}
                    />,
                    <DangerButton
                      key="delete"
                      ...
                      onConfirm={e => {
                        e.stopPropagation();
                        deleteSectionContent(sectionIndex, contentIndex);
                      }}
                    />,
                  ]}
                >
                  <FeatureEditPanel
                    ...
                    onChange={f =>
                      setSectionContentFeature(
                        sectionIndex,
                        contentIndex,
                        f as FeatureText | FeatureAbility,
                      )
                    }
                  />
                </Expander>
              ))}
              {section.content.length === 0 ? <Empty /> : null}
              <Button icon={<PlusOutlined />} onClick={() => addSectionContent(sectionIndex)} />
            </Space>
          </Expander>
        ))}
        {terrain.sections.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
  ...
  const getTerrainCustomizationSection = () => {
    ...
    const moveUpgradeSection = (
      upgradeIndex: number,
      sectionIndex: number,
      direction: 'up' | 'down',
    ) => {
      const copy = Utils.copy(terrain);
      copy.upgrades[upgradeIndex].sections = Collections.move(
        copy.sections,
        sectionIndex,
        direction,
      );
    ...
    const moveUpgradeSectionContent = (
      upgradeIndex: number,
      sectionIndex: number,
      contentIndex: number,
      direction: 'up' | 'down',
      ...
      const copy = Utils.copy(terrain);
      copy.upgrades[upgradeIndex].sections[sectionIndex].content = Collections.move(
        copy.sections[sectionIndex].content,
        contentIndex,
        direction,
      );
    ...
    const deleteUpgradeSectionContent = (
      upgradeIndex: number,
      sectionIndex: number,
      contentIndex: number,
    ) => {
    ...
    const setUpgradeSectionContentFeature = (
      upgradeIndex: number,
      sectionIndex: number,
      contentIndex: number,
      value: FeatureText | FeatureAbility,
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addUpgrade} />}>
          Customizations
        </HeaderText>
        {terrain.upgrades.map((upgrade, upgradeIndex) => (
          ...
          >
            <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              <MarkdownEditor
                value={upgrade.text}
                onChange={value => setUpgradeText(upgradeIndex, value)}
              />
              <HeaderText>Cost</HeaderText>
              <NumberSpin
                min={1}
                value={upgrade.cost}
                onChange={value => setUpgradeCost(upgradeIndex, value)}
              />
              <HeaderText
                extra={
                  <Button
                    type="text"
                    icon={<PlusOutlined />}
                    onClick={() => addUpgradeSection(upgradeIndex)}
                  />
                }
              >
                Sections
                ...
                <Expander
                  ...
                  extra={[
                    <Button
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        moveUpgradeSection(upgradeIndex, sectionIndex, 'up');
                      }}
                    />,
                    <Button
                      key="down"
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        moveUpgradeSection(upgradeIndex, sectionIndex, 'down');
                      }}
                    />,
                    <DangerButton
                      key="delete"
                      ...
                      onConfirm={e => {
                        e.stopPropagation();
                        deleteUpgradeSection(upgradeIndex, sectionIndex);
                      }}
                    />,
                  ]}
                >
                    ...
                    <HeaderText
                      extra={
                        <Button
                          type="text"
                          icon={<PlusOutlined />}
                          onClick={() => addUpgradeSectionContent(upgradeIndex, sectionIndex)}
                        />
                      }
                    >
                      Content
                      ...
                      <Expander
                        ...
                        extra={[
                          ...
                          <DangerButton
                            ...
                            onConfirm={e => {
                              e.stopPropagation();
                              deleteUpgradeSectionContent(upgradeIndex, sectionIndex, contentIndex);
                            }}
                          />,
                        ]}
                      >
                        <FeatureEditPanel
                          ...
                          options={props.options}
                          onChange={f =>
                            setUpgradeSectionContentFeature(
                              upgradeIndex,
                              sectionIndex,
                              contentIndex,
                              f as FeatureText | FeatureAbility,
              ...
              {upgrade.sections.length === 0 ? <Empty /> : null}
            </Space>
          </Expander>
        ))}
        {terrain.upgrades.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
