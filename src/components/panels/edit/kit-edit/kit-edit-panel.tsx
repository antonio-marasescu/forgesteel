...
export const KitEditPanel = (props: Props) => {
  ...
  const getKitDetailsSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        <TextInput placeholder="Type" allowClear={true} value={kit.type} onChange={setType} />
        <Alert
          type="info"
          showIcon={true}
        ...
        <Select
          ...
          options={[KitArmor.Light, KitArmor.Medium, KitArmor.Heavy, KitArmor.Shield].map(
            option => ({ value: option }),
          )}
  ...
  const getKitStatsEditSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        <NumberSpin min={0} value={kit.stamina} onChange={setStamina} />
        <HeaderText>Speed</HeaderText>
        <NumberSpin min={0} value={kit.speed} onChange={setSpeed} />
        <HeaderText>Stability</HeaderText>
        <NumberSpin min={0} value={kit.stability} onChange={setStability} />
        <HeaderText>Distance</HeaderText>
        <NumberSpin label="Melee" min={0} value={kit.meleeDistance} onChange={setMeleeDistance} />
        <NumberSpin
          label="Ranged"
          min={0}
        ...
        <NumberSpin min={0} value={kit.disengage} onChange={setDisengage} />
      </Space>
    );
  };
      ...
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        {kit.meleeDamage ? (
          <NumberSpin
            label="Tier 1"
            min={0}
            value={kit.meleeDamage.tier1}
            onChange={setMeleeDamage1}
          />
        ) : null}
        {kit.meleeDamage ? (
          <NumberSpin
            label="Tier 2"
            min={0}
            value={kit.meleeDamage.tier2}
            onChange={setMeleeDamage2}
          />
        ) : null}
        {kit.meleeDamage ? (
          <NumberSpin
            label="Tier 3"
            min={0}
            value={kit.meleeDamage.tier3}
            onChange={setMeleeDamage3}
          />
        ) : null}
        ...
        {kit.rangedDamage ? (
          <NumberSpin
            label="Tier 1"
            min={0}
            value={kit.rangedDamage.tier1}
            onChange={setRangedDamage1}
          />
        ) : null}
        {kit.rangedDamage ? (
          <NumberSpin
            label="Tier 2"
            min={0}
            value={kit.rangedDamage.tier2}
            onChange={setRangedDamage2}
          />
        ) : null}
        {kit.rangedDamage ? (
          <NumberSpin
            label="Tier 3"
            min={0}
            value={kit.rangedDamage.tier3}
            onChange={setRangedDamage3}
          />
        ) : null}
      </Space>
    );
  ...
  const getFeaturesEditSection = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        {kit.features.length === 0 ? <Empty /> : null}
      </Space>
    );
  };
  ...
  const getTuningSection = () => {
    ...
    const minMeleeDamage = kit.meleeDamage
      ? Math.min(kit.meleeDamage.tier1, kit.meleeDamage.tier2, kit.meleeDamage.tier3)
      : 0;
    ...
    const minRangedDamage = kit.rangedDamage
      ? Math.min(kit.rangedDamage.tier1, kit.rangedDamage.tier2, kit.rangedDamage.tier3)
      : 0;
    ...
    constraints.push({
      name: 'Has disengage OR stability',
      value:
        (kit.disengage > 0 && kit.stability === 0) || (kit.disengage === 0 && kit.stability < 0),
    });
    ...
    gear.push({
      name: 'Heavy Weapon (melee)',
      value: !!kit.meleeDamage && kit.meleeDamage.tier3 >= 4,
    });
    gear.push({ name: 'Light Weapon (ranged)', value: minRangedDamage >= 1 });
    gear.push({ name: 'Medium Weapon (ranged)', value: minRangedDamage >= 2 });
    gear.push({
      name: 'Heavy Weapon (ranged)',
      value: !!kit.rangedDamage && kit.rangedDamage.tier3 >= 4,
    });
    ...
    return (
      <div>
        ...
        <div className="ds-text">
          The power level of a kit should be <b>8</b>. The calculation takes a number of kit
          statistics into account, listed below.
        </div>
        <StatsRow>
          {powerA.map((p, n) => (
            <Field key={n} orientation="vertical" label={p.name} value={p.value} />
          ))}
        </StatsRow>
        <StatsRow>
          {powerB.map((p, n) => (
        ...
        {constraints.map((c, n) => (
          <CheckLabel key={n} state={c.value ? 'success' : 'failure'}>
            <div style={{ fontWeight: c.value ? '400' : '600', opacity: c.value ? '0.5' : '1' }}>
              {c.name}
            </div>
          </CheckLabel>
        ))}
        ...
        {gear.map((c, n) => (
          <CheckLabel key={n} state={c.value ? 'success' : 'failure'}>
            <div style={{ fontWeight: c.value ? '600' : '400', opacity: c.value ? '1' : '0.5' }}>
              {c.name}
            </div>
          </CheckLabel>
        ))}
