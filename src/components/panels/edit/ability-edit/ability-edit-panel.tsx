    ...
    const setDistanceMainType = (index: number, value: string) => {
      ...
      switch (value) {
        ...
        case 'Ranged':
          ...
          break;
        case 'Area':
          copy.distance[index] = FactoryLogic.distance.create({
      ...
      <div>
        ...
        <Space orientation="vertical" style={{ width: '100%' }}>
          <Select
            ...
            options={[
              AbilityUsage.MainAction,
              AbilityUsage.Maneuver,
              AbilityUsage.Move,
              AbilityUsage.Trigger,
              AbilityUsage.VillainAction,
              AbilityUsage.ChampionAction,
              AbilityUsage.NoAction,
              AbilityUsage.Other,
          ...
          {ability.type.usage === AbilityUsage.VillainAction ? (
            <NumberSpin
              label="Villain Action Order"
              min={1}
              max={3}
              value={ability.type.order || 1}
          ...
          ability.type.usage === AbilityUsage.Maneuver ||
          ability.type.usage === AbilityUsage.Trigger ? (
            <Toggle label="Free" value={ability.type.free} onChange={setTypeFree} />
          ) : null}
          <Toggle
            label="Can be used as a free strike"
            value={ability.type.freeStrike}
            onChange={setTypeFreeStrike}
          />
        </Space>
        ...
        />
        <HeaderText extra={<Button type="text" icon={<PlusOutlined />} onClick={addDistance} />}>
          Distance
        </HeaderText>
        <Space orientation="vertical" style={{ width: '100%' }}>
            ...
            >
              <Space orientation="vertical" style={{ width: '100%' }}>
                ...
                <Select
                  ...
                  options={['Self', 'Melee', 'Ranged', 'Area', 'Line', 'Summoner', 'Special'].map(
                    option => ({ value: option }),
                  )}
                  optionRender={option => <div className="ds-text">{option.data.value}</div>}
                ...
                />
                {getDistanceMainType(n) === 'Area' ? <HeaderText>Area Type</HeaderText> : null}
                ...
                {getDistanceMainType(n) !== 'Self' &&
                getDistanceMainType(n) !== 'Summoner Range' &&
                ...
                getDistanceMainType(n) !== 'Special' ? (
                  <NumberSpin
                    ...
                    onChange={value => setDistanceValue(n, value)}
                  />
                ) : null}
                {getDistanceMainType(n) === 'Line' ? <HeaderText>Value 2</HeaderText> : null}
                {getDistanceMainType(n) === 'Line' ? (
                  <NumberSpin
                    min={1}
                    value={distance.value2}
                    onChange={value => setDistanceValue2(n, value)}
                  />
                ) : null}
                ...
                {getDistanceMainType(n) === 'Area' || getDistanceMainType(n) === 'Line' ? (
                  <NumberSpin
                    ...
                    onChange={value => setDistanceWithin(n, value)}
                  />
                ) : null}
                {getDistanceMainType(n) === 'Special' ? <HeaderText>Special</HeaderText> : null}
                {getDistanceMainType(n) === 'Special' ? (
                  <TextInput
          ...
          {ability.distance.length === 0 ? <Empty /> : null}
        </Space>
        ...
        <Toggle
          label="Signature"
          value={ability.cost === 'signature'}
          onChange={value => setCost(value ? 'signature' : 0)}
        />
  ...
  const getContentPage = () => {
    ...
    return (
      <div>
            ...
            <Popover
              ...
              content={
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <Button type="text" onClick={() => addSection('text')}>
                    Add Text
                  </Button>
                  <Button type="text" onClick={() => addSection('field')}>
                    Add a Field
                  </Button>
                  <Button type="text" onClick={() => addSection('roll')}>
                    Add a Roll
                  </Button>
                  <Button type="text" onClick={() => addSection('package')}>
                    Add a Package
                  </Button>
                </div>
              }
                  ...
                  return (
                    <Space orientation="vertical" style={{ width: '100%' }}>
                      ...
                      <Toggle
                        ...
                        onChange={setFieldSectionRepeatable}
                      />
                    </Space>
                  );
                }
                case 'roll': {
                  ...
                  return (
                    <Space orientation="vertical" style={{ width: '100%' }}>
                      ...
                      />
                      <NumberSpin
                        disabled={section.roll.characteristic.length > 0}
                        label="Bonus"
                        min={0}
                        value={section.roll.bonus}
                        onChange={setRollSectionBonus}
                      />
                      {section.roll.characteristic.length === 0 && section.roll.bonus === 0 ? (
                        <Alert
                          type="warning"
        ...
        {ability.sections.length === 0 ? <Empty /> : null}
      </div>
    );
  };
