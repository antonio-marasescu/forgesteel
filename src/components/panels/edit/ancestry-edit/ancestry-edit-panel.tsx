...
export const AncestryEditPanel = (props: Props) => {
  ...
  const getPurchasedEditSection = () => {
    ...
    const changeFeature = (f: { feature: Feature; value: number }) => {
      ...
      if (choice) {
        const index = (choice as FeatureChoice).data.options.findIndex(
          x => x.feature.id === f.feature.id,
        );
        if (index !== -1) {
    ...
    const moveFeature = (featureID: string, direction: 'up' | 'down') => {
      ...
      if (choice) {
        ...
        (choice as FeatureChoice).data.options = Collections.move(
          (choice as FeatureChoice).data.options,
          index,
          direction,
        );
        setAncestry(copy);
    ...
    const deleteFeature = (featureID: string) => {
      ...
      const choice = copy.features.find(x => x.id === choiceFeature.id);
      if (choice) {
        (choice as FeatureChoice).data.options = (choice as FeatureChoice).data.options.filter(
          f => f.feature.id !== featureID,
        );
        setAncestry(copy);
        props.onChange(copy);
        ...
        {features.map(f => (
          ...
          >
            ...
            <NumberSpin
              ...
              max={2}
              value={f.value}
              onChange={v => changeFeature({ feature: f.feature, value: v })}
            />
            <FeatureEditPanel
              feature={f.feature}
              sourcebooks={props.sourcebooks}
  ...
  const getCherryPick = () => {
    ...
    const availableSignatureFeatures = SourcebookLogic.getAncestries(props.sourcebooks)
      .flatMap(AncestryLogic.getSignatureFeatures)
      .filter(f => !currentFeatureNames.includes(f.name))
      .map(f => ({ feature: f, value: 0 }));
    const availablePurchasedFeatures = SourcebookLogic.getAncestries(props.sourcebooks)
      .flatMap(AncestryLogic.getPurchasedFeatures)
      .filter(f => !currentFeatureNames.includes(f.feature.name));

    const features = Collections.sort(
      Collections.distinct(
        [...availableSignatureFeatures, ...availablePurchasedFeatures],
        f => f.feature.name,
      ),
      f => f.feature.name,
    );
