...
export const FollowerEditPanel = (props: Props) => {
  ...
  const getDetailsEditSection = () => {
    ...
    const setFollowerCharacteristics = (
      value: { characteristic: Characteristic; value: number }[],
    ) => {
      const copy = Utils.copy(follower) as Follower;
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        <Select
          ...
          options={FollowerLogic.getCharacteristicArrays(follower.type).map(o => ({
            value: o
              .filter(ch => ch.value !== 0)
              .map(ch => `${ch.characteristic} ${ch.value}`)
              .join(', '),
            array: o,
          }))}
          optionRender={option => <div className="ds-text">{option.data.value}</div>}
          value={follower.characteristics
            .filter(ch => ch.value !== 0)
            .map(ch => `${ch.characteristic} ${ch.value}`)
            .join(', ')}
          onChange={(_text, option) => {
            const data = option as unknown as {
              value: string;
              array: { characteristic: Characteristic; value: number }[];
            };
        ...
        <Select
          ...
          allowClear={true}
          placeholder="Skills"
          options={FollowerLogic.getSkillOptions(follower.type, props.sourcebooks).map(s => ({
            value: s.name,
            label: s.name,
            desc: s.description,
          }))}
        ...
        <Select
          ...
          options={FollowerLogic.getLanguageOptions(props.sourcebooks).map(s => ({
            value: s.name,
            label: s.name,
            desc: s.description,
          }))}
