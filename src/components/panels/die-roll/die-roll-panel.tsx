...
export const DieRollPanel = (props: Props) => {
  ...
  const setTierResult = (rolls: number[], rollState: RollState) => {
    if (props.onRoll) {
      ...
      const total = Collections.sum(
        [...rolls, ...props.modifiers, RollLogic.getBonus(rollState)],
        r => r,
      );
      if (total <= 11) {
        tier = 1;
  ...
  switch (props.type) {
    ...
    case 'Saving Throw':
      max = 10;
      marks[1] = <div className="ds-text dimmed-text small-text">1</div>;
      marks[(props.hero ? HeroLogic.getSaveThreshold(props.hero) : 6) - 0.5] = (
        <div className="ds-text dimmed-text small-text">-</div>
      );
  ...
  return (
    <ErrorBoundary>
      <div className="die-roll-panel">
          ...
          <div className="result-row">
            ...
            {props.type === 'Power Roll'
              ? props.modifiers
                  .filter(m => m !== 0)
                  .map((m, n) => (
                    <Statistic key={n} title="Modifier" value={`${m >= 0 ? '+' : ''}${m}`} />
                  ))
              : null}
            {props.type === 'Power Roll' && bonus ? (
              <Statistic
                title={bonus > 0 ? 'Edge' : 'Bane'}
                value={`${bonus >= 0 ? '+' : ''}${bonus}`}
              />
            ) : null}
        ...
        {tierMessage ? <Alert type="warning" showIcon={true} title={tierMessage} /> : null}
        {props.type === 'Power Roll' && Collections.sum(results, r => r) >= 19 ? (
          <Alert type="success" showIcon={true} title="Critical hit!" />
        ) : null}
