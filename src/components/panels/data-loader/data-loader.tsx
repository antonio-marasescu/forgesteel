...
export const DataLoader = (props: Props) => {
  ...
  async function updateLoadingStatus<T>(
    getterPromise: Promise<T>,
    setStateFunc: (value: SetStateAction<LoadingStatus>) => void,
  ): Promise<T> {
        ...
        Promise.all(promises)
          .then(results => {
            ...
            if (playbook) {
              ...
              if (
                playbook.adventures.length > 0 ||
                playbook.encounters.length > 0 ||
                playbook.montages.length > 0 ||
                playbook.negotiations.length > 0 ||
                playbook.tacticalMaps.length > 0
              ) {
                ...
                sb.adventures.push(
                  ...playbook.adventures.filter(
                    adventure => !sb.adventures.some(a => a.id === adventure.id),
                  ),
                );
                sb.encounters.push(
                  ...playbook.encounters.filter(
                    encounter => !sb.encounters.some(e => e.id === encounter.id),
                  ),
                );
                sb.montages.push(
                  ...playbook.montages.filter(
                    montage => !sb.montages.some(m => m.id === montage.id),
                  ),
                );
                sb.negotiations.push(
                  ...playbook.negotiations.filter(
                    negotiation => !sb.negotiations.some(n => n.id === negotiation.id),
                  ),
                );
                sb.tacticalMaps.push(
                  ...playbook.tacticalMaps.filter(
                    map => !sb.tacticalMaps.some(tm => tm.id === map.id),
                  ),
                );
    ...
    <div className="data-loader">
      <div className="data-loader-container">
        ...
        <Flex vertical={true}>
          ...
          {error ? (
            <Alert type="error" showIcon={true} title="Data load error" description={error} />
          ) : null}
          {error &&
          connectionSettings?.useWarehouse &&
          FeatureFlags.hasFlag(FeatureFlags.warehouse.code) ? (
            <Flex gap="small" justify="space-between" vertical={true}>
              ...
              />
              <Button block={true} type="primary" onClick={() => location.reload()}>
                Retry
              </Button>
