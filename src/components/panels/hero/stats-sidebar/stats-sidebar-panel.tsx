      ...
      <div className="stats-sidebar-panel">
        ...
        <div className={props.showStats ? 'stats-section' : 'stats-section ghost'}>
          <Field
            orientation="vertical"
            label="Size"
            value={FormatLogic.getSize(HeroLogic.getSize(props.hero))}
          />
          <Field
            orientation="vertical"
            label="Spd"
            value={FormatLogic.getSpeed(HeroLogic.getSpeed(props.hero))}
          />
          ...
          {saveBonus !== 0 ? (
            <Field
              orientation="vertical"
              label="Save"
              value={`${HeroLogic.getSaveThreshold(props.hero)} ${saveBonus > 0 ? '+' : ''}${saveBonus}`}
            />
          ) : (
            <Field
              orientation="vertical"
