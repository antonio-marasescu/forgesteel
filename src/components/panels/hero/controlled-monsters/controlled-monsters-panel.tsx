...
export const ControlledMonstersPanel = (props: Props) => {
  ...
  const getSlot = (slot: EncounterSlot) => {
    const isMinionSlot = slot.monsters.every(
      m => m.role.organization === MonsterOrganizationType.Minion,
    );
    const isRetainerSlot = slot.monsters.every(
      m => m.role.organization === MonsterOrganizationType.Retainer,
    );
    ...
    return (
      ...
      >
        ...
        {isMinionSlot ? (
          <div
            className="controlled-monster"
            onClick={() => props.onSelectControlledSquad(props.hero, slot)}
          >
