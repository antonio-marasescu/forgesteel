...
export const HeroPanel = (props: Props) => {
  ...
  const getNameSection = () => {
    return (
      <HeaderText
        ...
        ribbon={
          props.hero.picture ? (
            <HeroToken hero={props.hero} size={props.options.compactView ? 21 : 34} />
          ) : null
        }
  ...
  const getSidebarSection = () => {
    ...
    const abilities = HeroLogic.getAbilities(
      props.hero,
      props.sourcebooks,
      props.options.shownStandardAbilities,
    );
    ...
    const getTrigger = (ability: Ability) => {
      ...
      const distance = ability.distance
        .map(d => AbilityLogic.getDistance(d, ability, props.hero))
        .join(' or ');
      ...
      return (
        <div key={ability.id}>
          ...
          {showTarget && ability.target && ability.target !== distance ? (
            <Field compact={true} label="Target" value={ability.target} />
          ) : null}
        </div>
      );
    };
          ...
          <div key={label} className="overview-tile clickable" onClick={onShowSkills}>
            ...
            {skills.map(s => (
              <div key={s.name} className="ds-text">
                {s.name}{' '}
                {props.options.showSkillsInGroups ? null : <Tag variant="outlined">{s.list}</Tag>}
              </div>
            ))}
          </div>
        )
        ...
        <div className={`hero-sidebar ${display}`}>
            ...
            useRows ? (
              ...
              <div className="overview-tile danger clickable" onClick={onShowVitals}>
                ...
                <div className="ds-text">
                  You can’t take the Catch Breath maneuver in combat, and you are bleeding, and this
                  condition can’t be removed in any way until you are no longer dying.
                </div>
                <div className="ds-text">
                  Your allies can help you spend Recoveries in combat, and you can spend Recoveries
              ...
              {heroicResources.map(hr =>
                useRows ? (
                  <div
                    ...
                    className={
                      hr.value >= 0
                        ? 'selectable-row clickable'
                        : 'selectable-row warning clickable'
                    }
                    onClick={onShowStats}
                  >
                    <div>
                      Resource: <b>{hr.name}</b>
                    </div>
                    <div>{hr.value}</div>
                  </div>
                ) : (
                  <div
                    key={hr.id}
                    className={
                      hr.value >= 0 ? 'overview-tile clickable' : 'overview-tile warning clickable'
                    }
                    onClick={onShowStats}
                  >
                    <HeaderText
          ...
          {triggers.length > 0 && !props.options.singlePage ? (
            useRows ? (
              <div className="selectable-row clickable" onClick={() => setTab('Triggers')}>
                <div>
                  Triggers: <b>{triggers.map(t => t.ability.name).join(', ')}</b>
                </div>
              </div>
            ) : (
              <div className="overview-tile clickable" onClick={() => setTab('Triggers')}>
                <HeaderText>Triggered Actions</HeaderText>
                <Space orientation="vertical">{triggers.map(t => getTrigger(t.ability))}</Space>
              </div>
            )
          ) : null}
          ...
          {damageImmunities.length > 0 ? (
            useRows ? (
              <div className="selectable-row">
                <div>
                  Immunities:{' '}
                  <b>{damageImmunities.map(dm => `${dm.damageType} ${dm.value}`).join(', ')}</b>
                </div>
              </div>
            ) : (
              <div className="overview-tile">
                <HeaderText>Immunities</HeaderText>
                {damageImmunities.map((dm, n) => (
                  <div key={n} className="ds-text damage-modifier">
                    <span>{dm.damageType}</span>
                    <span>{dm.value}</span>
                  </div>
                ))}
              </div>
          ...
          {damageWeaknesses.length > 0 ? (
            useRows ? (
              <div className="selectable-row">
                <div>
                  Weaknesses:{' '}
                  <b>{damageWeaknesses.map(dm => `${dm.damageType} ${dm.value}`).join(', ')}</b>
                </div>
              </div>
            ) : (
              <div className="overview-tile">
                <HeaderText>Weaknesses</HeaderText>
                {damageWeaknesses.map((dm, n) => (
                  <div key={n} className="ds-text damage-modifier">
                    <span>{dm.damageType}</span>
                    <span>{dm.value}</span>
                  </div>
          ...
          {useRows ? (
            <div className="selectable-row clickable" onClick={onShowLanguages}>
              <div>
                Languages: <b>{languages.map(l => l.name).join(', ')}</b>
              </div>
            </div>
          ) : (
            <div className="overview-tile clickable" onClick={onShowLanguages}>
              ...
              {languages.length > 0 ? (
                languages.map(l => (
                  <div key={l.name} className="ds-text">
                    {l.name}
                  </div>
                ))
              ) : (
          ...
          {props.options.showSkillsInGroups || false
            ? [
                ...
                getSkills(
                  `${list} Skills`,
                  HeroLogic.getSkills(props.hero, props.sourcebooks).filter(s => s.list === list),
                ),
              )
  ...
  const getStatsSection = () => {
    ...
    const xpSuffix = HeroLogic.canLevelUp(props.hero, props.options) ? (
      <ArrowUpOutlined />
    ) : undefined;
    ...
    const speedStr =
      speed.modes.length === 0
        ? 'Speed'
        : `Speed (${FormatLogic.getSpeedModes(speed.modes).toLowerCase()})`;

    const maxStamina = HeroLogic.getStamina(props.hero);
    const stamina =
      props.hero.state.staminaDamage === 0
        ? maxStamina
        : maxStamina - props.hero.state.staminaDamage;
    ...
    const recoveries =
      props.hero.state.recoveriesUsed === 0
        ? maxRecoveries
        : maxRecoveries - props.hero.state.recoveriesUsed;
    ...
    return (
      <ErrorBoundary>
        <div className="stats-section">
          <Flex gap={10}>
            <StatsRow
              caption={isSmall ? 'M' : 'Might'}
              onClick={() => onSelectCharacteristic(Characteristic.Might)}
              style={{ flex: '1 1 0' }}
            >
              <Statistic value={HeroLogic.getCharacteristic(props.hero, Characteristic.Might)} />
            </StatsRow>
            <StatsRow
              caption={isSmall ? 'A' : 'Agility'}
              onClick={() => onSelectCharacteristic(Characteristic.Agility)}
              style={{ flex: '1 1 0' }}
            >
              <Statistic value={HeroLogic.getCharacteristic(props.hero, Characteristic.Agility)} />
            </StatsRow>
            <StatsRow
              caption={isSmall ? 'R' : 'Reason'}
              onClick={() => onSelectCharacteristic(Characteristic.Reason)}
              style={{ flex: '1 1 0' }}
            >
              <Statistic value={HeroLogic.getCharacteristic(props.hero, Characteristic.Reason)} />
            </StatsRow>
            <StatsRow
              caption={isSmall ? 'I' : 'Intuition'}
              onClick={() => onSelectCharacteristic(Characteristic.Intuition)}
              style={{ flex: '1 1 0' }}
            >
              <Statistic
            ...
            <StatsRow
              caption={isSmall ? 'P' : 'Presence'}
              onClick={() => onSelectCharacteristic(Characteristic.Presence)}
              style={{ flex: '1 1 0' }}
            >
            ...
            <>
              <div className="selectable-row clickable" onClick={onShowResources}>
                {HeroLogic.getHeroicResources(props.hero).map(hr => (
                  <div key={hr.id}>
                    {hr.name}: <b>{hr.value}</b>
                  </div>
                ))}
                <div>
                  Surges: <b>{props.hero.state.surges}</b>
                </div>
                <div>
                  Victories: <b>{props.hero.state.victories}</b>
                </div>
                <div>
                  XP: <b>{props.hero.state.xp}</b>
                </div>
                <div>
                  Renown: <b>{HeroLogic.getRenown(props.hero)}</b>
                </div>
                <div>
                  Wealth: <b>{HeroLogic.getWealth(props.hero)}</b>
                </div>
              </div>
              <div className="selectable-row">
                <div>
                  Size: <b>{FormatLogic.getSize(size)}</b>
                </div>
                <div>
                  {speedStr}: <b>{speed.value}</b>
                </div>
                <div>
                  Stability: <b>{HeroLogic.getStability(props.hero)}</b>
                </div>
                <div>
                  Disengage: <b>{HeroLogic.getDisengage(props.hero)}</b>
                </div>
                <div>
                  Save: <b>{HeroLogic.getSaveThreshold(props.hero)}</b>
                </div>
              </div>
              <div className="selectable-row clickable" onClick={onShowVitals}>
                ...
                <div>
                  Recoveries: <b>{recoveries}</b>
                </div>
                <div>
                  Recovery Value: <b>{HeroLogic.getRecoveryValue(props.hero)}</b>
                </div>
              </div>
            </>
          ) : (
              ...
              <Flex gap={10}>
                <StatsRow caption="Statistics" style={{ flex: '5 5 0' }}>
                  ...
                  <Statistic
                    ...
                    suffix={
                      HeroLogic.getSaveBonus(props.hero)
                        ? `+${HeroLogic.getSaveBonus(props.hero)}`
                        : undefined
                    }
                  />
          ...
          {props.hero.ancestry ? (
            useRows ? (
              <div className="selectable-row clickable" onClick={onSelectAncestry}>
                <div>
                  Ancestry: <b>{props.hero.ancestry.name}</b>
                </div>
              </div>
            ) : (
              <div className="overview-tile clickable" onClick={onSelectAncestry}>
                ...
                {HeroLogic.getFormerAncestries(props.hero).map(a => (
                  <Field key={a.id} label="Former Life" value={a.name} />
                ))}
              </div>
            )
          ) : (
          ...
          {props.hero.culture ? (
            useRows ? (
              <div className="selectable-row clickable" onClick={onSelectCulture}>
                <div>
                  Culture: <b>{props.hero.culture.name}</b>
                </div>
              </div>
            ) : (
          ...
          {props.hero.career ? (
            useRows ? (
              <div className="selectable-row clickable" onClick={onSelectCareer}>
                <div>
                  Career: <b>{props.hero.career.name}</b>
                </div>
              </div>
            ) : (
              <div className="overview-tile clickable" onClick={onSelectCareer}>
                ...
                {incitingIncident ? (
                  <Field label="Inciting Incident" value={incitingIncident.name} />
                ) : null}
              </div>
            )
          ) : (
          ...
          {props.hero.class ? (
            useRows ? (
              <div className="selectable-row clickable" onClick={onSelectClass}>
                {props.hero.class.subclasses.filter(sc => sc.selected).length > 0 ? (
                  <div>
                    Class:{' '}
                    <b>
                      {props.hero.class.name} (
                      {props.hero.class.subclasses
                        .filter(sc => sc.selected)
                        .map(sc => sc.name)
                        .join(' ')}
                      , level {props.hero.class.level})
                    </b>
                  </div>
                ) : (
                  <div>
                    Class:{' '}
                    <b>
                      {props.hero.class.name} (level {props.hero.class.level})
                    </b>
                  </div>
                )}
              </div>
              ...
              <div className="overview-tile clickable" onClick={onSelectClass}>
                ...
                {props.hero.class.subclasses.filter(sc => sc.selected).length > 0 ? (
                  <Field
                    label={props.hero.class.subclassName}
                    value={
                      props.hero.class.subclasses
                        .filter(sc => sc.selected)
                        .map(sc => sc.name)
          ...
          {HeroLogic.getDomains(props.hero).length > 0
            ? HeroLogic.getDomains(props.hero).map(domain =>
                useRows ? (
                  <div
                    key={domain.id}
                    className="selectable-row clickable"
            ...
            ? HeroLogic.getKits(props.hero).map(kit =>
                useRows ? (
                  ...
                  >
                    ...
                    {kit.armor.length > 0 ? (
                      <Field label="Armor" value={kit.armor.join(', ')} />
                    ) : null}
                    {kit.weapon.length > 0 ? (
                      <Field label="Weapons" value={kit.weapon.join(', ')} />
                    ) : null}
                  </div>
                ),
              )
          ...
          {props.hero.complication ? (
            useRows ? (
              <div className="selectable-row clickable" onClick={onSelectComplication}>
                <div>
                  Complication: <b>{props.hero.complication.name}</b>
                </div>
              </div>
            ) : (
            ...
            ? props.hero.state.projects.map(project =>
                useRows ? (
                  ...
                  >
                    ...
                    {project.progress ? (
                      <Field label="State" value={ProjectLogic.getStatus(project)} />
                    ) : null}
                  </div>
                ),
              )
  ...
  const getFeaturesSection = () => {
    ...
    const features = HeroLogic.getFeatures(props.hero).filter(f =>
      featureTypes.includes(f.feature.type),
    );
    ...
    const getRow = (data: { feature: Feature; source: string }) => {
      return (
        <div
          key={data.feature.id}
          className="selectable-row clickable"
          onClick={() => showFeature(data.feature)}
        >
          <div>
            <b>{data.feature.name}</b>
          </div>
    ...
    const mainFeatures = features.filter(
      f => !props.options.separateInventoryFeatures || !itemNames.includes(f.source),
    );
    const inventoryFeatures = features.filter(
      f => props.options.separateInventoryFeatures && itemNames.includes(f.source),
    );
        ...
        <div className="features-section">
          {mainFeatures.length > 0 ? (
            <div className={`features-grid ${useRows ? 'compact' : ''}`}>
              {useRows ? (
                <HeaderText level={props.options.compactView ? 3 : 1}>Features</HeaderText>
              ) : null}
              {mainFeatures.map(f =>
                useRows ? (
                  getRow(f)
  ...
  const getAbilitiesSection = (
    title: string,
    abilities: { ability: Ability; source: string }[],
  ) => {
    ...
    const getRow = (data: { ability: Ability; source: string }) => {
      return (
        <div
          key={data.ability.id}
          className="selectable-row clickable"
          onClick={() => showAbility(data.ability)}
        >
          <div>
            <b>{data.ability.name}</b>
          </div>
          <div>
            {data.ability.distance
              .map(d => AbilityLogic.getDistance(d, data.ability, props.hero))
              .join(' or ')}
          </div>
          ...
          {data.ability.cost === 'signature' ? (
            ...
            <ResourcePill value={data.ability.cost} repeatable={data.ability.repeatable} />
          ) : null}
    ...
    return (
      <ErrorBoundary>
        <div className="abilities-section">
          {useRows ? (
            <HeaderText level={props.options.compactView ? 3 : 1}>{title}</HeaderText>
          ) : null}
          {nonStandard.length === 0 && standard.length === 0 ? <Empty /> : null}
          <div
            className={`abilities-grid ${useRows ? 'compact' : ''} ${props.options.abilityWidth.toLowerCase().replace(' ', '-')}`}
          >
            {nonStandard.map(a =>
              useRows ? (
                getRow(a)
                ...
                <SelectablePanel
                  ...
                  style={
                    isSmall
                      ? undefined
                      : { gridColumn: `span ${AbilityLogic.getPanelWidth(a.ability)}` }
                  }
                  onSelect={() => showAbility(a.ability)}
                >
                  <AbilityPanel
                    ability={a.ability}
                    hero={props.hero}
          ...
          {nonStandard.length > 0 && standard.length > 0 ? <Divider /> : null}
          <div
            className={`abilities-grid ${useRows ? 'compact' : ''} ${props.options.abilityWidth.toLowerCase().replace(' ', '-')}`}
          >
            {standard.map(a =>
              useRows ? (
                ...
                <SelectablePanel
                  key={a.ability.id}
                  style={{ gridColumn: `span ${AbilityLogic.getPanelWidth(a.ability)}` }}
                  onSelect={() => showAbility(a.ability)}
                >
                  <AbilityPanel
                    ability={a.ability}
      ...
      <ErrorBoundary>
        <div className="retinue-section">
          {monsters.length > 0 ? (
            <>
              <div
                className={`retinue-grid ${useRows ? 'compact' : ''} ${props.options.abilityWidth.toLowerCase().replace(' ', '-')}`}
              >
                {Collections.sort(monsters, m => m.monster.name).map(m =>
                  useRows ? (
                    <div
                      key={m.monster.id}
                      className="selectable-row clickable"
                      onClick={() => onSelectMonster(m.monster, m.summon)}
                    >
                      <div>
                        Companion: <b>{m.monster.name}</b>
                      </div>
                    </div>
                  ) : (
                    <SelectablePanel
                      key={m.monster.id}
                      onSelect={() => onSelectMonster(m.monster, m.summon)}
                    >
                      <MonsterPanel
                        monster={m.monster}
                        summon={m.summon}
  ...
  const getTabs = () => {
    ...
    if (props.options.compactView) {
      ...
      const others = abilities.filter(
        a =>
          a.ability.type.usage === AbilityUsage.Other ||
          a.ability.type.usage === AbilityUsage.NoAction,
      );
    ...
    const retinue =
      HeroLogic.getCompanions(props.hero).length +
    ...
    return (
      <ErrorBoundary>
        <div className="hero-panel compact">
          ...
          {props.hero.ancestry ? <Field label="Ancestry" value={props.hero.ancestry.name} /> : null}
          {background.length > 0 ? (
            <Field label="Background" value={background.join(' / ')} />
          ) : null}
          {props.hero.class ? (
            <Field
              label="Class"
      ...
      <div className="hero-panel" id={SheetFormatter.getPageId('hero', props.hero.id)}>
        <div className="hero-main-section">
          {!isSmall && !props.options.singlePage ? (
            <StatsSidebarPanel hero={props.hero} showStats={tab !== 'Hero'} />
          ) : null}
          <div className="hero-center-column">
            {props.options.singlePage ? null : (
              <div className="center-top">
                <Flex align="center" justify="space-between" gap={10}>
                  {isSmall ? (
                    <Select
                      style={{ flex: '1 1 0' }}
                      options={getTabs().map(tab => ({
                        value: tab,
                        label: tab,
                      }))}
