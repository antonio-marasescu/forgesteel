    ...
    case 'monster':
      content = (
        <MonsterSheet
          ...
          monsterGroup={
            SourcebookLogic.getMonsterGroup(props.sourcebooks, props.element.id) as MonsterGroup
          }
          sourcebooks={props.sourcebooks}
          options={props.options}
        />
  ...
  return (
    <>
      ...
      {props.ancestry.culture ? (
        <CultureSheet
          culture={props.ancestry.culture}
          sourcebooks={props.sourcebooks}
          options={props.options}
        />
      ) : null}
  ...
  return (
    <>
      ...
      <Field
        ...
        value={
          props.heroClass.primaryCharacteristics.join(', ') ||
          props.heroClass.primaryCharacteristicsOptions
            .map(array => array.join(', ') || 'None')
            .join(' or ') ||
          'None'
        }
      />
...
const CultureSheet = (props: CultureProps) => {
  return (
    <>
      ...
      <FeaturePanel
        feature={props.culture.language}
        sourcebooks={props.sourcebooks}
        options={props.options}
        mode={PanelMode.Full}
      />
      {props.culture.environment ? (
        <FeaturePanel
          feature={props.culture.environment}
          sourcebooks={props.sourcebooks}
          options={props.options}
          mode={PanelMode.Full}
        />
      ) : null}
      {props.culture.organization ? (
        <FeaturePanel
          feature={props.culture.organization}
          sourcebooks={props.sourcebooks}
          options={props.options}
          mode={PanelMode.Full}
        />
      ) : null}
      {props.culture.upbringing ? (
        <FeaturePanel
          feature={props.culture.upbringing}
          sourcebooks={props.sourcebooks}
          options={props.options}
          mode={PanelMode.Full}
        />
...
const ImbuementSheet = (props: ImbuementProps) => {
  return (
    <>
      <HeaderText level={1}>{props.imbuement.name || 'Unnamed Imbuement'}</HeaderText>
      ...
      {props.item.featuresByLevel
        ...
        .map(lvl => (
          <>
            ...
            {lvl.features.map(f => (
              <FeaturePanel key={f.id} feature={f} options={props.options} mode={PanelMode.Full} />
            ))}
          </>
        ))}
    </>
...
const KitSheet = (props: KitProps) => {
  return (
    <>
      <HeaderText level={1}>{props.kit.name || 'Unnamed Kit'}</HeaderText>
      <Markdown text={props.kit.description} />
      {props.kit.armor.length > 0 ? (
        <Field label="Armor" value={props.kit.armor.join(', ')} />
      ) : null}
      {props.kit.weapon.length > 0 ? (
        <Field label="Weapon" value={props.kit.weapon.join(', ')} />
      ) : null}
      ...
      {props.kit.stability > 0 ? (
        <Field label="Stability" value={`+${props.kit.stability}`} />
      ) : null}
      {props.kit.meleeDamage ? (
        <Field
          label="Melee Damage"
          value={`+${props.kit.meleeDamage.tier1} / +${props.kit.meleeDamage.tier2} / +${props.kit.meleeDamage.tier3}`}
        />
      ) : null}
      {props.kit.rangedDamage ? (
        <Field
          label="Ranged Damage"
          value={`+${props.kit.rangedDamage.tier1} / +${props.kit.rangedDamage.tier2} / +${props.kit.rangedDamage.tier3}`}
        />
      ) : null}
      {props.kit.meleeDistance > 0 ? (
        <Field label="Melee Distance" value={`+${props.kit.meleeDistance}`} />
      ) : null}
      {props.kit.rangedDistance > 0 ? (
        <Field label="Ranged Distance" value={`+${props.kit.rangedDistance}`} />
      ) : null}
      {props.kit.disengage > 0 ? (
        <Field label="Disengage" value={`+${props.kit.disengage}`} />
      ) : null}
      ...
      {props.monsterGroup.malice.length > 0 ? (
        <>
          ...
          <div className="ds-text">
            At the start of any {props.monsterGroup.name}'s turn, you can spend malice to activate
...
const MonsterSheet = (props: MonsterProps) => {
  ...
  const features = MonsterLogic.getFeatures(props.monster).filter(
    f => f.type === FeatureType.Text || f.type === FeatureType.AddOn,
  );
  const abilities = MonsterLogic.getFeatures(props.monster)
    .filter(f => f.type === FeatureType.Ability)
    .map(f => f.data.ability);

  let rightOfTags = null;
  if (props.monster.role.organization === MonsterOrganizationType.Minion) {
    rightOfTags = <Field label="EV" value={`${props.monster.encounterValue} for 4 minions`} />;
  } else if (props.monster.encounterValue > 0) {
    rightOfTags = <Field label="EV" value={props.monster.encounterValue} />;
  }
    ...
    <>
      ...
      <StatsRow>
        <Field
          ...
          label="Size"
          value={FormatLogic.getSize(props.monster.size)}
        />
        <Field orientation="vertical" label="Speed" value={speedStr} />
        <Field
          orientation="vertical"
          label="Stamina"
          value={MonsterLogic.getStaminaDescription(props.monster)}
        />
        <Field
          orientation="vertical"
          label="Stability"
          value={MonsterLogic.getStability(props.monster)}
        />
        <Field
          orientation="vertical"
          label="Free Strike"
          value={MonsterLogic.getFreeStrikeDamage(props.monster)}
        />
      </StatsRow>
      <StatsRow>
        {[
          ...
          <Field
            ...
            orientation="vertical"
            label={ch}
            value={MonsterLogic.getCharacteristic(props.monster, ch)}
          />
        ))}
      </StatsRow>
      ...
      {conditions.length > 0 ? <Field label="Cannot Be" value={conditions.join(', ')} /> : null}
      ...
      {features.map(f => (
        <Field key={f.id} label={f.name} value={<Markdown text={f.description} useSpan={true} />} />
      ))}
      {abilities.length > 0
        ? abilities.map(a => (
        ...
        <>
          {props.monster.retainer.level4 && props.monster.retainer.level < 4 ? (
            <>
              ...
              <FeaturePanel
                key={props.monster.retainer.level4.id}
                feature={props.monster.retainer.level4}
                options={props.options}
                mode={PanelMode.Full}
              />
            </>
          ...
          {props.monster.retainer.level7 && props.monster.retainer.level < 7 ? (
            <>
              ...
              <FeaturePanel
                key={props.monster.retainer.level7.id}
                feature={props.monster.retainer.level7}
                options={props.options}
                mode={PanelMode.Full}
              />
            </>
          ...
          {props.monster.retainer.level10 && props.monster.retainer.level < 10 ? (
            <>
              ...
              <FeaturePanel
                key={props.monster.retainer.level10.id}
                feature={props.monster.retainer.level10}
                options={props.options}
                mode={PanelMode.Full}
              />
            </>
...
const ProjectSheet = (props: ProjectProps) => {
  return (
    <>
      ...
      {props.project.itemPrerequisites ? (
      ...
      <Field
        ...
        value={
          props.project.characteristic.length === 5
            ? 'highest characteristic'
            : props.project.characteristic.join(' or ')
        }
      />
            ...
            case FeatureType.Text:
              return (
                <Field
                  key={content.id}
                  label={content.name}
                  value={<Markdown text={content.description} useSpan={true} />}
                />
              );
    ...
    <>
      ...
      <Flex align="center" justify="space-between">
        ...
        <Field
          ...
          value={
            props.terrain.area
              ? `${props.terrain.encounterValue} per ${props.terrain.area}`
              : props.terrain.encounterValue === 0
                ? '-'
                : props.terrain.encounterValue
          }
        />
      </Flex>
      <div>
        <Field
          label="Size"
          value={
            typeof props.terrain.size === 'string'
              ? props.terrain.size
              : FormatLogic.getSize(props.terrain.size)
          }
        />
        ...
        {props.terrain.link ? <Field label="Link" value={props.terrain.link} /> : null}
        {immunities.length > 0 ? (
          <Field
            label="Immunities"
            value={immunities.map(mod => `${mod.damageType} ${mod.value}`).join(', ')}
