...
export const PowerRollPanel = (props: Props) => {
  const [distance, setDistance] = useState<AbilityDistanceType | undefined>(
    props.ability && props.ability.distance.length > 1 ? props.ability.distance[0].type : undefined,
  );
  ...
  const getHeader = () => {
    ...
    if (
      (CreatureLogic.isHero(props.creature) ||
        (CreatureLogic.isMonster(props.creature) && props.creature.retainer)) &&
      props.autoCalc
    ) {
      let bonus = 0;
      if (props.ability) {
        ...
        const values = props.powerRoll.characteristic.map(ch =>
          CreatureLogic.getCharacteristic(props.creature, ch),
        );
  ...
  const getFooter = () => {
    ...
    if (props.creature && props.ability) {
      ...
      if (CreatureLogic.isHero(props.creature)) {
        let isMelee =
          props.ability.keywords.includes(AbilityKeyword.Melee) &&
          props.ability.keywords.includes(AbilityKeyword.Weapon);
        let isRanged =
          props.ability.keywords.includes(AbilityKeyword.Ranged) &&
          props.ability.keywords.includes(AbilityKeyword.Weapon);
        ...
        const dmgKits = HeroLogic.getKitDamageBonuses(props.creature).filter(dmg => {
          switch (dmg.type) {
            case 'melee':
              return isMelee;
            case 'ranged':
      ...
      if (!props.autoCalc && CreatureLogic.isHero(props.creature)) {
        HeroLogic.getFeatureDamageBonuses(props.creature, props.ability, distance).forEach(
          (bonus, n) => {
  ...
  const getTier = (tier: number, value: string) => {
    if (props.autoCalc && props.ability) {
      return AbilityLogic.getTierEffectCreature(
        value,
        tier,
        props.ability,
        distance,
  ...
  return (
    <ErrorBoundary>
      <div className="power-roll-panel">
          ...
          <Space onClick={e => e.stopPropagation()}>
            ...
            {props.odds ? (
              <Button
                ...
                title="Odds"
                icon={
                  <BarChartOutlined style={showOdds ? { color: 'rgb(22, 119, 255)' } : undefined} />
                }
        ...
        <div
          className={props.highlightTier === 1 ? 'power-roll-row highlight-row' : 'power-roll-row'}
        >
          <div className="tier">≤ 11</div>
          <div className="effect">
            <Markdown text={getTier(1, props.powerRoll.tier1)} />
          </div>
          {showOdds && props.odds ? (
            <div className="odds">{props.odds.filter(n => n === 1).length}%</div>
          ) : null}
        </div>
        <div
          className={props.highlightTier === 2 ? 'power-roll-row highlight-row' : 'power-roll-row'}
        >
          <div className="tier">12 - 16</div>
          <div className="effect">
            <Markdown text={getTier(2, props.powerRoll.tier2)} />
          </div>
          {showOdds && props.odds ? (
            <div className="odds">{props.odds.filter(n => n === 2).length}%</div>
          ) : null}
        </div>
        <div
          className={props.highlightTier === 3 ? 'power-roll-row highlight-row' : 'power-roll-row'}
        >
          <div className="tier">≥ 17</div>
          <div className="effect">
            <Markdown text={getTier(3, props.powerRoll.tier3)} />
          </div>
          {showOdds && props.odds ? (
            <div className="odds">{props.odds.filter(n => n === 3).length}%</div>
          ) : null}
        </div>
