...
export const PlotGraphPanel = (props: Props) => {
  ...
  const plotPositions: {
    [plotID: string]: { x: number; y: number; width: number; height: number };
  } = {};
    ...
    plots.push(
      ...
      >
        <div
          ...
          onClick={e => {
            e.stopPropagation();
            selectPlotPoint(plot);
          }}
          onDoubleClick={e => e.stopPropagation()}
        >
          {plot.plots.length > 0 ? (
            <PartitionOutlined
              className="subplot-indicator"
              rotate={90}
              style={{ fontSize: position.height / 2 }}
            />
          ) : null}
    ...
    plot.links
      ...
      .forEach(link => {
        ...
        if (link.label) {
          ...
          const maxX = Math.max(
            fromPosition.x + fromPosition.width,
            toPosition.x + toPosition.width,
          );
  ...
  const canGoIn =
    props.onOpen && props.selectedPlot && (props.selectedPlot.plots.length > 0 || props.onCreate);

  return (
    <ErrorBoundary>
      <div
        className="plot-graph-panel"
        onClick={() => selectPlotPoint(null)}
        onDoubleClick={props.onCreate}
      >
            ...
            extra={
              <Flex gap={5}>
                {canGoOut ? (
                  <Button
                    type="text"
                    icon={<LogoutOutlined rotate={210} />}
                    onClick={() => openPlotPoint(parentPlot)}
                  />
                ) : null}
                {canGoIn ? (
                  <Button
                    type="text"
                    icon={<LoginOutlined rotate={30} />}
                    onClick={() => openPlotPoint(props.selectedPlot!)}
                  />
                ) : null}
              </Flex>
            }
