...
export const PrimaryReferenceCard = (props: Props) => {
  const character = useMemo(() => props.character, [props.character]);

  const showTriggerHelp =
    props.options.classicSheetPageSize === SheetPageSize.A4 &&
  ...
  return (
    <div className="primary-reference card">
      ...
      <div className="hero-tokens">
        ...
        <p>
          <strong>1 Token:</strong> Gain 2 Surges.
        </p>
        <p>
          <strong>1 Token:</strong> Succeed on a saving throw instead of failing.
        </p>
        <p>
          <strong>1 Token:</strong> Reroll a test and use the new result.
        </p>
        <p>
          <strong>2 Tokens:</strong> On your turn or when you take damage, regain Stamina equal to
          your recovery value without spending a Recovery before taking the damage.
        </p>
      </div>
      <div className="turn-reference">
        <h3>Your Turn</h3>
        <p>
          Each creature can take a move action, a maneuver, and a main action on their turn — in any
          order
        </p>
        {showTriggerHelp || !showActionsManeuversReference ? (
          <p>
            You can also take <strong>one triggered action per round</strong> when the trigger
            happens. There is no limit to the number of free triggered actions you can take.
          </p>
        ) : null}
