...
export const AbilityCard = (props: Props) => {
  ...
  const getAbilityCost = () => {
    if (ability.cost > 0) {
      return <span className="cost">{ability.cost}</span>;
    }
  };
      ...
      <div className="stats">
        ...
        <div className="distance-target">
          <div className="distance">
            {ability.distance?.length ? <img src={distanceIcon} alt="Distance" /> : undefined}
            <span>{ability.distance}</span>
          </div>
          <div className="target">
            {ability.target?.length ? <img src={targetIcon} alt="Target" /> : undefined}
    ...
    if (ability.hasPowerRoll) {
      return (
        <div className="power-roll">
          <div className="power">
            Power Roll +{' '}
            <DrawSteelSymbolText content={ability.rollPower} lookFor="characteristics" />
          </div>
          ...
          {ability.rollBonuses ? (
            <div className="roll-bonuses">
              {ability.rollBonuses.map(bonus => (
                <p key={bonus.name}>
                  <strong>{bonus.name}</strong>: {bonus.tier1} | {bonus.tier2} | {bonus.tier3}{' '}
                  {bonus.type} damage
                </p>
              ))}
            </div>
  ...
  const getEffectSection = () => {
    if (ability.effect) {
      return (
        <div className="effect">
          {!(ability.effect.startsWith('##') || ability.isNotTrueAbility) ? <h4>Effect:</h4> : null}
          <Markdown text={ability.effect} className="ability-effect" />
        </div>
      );
    }
  };
    ...
    <div className={getCardClasses(ability)}>
      <section className="bordered">
        ...
        {ability.qualifiers?.map((q, i) => {
          return (
            <div className="action-qualifier" key={`qualifier-${i}`}>
              {q}
            </div>
          );
        })}
