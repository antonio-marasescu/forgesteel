...
export const CompanionCard = (props: Props) => {
    ...
    if (companion.stamina || companion.recoveries) {
      return (
        <div className="stamina-recoveries">
          {companion.stamina ? (
            <StaminaComponent stamina={companion.stamina} options={props.options} />
          ) : null}
          {companion.recoveries ? (
            <RecoveriesComponent recoveries={companion.recoveries} options={props.options} />
          ) : null}
        </div>
      );
    }
          ...
          <div className="advancement" key={`${companion.id}-advancement-${a.level}`}>
            {a.ability ? (
              <>
                ...
                <AbilityComponent ability={a.ability} />
              </>
            ) : null}
            {a.features ? (
              <>
                <h4>
                  Level {a.level} {companion.name} Advancement Feature
                </h4>
      ...
      <>
        {companion.features?.map(f => (
          <div className="wrapper" key={f.id}>
            {getFeatureIcon()}
            <FeatureComponent feature={SheetFormatter.enhanceFeature(f)} />
          </div>
        ))}
      </>
    );
  ...
  const type =
    companion.type !== companion.classification
      ? `${companion.type} ${companion.classification}`
      : companion.type;
  return (
