      ...
      <table>
        <thead>
          <tr>
            ...
            <th>
              ...
              <div className="right">
                Total EV:<span className="ev-value">{encounter.encounterEv}</span>
              </div>
            </th>
        ...
        <tbody>
          {encounter.groups?.map((group, i) => {
            ...
            const minionSlots = groupMonsters.filter(
              m => m.role.organization === MonsterOrganizationType.Minion,
            ).length;
            const nonMinionSlots = groupMonsters.filter(
              m => m.role.organization !== MonsterOrganizationType.Minion,
            ).length;
            return (
              <tr className="creature-group" key={`group-${i}`}>
                <td className="encounter-group">
                  ...
                  {group.name ? <div className="group-name">{group.name}</div> : null}
                ...
                <td className="group-creatures">
                  <div className="wrapper">
                    {group.slots.map((slot, i) => {
                      ...
                      let extra = null;
                      if (slot.isMinion) {
                        if (slot.count > 1) extra = slot.count.toString();
                      } else if (minionSlots === 1 && nonMinionSlots === 1) {
                        extra = 'Captain';
                      }
                      return (
                        <div
                          key={`monsters-${slot.id}-${i}`}
                          className={`encounter-slot ${monster.role.type.toLocaleLowerCase().split(' ').join('-')}`}
                        >
                          {monster.name}
                          {extra ? ` (${extra})` : null}
                        </div>
                    ...
                    <div className="ev">
                      EV:<span className="ev-value">{slotEv}</span>
                    </div>
                  </div>
                </td>
                ...
                <td className="notes">{group.name ? group.name : null}</td>
              </tr>
            );
          })}
        </tbody>
        <tbody>
          {encounter.terrain?.length ? (
            <tr className="terrain-header">
              <th className="terrain" colSpan={2}>
                Dynamic Terrain Objects
              </th>
              <th className="stamina-tracker-header" colSpan={2}>
                Stamina Tracker
              </th>
              <th className="notes">Notes/Temporary Effects</th>
            </tr>
              ...
              <tr className="terrain-object" key={`roster-terrain-${terrain.id}`}>
                <td colSpan={2}>
                  <div className="wrapper">
                    ...
                    <div className="ev">
                      EV:<span className="ev-value">{terrain.encounterValue}</span>
                    </div>
                  </div>
                </td>
