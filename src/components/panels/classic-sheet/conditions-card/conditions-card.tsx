...
export const ConditionsCard = (props: Props) => {
  ...
  const otherConditions: Condition[] =
    character.conditions?.filter(c => !conditionTypes.includes(c.type)) || [];
    ...
    <div className="conditions card">
      ...
      <div className="conditions-table">
          ...
          return (
            <div className={classes.join(' ')} key={ct}>
              ...
              <LabeledBooleanField
                value={
                  showState &&
                  character.conditions?.find(c => c.type === ct)?.ends ===
              ...
              <LabeledBooleanField
                value={
                  showState &&
                  character.conditions?.find(c => c.type === ct)?.ends === ConditionEndType.SaveEnds
                }
                label=""
                additionalClasses={['center', 'no-label']}
              />
        ...
        <div className="condition">
          <div></div>
          <LabeledBooleanField value={false} label="" additionalClasses={['center', 'no-label']} />
          <LabeledBooleanField value={false} label="" additionalClasses={['center', 'no-label']} />
        </div>
      </div>
      <div className="save-ends-description">
        * Save Ends = <span className="save-target">{character.saveTarget}</span> or higher on 1d10
        {getSaveBonus()} at the end of your turn removes the effect.
      </div>
    </div>
  );
};
