...
const ChoiceFeatureComponent = (
  feature: FeatureChoice | FeatureLanguageChoice | FeatureItemChoice,
  hero?: Hero,
) => {
  let selectedOptions;
  if (feature.data.selected.length > 0) {
    selectedOptions = feature.data.selected
      .map(s => (typeof s === 'string' ? s : s.name))
      .map(s => {
        return (
          <div className="feature-iteration" key={s}>
            {s}
          </div>
        );
      });
    ...
    selectedOptions = [
      <div className="feature-iteration no-selection" key="unselected">
        Unselected
      </div>,
    ];
  }
...
const BonusFeatureComponent = (feature: FeatureBonus, hero?: Hero) => {
  const value = hero
    ? ModifierLogic.calculateModifierValue(feature.data, hero)
    : feature.data.value;
  return (
    <>
      <div className="feature-line">
        <strong>• {feature.name}: </strong>
        {SheetFormatter.addSign(value)} {feature.data.field}
      </div>
    </>
  );
};
...
const CharacteristicBonusFeatureComponent = (feature: FeatureCharacteristicBonus) => {
  return (
    <>
      <div className="feature-line">
        <strong>• Characteristic Bonus: </strong>
        {SheetFormatter.addSign(feature.data.value)} {feature.data.characteristic}
      </div>
    </>
  );
};
  ...
  return (
    <>
      ...
      <div className="feature-description">
        <em>{feature.description.replace(/^\s+/, '')}</em>
      </div>
    </>
  );
};
...
const ClassAbilityFeatureComponent = (feature: FeatureClassAbility) => {
  ...
  let ability = 'Ability';
  if (feature.data.count > 1) ability = 'Abilities';
...
const AbilityModifierComponent = (
  feature: FeatureAbilityDamage | FeatureAbilityDistance,
  hero?: Hero,
) => {
  const value = hero
    ? ModifierLogic.calculateModifierValue(feature.data, hero)
  ...
  return (
    <div className="feature-line">
      • [{feature.data.keywords.sort(SheetFormatter.sortKeywords).join(' ')}] Abilities:{' '}
      {SheetFormatter.addSign(value)} {type}
    </div>
  );
};
...
const DamageModifierComponent = (feature: FeatureDamageModifier, hero?: Hero) => {
  ...
  const modifiers = Array.from(modifiersMap.keys()).map(type => {
    const typeMods = modifiersMap.get(type)?.map(m => {
      return (
        <div
          className={`feature-iteration damage-type ${m.damageType.toLocaleLowerCase()}`}
          key={`${m.type}-${m.damageType}`}
        >
...
const DomainFeatureComponent = (feature: FeatureDomain | FeatureDomainFeature) => {
  ...
  if (feature.data.selected.length) {
    selectedOptions = feature.data.selected.map(s => {
      return (
        <div className="feature-iteration" key={s.id}>
          {s.name}
        </div>
      );
    });
  ...
  return (
    <>
      <div className="feature-line">
        <strong>• {SheetFormatter.pluralize(feature.name, feature.data.count)}:</strong>
      </div>
      {selectedOptions}
    </>
  );
...
const ConditionImmunityFeatureComponent = (feature: FeatureConditionImmunity) => {
  const immunities = feature.data.conditions.map(c => {
    return (
      <div className="feature-iteration" key={`immunity-${c.toString}`}>
        {c.toString()}
      </div>
    );
  });
  return (
    <>
      <div className="feature-line">
        <strong>• {feature.name}</strong>: Immune to{' '}
      </div>
      {immunities}
    </>
  );
...
const FollowerFeatureComponent = (feature: FeatureFollower) => {
  ...
  return (
    <div className="feature-line">
      ...
      {follower.name}
      {follower.type ? <em> ({follower.type})</em> : undefined}
    </div>
  );
};
...
const CompanionFeatureComponent = (feature: FeatureCompanion) => {
  ...
  if (companion?.retainer) {
    details = (
      <em>{` (Level ${companion.retainer.level} ${companion.role.type} ${companion.role.organization})`}</em>
    );
  }
...
const MaliceFeatureComponent = (feature: FeatureMalice) => {
  ...
  const sections = (feature.data.sections ?? []).map((section, n) =>
    ...
    <div className="feature feature-malice ability" key={feature.id}>
      ...
      <div className="details">
        <div className="header">
          ...
          <div className="ability-type">{feature.data.cost} Malice</div>
        </div>
        {sections}
      </div>
    </div>
...
const MaliceAbilityFeatureComponent = (feature: FeatureMaliceAbility) => {
  ...
  return (
    <div className="feature feature-malice" key={feature.id}>
      ...
      <div className="details">
        <AbilityComponent ability={abilitySheet} />
      </div>
    </div>
  );
};
