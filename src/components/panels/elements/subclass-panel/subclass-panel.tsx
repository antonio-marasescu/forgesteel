...
export const SubclassPanel = (props: Props) => {
  ...
  const getOverview = () => {
    return <Markdown text={props.subclass.description} />;
  };
  ...
  if (props.sourcebooks.length > 0) {
    const sourcebookType =
      SourcebookLogic.getSubclassSourcebook(props.sourcebooks, props.subclass)?.type ||
  ...
  return (
    <ErrorBoundary>
      <div
        className="subclass-panel"
        id={SheetFormatter.getPageId('subclass', props.subclass.id)}
        style={props.style}
      >
        <HeaderText level={1} tags={tags}>
          {props.subclass.name || 'Unnamed Subclass'}
