...
export const EncounterPanel = (props: Props) => {
  ...
  const getOverview = () => {
    return (
      <>
        ...
        />
        {props.encounter.notes.map(note => (
          <Field
            key={note.id}
            label={note.name}
            value={<Markdown text={note.description} useSpan={true} />}
          />
        ))}
      </>
    );
  ...
  const getEncounterGroups = () => {
    if (props.encounter.groups.length === 0 && props.encounter.terrain.length === 0) {
      return <Empty text="No monsters or terrain" />;
    }
          ...
          .map((group, n) => (
            <SelectablePanel key={group.id} style={{ paddingTop: '0' }}>
              ...
              <Space orientation="vertical">
                ...
                {group.slots.length === 0 ? <Empty /> : null}
              </Space>
            </SelectablePanel>
          ))}
            ...
            return (
              <div key={group.id}>
                ...
                <div className="encounter-malice">
                  {group.malice
                    .filter(m => m.data.echelon <= maxEchelon)
                    .map(m => (
                      <SelectablePanel key={m.id}>
                        <FeaturePanel
                          ...
                          mode={PanelMode.Full}
                          cost={
                            m.type === FeatureType.MaliceAbility ? m.data.ability.cost : m.data.cost
                          }
  ...
  if (props.sourcebooks.length > 0) {
    const sourcebookType =
      SourcebookLogic.getEncounterSourcebook(props.sourcebooks, props.encounter)?.type ||
  ...
  return (
    <ErrorBoundary>
      <div
        ...
        id={SheetFormatter.getPageId('encounter', props.encounter.id)}
      >
        <HeaderText
          level={1}
          tags={tags}
