...
export const ProjectPanel = (props: Props) => {
  ...
  const getPreparation = () => {
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        ...
        {project.itemPrerequisites && project.progress ? (
          <Toggle
            label="Obtained Items"
            value={project.progress.prerequisites}
            onChange={setPrerequisites}
          />
        ) : null}
        {project.source ? <Field label="Source" value={props.project.source} /> : null}
        {project.source && project.progress ? (
          <Toggle label="Obtained Source" value={project.progress.source} onChange={setSource} />
        ) : null}
      </Space>
    );
  };
  ...
  const getProgress = () => {
    ...
    const getFollower = (follower: Follower, project: Project) => {
      ...
      return (
        <div>
          <HeaderText tags={[follower.type]}>{follower.name || 'Unnamed Follower'}</HeaderText>
          <Field
    ...
    const follower = HeroLogic.getFollowers(props.hero).find(
      f => f.id === project.progress!.followerID,
    );
      ...
      <Space orientation="vertical" style={{ width: '100%' }}>
        {HeroLogic.getFollowers(props.hero).length > 0 ? (
          <Select
            ...
            allowClear={true}
            options={HeroLogic.getFollowers(props.hero).map(f => ({
              value: f.id,
              label: f.name,
              follower: f,
            }))}
  ...
  const getItem = () => {
    ...
    if (
      project.goal === 0 ||
      !project.progress ||
      project.progress.points < project.goal ||
      !props.hero ||
      !props.addItemAndDeleteProject ||
      !getPrerequisitesMet()
    ) {
      return null;
    }

    let item = SourcebookLogic.getItems(props.sourcebooks).find(
      i => i.crafting && i.crafting.id === project.id,
    );
    if (!item) {
      const imbuement = SourcebookLogic.getImbuements(props.sourcebooks).find(
        i => i.crafting && i.crafting.id === project.id,
      );
      if (imbuement) {
  ...
  if (props.sourcebooks.length > 0) {
    ...
    if (!sourcebook) {
      ...
      sourcebook = props.sourcebooks.find(sb =>
        sb.items.some(i => i.crafting && i.crafting.id === project.id),
      );
    }
    if (!sourcebook) {
      // This might be a crafting project from an imbuement
      sourcebook = props.sourcebooks.find(sb =>
        sb.imbuements.some(i => i.crafting && i.crafting.id === project.id),
      );
    }
