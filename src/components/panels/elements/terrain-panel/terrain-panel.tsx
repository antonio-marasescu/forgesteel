...
export const TerrainPanel = (props: Props) => {
  ...
  if (props.sourcebooks.length > 0) {
    const sourcebookType =
      SourcebookLogic.getTerrainSourcebook(props.sourcebooks, terrain)?.type ||
  ...
  return (
    <ErrorBoundary>
      <div
        className={props.mode === PanelMode.Full ? 'terrain-panel' : 'terrain-panel compact'}
        id={
          props.mode === PanelMode.Full
            ? SheetFormatter.getPageId('terrain', terrain.id)
            : undefined
        }
        style={props.style}
      >
        <HeaderText level={1} tags={tags} extra={props.extra}>
        ...
        <Flex align="center" justify="space-between">
          ...
          <Field
            ...
            value={
              terrain.area
                ? `${terrain.encounterValue} per ${terrain.area}`
                : terrain.encounterValue === 0
                  ? '-'
                  : terrain.encounterValue
            }
          />
        </Flex>
        {props.mode === PanelMode.Full ? (
          <div>
            ...
            <Field
              label="Size"
              value={
                typeof terrain.size === 'string' ? terrain.size : FormatLogic.getSize(terrain.size)
              }
            />
            <Field
              label="Stamina"
              value={
                props.updateTerrain
                  ...
                  : TerrainLogic.getStaminaDescription(terrain)
              }
            />
            {terrain.direction ? <Field label="Direction" value={terrain.direction} /> : null}
            {terrain.link ? <Field label="Link" value={terrain.link} /> : null}
            {immunities.length > 0 ? (
              <Field
                label="Immunities"
            ...
            {props.upgradeIDs
              ...
              : null}
            {props.showCustomizations && terrain.upgrades.length > 0 ? (
            ...
            {props.showCustomizations
              ? terrain.upgrades.map(upgrade => (
                  <div key={upgrade.id}>
                    {upgrade.cost >= 0 ? (
                      <HeaderText ribbon={<Pill>+{upgrade.cost} EV</Pill>}>
                        {upgrade.label}
                      </HeaderText>
                    ) : (
                      <HeaderText ribbon={<Pill>{upgrade.cost} EV</Pill>}>
