...
export const ImbuementPanel = (props: Props) => {
  ...
  if (props.sourcebooks.length > 0) {
    const sourcebookType =
      SourcebookLogic.getImbuementSourcebook(props.sourcebooks, props.imbuement)?.type ||
  ...
  return (
    <ErrorBoundary>
      <div
        className={props.mode === PanelMode.Full ? 'imbuement-panel' : 'imbuement-panel compact'}
        id={
          props.mode === PanelMode.Full
            ? SheetFormatter.getPageId('imbuement', props.imbuement.id)
            : undefined
        }
      >
        ...
        {props.imbuement.crafting ? (
          <Expander title="Crafting">
            <ProjectPanel
              project={props.imbuement.crafting}
              sourcebooks={props.sourcebooks}
              mode={PanelMode.Full}
            />
          </Expander>
