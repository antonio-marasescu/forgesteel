...
export const FeaturePanel = (props: Props) => {
  ...
  const getTags = () => {
    ...
    if (list !== undefined) {
      if (props.sourcebooks && props.sourcebooks.length > 0) {
        const sourcebookType =
          SourcebookLogic.getPerkSourcebook(props.sourcebooks, props.feature as Perk)?.type ||
    ...
    if (
      props.feature.type === FeatureType.Malice ||
      props.feature.type === FeatureType.MaliceAbility
    ) {
  ...
  if (
    props.feature.type === FeatureType.Ability ||
    props.feature.type === FeatureType.MaliceAbility
  ) {
    return (
      <AbilityPanel
  ...
  if (props.feature.type === FeatureType.AncestryFeatureChoice) {
    if (props.feature.data.selected) {
      return (
        <FeaturePanel
          feature={props.feature.data.selected}
  ...
  if (props.feature.type === FeatureType.Follower) {
    return <FollowerPanel follower={props.feature.data.follower} mode={PanelMode.Full} />;
  }

  let customization: AbilityCustomization | null = null;
  if (props.hero) {
    customization =
      props.hero.abilityCustomizations.find(ac => ac.abilityID === props.feature.id) || null;
  }

  return (
    <ErrorBoundary>
      <div
        className={props.mode === PanelMode.Full ? 'feature-panel' : 'feature-panel compact'}
        id={
          props.mode === PanelMode.Full
            ? SheetFormatter.getPageId('feaure', props.feature.id)
            : undefined
        }
        style={props.style}
      >
        ...
        {customization && customization.notes ? (
          <Field label="Notes" value={<Markdown text={customization.notes} useSpan={true} />} />
        ) : null}
      </div>
    </ErrorBoundary>
  );
