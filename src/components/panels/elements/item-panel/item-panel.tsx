...
export const ItemPanel = (props: Props) => {
  ...
  const getImbuements = () => {
    const setFeatureData = (featureID: string, data: FeatureData) => {
      const copy = Utils.copy(item);
      const feature = copy.imbuements.map(f => f.feature).find(f => f.id === featureID);
      if (feature) {
        feature.data = data;
      }
      if (props.onChange) {
    ...
    const getAvailableImbuements = () => {
      ...
      const options = SourcebookLogic.getImbuements(props.sourcebooks ?? []).filter(imbuement => {
      ...
      return (
        <Space orientation="vertical" style={{ width: '100%' }}>
          ...
          {options.length === 0 ? <Empty text="No imbuements are available." /> : null}
        </Space>
      );
    };
    ...
    return (
      <>
        ...
        {item.imbuements.length === 0 ? <Empty /> : null}
        <Drawer
          open={imbuementsOpen}
        ...
        >
          <Modal
            content={<div style={{ padding: '20px' }}>{getAvailableImbuements()}</div>}
            onClose={() => setImbuementsOpen(false)}
          />
        </Drawer>
      </>
  ...
  const getContent = () => {
    ...
    const imbueable =
      item.type === ItemType.ImbuedArmor ||
      item.type === ItemType.ImbuedImplement ||
      item.type === ItemType.ImbuedWeapon;
    if (imbueable) {
  ...
  if (props.sourcebooks.length > 0) {
    const sourcebookType =
      SourcebookLogic.getItemSourcebook(props.sourcebooks, item)?.type || SourcebookType.Official;
  ...
  return (
    <ErrorBoundary>
      <div
        ...
        id={SheetFormatter.getPageId('item', item.id)}
        style={props.style}
      >
        <HeaderText level={1} tags={tags}>
          {item.name || 'Unnamed Item'}
