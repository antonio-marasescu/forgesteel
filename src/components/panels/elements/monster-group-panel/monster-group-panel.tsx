...
export const MonsterGroupPanel = (props: Props) => {
    ...
    return (
      <>
        <div className="ds-text">
          At the start of any {props.monsterGroup.name}'s turn, you can spend malice to activate one
    ...
    return (
      <div className="monsters">
        {props.monsterGroup.monsters.map(m => (
          <SelectablePanel
            key={m.id}
            onSelect={props.onSelectMonster ? () => props.onSelectMonster!(m) : undefined}
          >
            <MonsterPanel
              monster={m}
  ...
  const getContent = () => {
    ...
    const options: { value: string; label: string }[] = [{ value: 'overview', label: 'Overview' }];
    if (props.monsterGroup.malice.length > 0) {
  ...
  if (props.sourcebooks.length > 0) {
    const sourcebookType =
      SourcebookLogic.getMonsterGroupSourcebook(props.sourcebooks, props.monsterGroup)?.type ||
  ...
  return (
    <ErrorBoundary>
      <div
        className="monster-group-panel"
        id={SheetFormatter.getPageId('monster-group', props.monsterGroup.id)}
      >
        <HeaderText level={1} tags={tags}>
          {props.monsterGroup.name || 'Unnamed Monster Group'}
        </HeaderText>
