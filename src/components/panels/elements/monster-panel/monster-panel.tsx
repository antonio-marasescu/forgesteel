...
export const MonsterPanel = (props: Props) => {
  ...
  if (props.sourcebooks.length > 0) {
    const sourcebookType =
      SourcebookLogic.getMonsterSourcebook(props.sourcebooks, props.monster)?.type ||
      ...
      case MonsterOrganizationType.Minion:
        rightOfTags = (
          <Field
            ...
            label="EV"
            value={`${props.monster.encounterValue} for 4 minions`}
          />
        );
        break;
      default:
  ...
  return (
    <ErrorBoundary>
      <div
        className={props.mode === PanelMode.Full ? 'monster-panel' : 'monster-panel compact'}
        id={
          props.mode === PanelMode.Full
            ? SheetFormatter.getPageId('monster', props.monster.id)
            : undefined
        }
        style={props.style}
      >
        <HeaderText
          level={1}
          ribbon={
            <MonsterToken monster={props.monster} monsterGroup={props.monsterGroup} size={28} />
          }
          tags={tags}
          extra={props.extra}
        >
