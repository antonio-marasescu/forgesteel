    ...
    switch (content.contentType) {
      case 'text':
        if (content.format === 'director') {
          return (
            <Alert type="info" showIcon={true} title={content.text} style={{ width: '100%' }} />
          );
        }
        return (
      ...
      case 'reference': {
        switch (content.type) {
          case 'encounter': {
            const encounter = SourcebookLogic.getEncounters(props.sourcebooks).find(
              e => e.id === content.contentID,
            );
            if (encounter) {
              return (
                <SelectablePanel
                  style={{ overflow: 'hidden' }}
                  onSelect={() => navigation.goToLibrary('encounter', encounter.id)}
                >
                  <EncounterPanel
                    encounter={encounter}
          ...
          case 'montage': {
            const montage = SourcebookLogic.getMontages(props.sourcebooks).find(
              m => m.id === content.contentID,
            );
            if (montage) {
              return (
                <SelectablePanel
                  style={{ overflow: 'hidden' }}
                  onSelect={() => navigation.goToLibrary('montage', montage.id)}
                >
                  <MontagePanel
                    montage={montage}
                    heroes={props.heroes}
          ...
          case 'negotiation': {
            const negotiation = SourcebookLogic.getNegotiations(props.sourcebooks).find(
              n => n.id === content.contentID,
            );
            if (negotiation) {
              return (
                <SelectablePanel
                  style={{ overflow: 'hidden' }}
                  onSelect={() => navigation.goToLibrary('negotiation', negotiation.id)}
                >
                  <NegotiationPanel
          ...
          case 'tactical-map': {
            const map = SourcebookLogic.getTacticalMaps(props.sourcebooks).find(
              tm => tm.id === content.contentID,
            );
            if (map) {
              return (
                <SelectablePanel
                  style={{ overflow: 'hidden' }}
                  onSelect={() => navigation.goToLibrary('tactical-map', map.id)}
                >
          ...
          default: {
            ...
            if (element) {
              return (
                <SelectablePanel
                  style={{ overflow: 'hidden' }}
                  onSelect={() => navigation.goToLibrary(content.type, element.id)}
                >
              ...
              return (
                <Flex vertical={true} gap={5}>
                  <Button
                    ...
                    onClick={() => setSelectedReference(content)}
                  />
                  <Button
                    title="Run"
                    icon={<PlayCircleOutlined />}
                    onClick={() => props.onStart!('encounter', encounter, '')}
                  />
                  <Button
                    title="Edit"
                    icon={<EditOutlined />}
                    onClick={() =>
                      navigation.goToLibraryEdit(
                        'encounter',
                        SourcebookLogic.getEncounterSourcebook(props.sourcebooks, encounter)!.id,
                        encounter.id,
                      )
                    }
                  />
              ...
              return (
                <Flex vertical={true} gap={5}>
                  ...
                  <Button
                    title="Run"
                    icon={<PlayCircleOutlined />}
                    onClick={() => props.onStart!('montage', montage, '')}
                  />
                  <Button
                    title="Edit"
                    icon={<EditOutlined />}
                    onClick={() =>
                      navigation.goToLibraryEdit(
                        'montage',
                        SourcebookLogic.getMontageSourcebook(props.sourcebooks, montage)!.id,
                        montage.id,
                      )
                    }
                  />
              ...
              return (
                <Flex vertical={true} gap={5}>
                  ...
                  <Button
                    title="Run"
                    icon={<PlayCircleOutlined />}
                    onClick={() => props.onStart!('negotiation', negotiation, '')}
                  />
                  <Button
                    title="Edit"
                    icon={<EditOutlined />}
                    onClick={() =>
                      navigation.goToLibraryEdit(
                        'negotiation',
                        SourcebookLogic.getNegotiationSourcebook(props.sourcebooks, negotiation)!
                          .id,
                        negotiation.id,
                      )
                    }
              ...
              return (
                <Flex vertical={true} gap={5}>
                  <Button
                    title="Run"
                    icon={<PlayCircleOutlined />}
                    onClick={() => props.onStart!('tactical-map', map, '')}
                  />
                  <Button
                    title="Edit"
                    icon={<EditOutlined />}
                    onClick={() =>
                      navigation.goToLibraryEdit(
                        'tactical-map',
                        SourcebookLogic.getTacticalMapSourcebook(props.sourcebooks, map)!.id,
                        map.id,
                      )
                    }
                  />
  ...
  return (
    <ErrorBoundary>
      <div className="plot-panel">
        ...
        {!props.plot.description && props.plot.content.length === 0 ? (
        ...
        <Space orientation="vertical" style={{ width: '100%' }}>
          {props.plot.links.map(l => (
            <Button
              ...
              block={true}
              onClick={() =>
                props.onSelect(props.adventure.plot.plots.find(p => p.id === l.plotID)!)
              }
            >
      ...
      >
        {selectedReference ? (
          <ElementModal
            ...
            element={
              SourcebookLogic.getElement(selectedReference.contentID, props.sourcebooks) as Element
            }
            sourcebooks={props.sourcebooks}
            options={props.options}
            onClose={() => setSelectedReference(null)}
