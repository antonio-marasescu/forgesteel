...
export const MapMiniPanel = (props: Props) => {
  ...
  const getContent = () => {
    if (props.mini.content && props.content) {
      ...
      if (props.mini.content.type === 'hero') {
        return <HeroToken hero={props.content as Hero} size={size} />;
      }

      if (props.mini.content.type === 'monster') {
        return <MonsterToken monster={props.content as Monster} size={size} />;
      }
    }
  ...
  const getInfo = () => {
    ...
    if (props.mini.content && props.content) {
      if (props.mini.content.type === 'hero') {
        content.push(<HeroInfo key="info" hero={props.content as Hero} />);
      ...
      if (props.mini.content.type === 'monster') {
        content.push(<MonsterInfo key="info" monster={props.content as Monster} />);

        content.push(
          <MonsterHealthPanel key="monster-health" monster={props.content as Monster} />,
        );
      }
    }

    if (props.mini.notes) {
      content.push(<Markdown key="notes" text={props.mini.notes} />);
    }
  ...
  return (
    <ErrorBoundary>
      <Popover content={getInfo()}>
        ...
        >
          <div className="mini-content">{getContent()}</div>
        </div>
      </Popover>
    </ErrorBoundary>
  );
