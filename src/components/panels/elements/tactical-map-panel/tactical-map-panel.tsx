...
export const TacticalMapPanel = (props: Props) => {
    ...
    if (mini.content) {
      if (props.encounters) {
        ...
        if (enc) {
          ...
          const monster = enc.groups
            .flatMap(g => g.slots)
            .flatMap(s => s.monsters)
            .find(m => m.id === mini.content!.id);
          if (monster) {
            return monster;
          }
        }
  ...
  const getMapItemStyle = (
    x: number,
    y: number,
    width: number,
    height: number,
    style: 'square' | 'rounded' | 'circle' | 'vertex' | 'wall' | null,
    dim: MapBoundaries,
  ): MapItemStyle => {
    let offsetX = 0;
    let offsetY = 0;
  ...
  const getTopToolbar = () => {
    ...
    return (
      <div className="tactical-map-toolbar top-toolbar">
        <Segmented name="edit" options={editModes} value={editMode} onChange={changeEditMode} />
        ...
        {editMode === TacticalMapEditMode.Fog ? <Button onClick={fillFog}>Fill Fog</Button> : null}
        {editMode === TacticalMapEditMode.Map ? (
          <DangerButton label="Clear Map" onConfirm={clearMap} />
        ) : null}
        {editMode === TacticalMapEditMode.Tiles ? (
          <DangerButton
            disabled={map.items.filter(i => i.type === 'tile').length === 0}
            label="Clear Tiles"
            onConfirm={clearTiles}
          />
        ) : null}
        {editMode === TacticalMapEditMode.Walls ? (
          <DangerButton
            disabled={map.items.filter(i => i.type === 'wall').length === 0}
            label="Clear Walls"
            onConfirm={clearWalls}
          />
        ) : null}
        {editMode === TacticalMapEditMode.Zones ? (
          <DangerButton
            disabled={map.items.filter(i => i.type === 'zone').length === 0}
            label="Clear Overlays"
            onConfirm={clearZones}
          />
        ) : null}
        {editMode === TacticalMapEditMode.Minis ? (
          <DangerButton
            disabled={map.items.filter(i => i.type === 'mini').length === 0}
            label="Clear Minis"
            onConfirm={clearMinis}
          />
        ) : null}
        {editMode === TacticalMapEditMode.Fog ? (
          <DangerButton
            disabled={map.items.filter(i => i.type === 'fog').length === 0}
            label="Clear Fog"
            onConfirm={clearFog}
          />
        ) : null}
  ...
  const getBottomToolbar = () => {
        ...
        if (mini.content && mini.content.type === 'monster') {
          ...
          if (props.encounters) {
            ...
            if (encounter) {
              ...
              if (slot) {
                monsterGroup =
                  SourcebookLogic.getMonsterGroup(props.sourcebooks || [], slot.monsterID) ||
                  undefined;
              }
            }
          }
              ...
              <Popover
                content={
                  <>
                    ...
                    />
                    <Toggle label="Openable" value={item.isOpenable} onChange={setIsOpenable} />
                    <Toggle
    ...
    if (editMode === TacticalMapEditMode.Minis && editAdding) {
      if (selectedMini) {
        return (
          <div className="tactical-map-toolbar bottom-toolbar">
            <div>Select a square to place this mini</div>
            <Divider orientation="vertical" />
      ...
      if (props.encounters) {
        props.encounters.forEach(enc =>
          sources.push({ value: enc.id, label: enc.name || 'Unnamed Encounter' }),
        );
      }
      ...
      const scatterMinis = () => {
        ...
        heroes.forEach(h =>
          tokens.push({
            ...
            encounterID: '',
            id: h.id,
            size: HeroLogic.getSize(h).value,
          }),
        );
        monsters.forEach(m =>
          tokens.push({ type: 'monster', encounterID: '', id: m.id, size: m.size.value }),
        );
        encounterHeroes.forEach(h =>
          tokens.push({
            type: 'hero',
            encounterID: h.encounter.id,
            id: h.hero.id,
            size: HeroLogic.getSize(h.hero).value,
          }),
        );
      ...
      const tokens = [
        ...
        ...encounterHeroes.map(h => (
          <HeroToken
            ...
            size={30}
            onClick={() =>
              setSelectedMini({ type: 'hero', encounterID: h.encounter.id, id: h.hero.id })
            }
          />
        )),
        ...encounterMonsters.map(m => (
          <MonsterToken
            ...
            size={30}
            onClick={() =>
              setSelectedMini({ type: 'monster', encounterID: m.encounter.id, id: m.monster.id })
            }
          />
        )),
      ];
    ...
    switch (editMode) {
      case TacticalMapEditMode.Tiles:
        message = editAdding
          ? 'Click on a square and drag to create a tile.'
          : 'Click on a tile to select it.';
        break;
      case TacticalMapEditMode.Walls:
        message = editAdding
          ? 'Click on a corner and drag to create a wall.'
          : 'Click on a wall to select it.';
        break;
      case TacticalMapEditMode.Zones:
        message = editAdding
          ? 'Click on a square and drag to create an overlay.'
          : 'Click on an overlay to select it.';
        break;
      case TacticalMapEditMode.Minis:
        message = editAdding
          ? 'Click on a square to create a mini token.'
          : 'Double-click on a hero or monster mini to edit it.';
        break;
    ...
    if (message) {
      return (
        <div className="tactical-map-toolbar bottom-toolbar">
          <div className="ds-text" style={{ padding: '0 10px' }}>
            {message}
          </div>
        </div>
      );
        ...
        return (
          <MapMiniPanel
            ...
            style={getMapItemStyle(
              mini.position.x,
              mini.position.y,
              mini.dimensions.width,
              mini.dimensions.height,
              'circle',
              boundaries,
            )}
  ...
  if (!boundaries) {
    if (props.display === TacticalMapDisplayType.DirectorEdit) {
      ...
      return <Empty text="Empty map" />;
    }
  }

  if (
    boundaries &&
    props.display === TacticalMapDisplayType.DirectorEdit &&
    editMode === TacticalMapEditMode.Tiles &&
    editAdding
  ) {
    // Apply a border
        ...
        {selectedMonster ? (
          <MonsterModal
            ...
            updateMonster={monster => {
              const mini = map.items
                .filter(item => item.type === 'mini')
                .find(mini => mini.id === selectedMapItemID);
              if (mini && mini.content) {
                const encounter = props.encounters
                  ? props.encounters.find(e => e.id === mini.content!.encounterID)
                  : undefined;
                if (encounter) {
