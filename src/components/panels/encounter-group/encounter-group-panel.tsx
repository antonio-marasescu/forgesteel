...
export const EncounterGroupHero = (props: EncounterGroupHeroProps) => {
  ...
  const showCharacteristics =
    size.width >= widthBase + widthStaminaColumn + widthCharacteristicsColumn;
  const showStats =
    size.width >= widthBase + widthStaminaColumn + widthCharacteristicsColumn + widthStatsColumn;
      ...
      <div className={className}>
        <div className="group-column">
          <Flex align="center" justify="space-between">
            <div className="group-name">Hero</div>
            <Popover
              trigger="click"
              content={
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <Segmented
                    ...
                    onChange={value =>
                      props.onSetState(props.hero, value as 'ready' | 'current' | 'finished')
                    }
                  />
        ...
        <div className="encounter-slots">
          <div className="encounter-slot" ref={setRef}>
            <div
              className={
                props.hero.state.defeated ? 'encounter-slot-row defeated' : 'encounter-slot-row'
              }
            ...
            >
              ...
              <div className="conditions-column">
                <Flex gap={3}>
                  {['healthy', 'injured'].includes(HeroLogic.getCombatState(props.hero)) ? null : (
                    <Tag variant="outlined">
                      {Format.capitalize(HeroLogic.getCombatState(props.hero))}
                    </Tag>
                  )}
                  {props.hero.state.hidden ? <Tag variant="outlined">Hidden</Tag> : null}
                  {props.hero.state.conditions.map(c => (
                    <Tag key={c.id} variant="outlined">
                      {ConditionLogic.getFullDescription(c)}
                    </Tag>
                  ))}
                </Flex>
              </div>
              ...
              <div key={slot.id} className="encounter-slot controlled-slot">
                <Flex align="center" style={{ paddingLeft: '5px' }}>
                  ...
                  <Button
                    type="text"
                    icon={<PlusOutlined />}
                    onClick={() => props.onAddMonsterToSquad(props.hero, slot.id)}
                  />
                  <DangerButton
                    mode="clear"
                    onConfirm={() => props.onRemoveSquad(props.hero, slot.id)}
                  />
                </Flex>
                <MonsterSlot
                  slot={slot}
...
export const EncounterGroupMonster = (props: EncounterGroupMonsterProps) => {
  const defeated = props.group.slots.every(
    s => s.state.defeated || s.monsters.every(m => m.state.defeated),
  );
  let className = 'encounter-group';
  if (defeated) {
    className += ' defeated';
  ...
  return (
    <ErrorBoundary>
      <div className={className}>
        <div className="group-column">
          <Flex align="center" justify="space-between">
            <div className="group-name">{props.group.name || `Group ${props.index + 1}`}</div>
            <Popover
              trigger="click"
              content={
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <Segmented
                    ...
                    onChange={value =>
                      props.onSetState(props.group, value as 'ready' | 'current' | 'finished')
                    }
                  />
                  <Button block={true} onClick={() => props.onDuplicate(props.group)}>
                    Duplicate
                  </Button>
...
export const MonsterSlot = (props: MonsterSlotProps) => {
  ...
  const isMinionSlot = props.slot.monsters.every(
    m => m.role.organization === MonsterOrganizationType.Minion,
  );
  ...
  const getMinionCountMessage = () => {
    ...
    const staminaRemaining =
      Collections.sum(props.slot.monsters, m => MonsterLogic.getStamina(m)) -
      props.slot.state.staminaDamage;
  ...
  const getMinionCaptainTag = () => {
    ...
    if (props.slot.state.captainID) {
      ...
      if (captain) {
        return <Tag variant="outlined">Captain: {captain.name}</Tag>;
      }
    }

    return <Tag variant="outlined">No captain</Tag>;
  };
      ...
      <div key={props.slot.id} className="encounter-slot" ref={setRef}>
        {isMinionSlot ? (
          <div
            key="minions"
            className={
              props.slot.state.defeated
                ? 'encounter-slot-row minion defeated'
                : 'encounter-slot-row minion'
            }
          ...
          >
            ...
            {showCharacteristics ? (
              <div className="characteristics-column">
                <div className="characteristics-column-item">
                  <div>M</div>
                  <div>
                    {MonsterLogic.getCharacteristic(props.slot.monsters[0], Characteristic.Might)}
                  </div>
                </div>
                <div className="characteristics-column-item">
                  <div>A</div>
                  <div>
                    {MonsterLogic.getCharacteristic(props.slot.monsters[0], Characteristic.Agility)}
                  </div>
                </div>
                <div className="characteristics-column-item">
                  <div>R</div>
                  <div>
                    {MonsterLogic.getCharacteristic(props.slot.monsters[0], Characteristic.Reason)}
                  </div>
                </div>
                <div className="characteristics-column-item">
                  <div>I</div>
                  <div>
                    {MonsterLogic.getCharacteristic(
                      props.slot.monsters[0],
            ...
            <div className="conditions-column">
              <Flex gap={3}>
                {getMinionCaptainTag()}
                {props.slot.state.conditions.map(c => (
                  <Tag key={c.id} variant="outlined">
                    {ConditionLogic.getFullDescription(c)}
                  </Tag>
                ))}
        ...
        {showMonsters
          ? props.slot.monsters.map(monster => (
              <div
                key={monster.id}
                className={
                  props.slot.state.defeated || monster.state.defeated
                    ? 'encounter-slot-row defeated'
                    : 'encounter-slot-row'
                }
              ...
              >
                ...
                <div className="conditions-column">
                  <Flex gap={3}>
                    {['healthy', 'injured'].includes(
                      MonsterLogic.getCombatState(monster),
                    ) ? null : (
                      <Tag variant="outlined">
                        {Format.capitalize(MonsterLogic.getCombatState(monster))}
                      </Tag>
                    )}
        ...
        <div className="group-column">
          <Flex align="center" justify="space-between">
            ...
            <Popover
              ...
              content={
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <Button block={true} onClick={() => props.onDuplicate(props.terrain)}>
                    Duplicate
                  </Button>
        ...
        <div className="encounter-slots">
          <div className="encounter-slot">
            <div className="encounter-slot-row" onClick={() => props.onSelect(props.terrain)}>
              ...
              <div className="conditions-column"></div>
            </div>
          </div>
        </div>
      </div>
