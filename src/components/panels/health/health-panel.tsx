...
const HealthPanel = (props: Props) => {
  const [page, setPage] = useState<string>(
    !props.stamina && props.recoveries ? 'recoveries' : 'stamina',
  );
      ...
      <Space orientation="vertical" style={{ flex: '1 1 0', width: '100%' }}>
        ...
        {page === 'stamina' ? (
          <Space orientation="vertical" style={{ width: '100%' }}>
            ...
            <Button block={true} disabled={damageValue === 0} onClick={takeDamage}>
              Take Damage
            </Button>
            <Button block={true} disabled={damageValue === 0} onClick={heal}>
              Regain Stamina
            </Button>
            {props.staminaTemp ? (
              <Button block={true} disabled={damageValue === 0} onClick={addTemp}>
                Add Temporary Stamina
              </Button>
            ) : null}
          </Space>
        ) : null}
        {page === 'recoveries' ? (
          <Space orientation="vertical" style={{ width: '100%' }}>
            <Button
              ...
              disabled={
                props.stamina!.staminaDamage === 0 ||
                props.recoveries!.recoveriesUsed >= props.recoveries!.recoveriesMax
              }
            ...
            >
              <div>
                <div>Spend a Recovery</div>
                <div className="subtext">Don't regain Stamina</div>
              </div>
            </Button>
            <Button
              block={true}
        ...
        <div className="health-panel compact">
          {props.stamina ? (
            <Field
              label="Stamina"
              value={
                props.stamina.staminaDamage
                  ? `${props.stamina!.staminaMax - props.stamina!.staminaDamage} / ${props.stamina!.staminaMax}`
                  : props.stamina!.staminaMax
              }
            />
          ) : null}
          {tags.length > 0 ? (
    ...
    <ErrorBoundary>
      <div className="health-panel">
        ...
        {props.stamina && !['healthy', 'injured', 'dying'].includes(props.stamina.state) ? (
          <Alert type="warning" showIcon={true} title={`You are ${props.stamina.state}.`} />
        ) : null}
...
Your allies can help you spend Recoveries in combat, and you can spend Recoveries out of combat as usual.`}
        ...
        {props.stamina && props.stamina.immunities.length > 0 ? (
          <Field
            label="Immunities"
            value={props.stamina.immunities.map(dm => `${dm.damageType} ${dm.value}`).join(', ')}
          />
        ) : null}
        {props.stamina && props.stamina.weaknesses.length > 0 ? (
          <Field
            label="Weakness"
            value={props.stamina.weaknesses.map(dm => `${dm.damageType} ${dm.value}`).join(', ')}
          />
        ) : null}
            ...
            <Flex align="center" justify="space-evenly" gap={10} style={{ margin: '10px 0' }}>
              ...
              {props.captain ? (
                <DropdownButton
                  ...
                  className="tall-button"
                  label="Captain"
                  items={props.captain.candidates.map(m => ({
                    key: m.id,
                    label: (
                      <div
                        style={{
                          ...
                          borderRadius: '5px',
                          background:
                            m.id === props.captain!.captainID ? 'rgb(64, 150, 255)' : undefined,
                          color:
                            m.id === props.captain!.captainID ? 'rgb(255, 255, 255)' : undefined,
                        }}
                      >
              ...
              <Popover
                ...
                content={
                  <Space orientation="vertical">
                    <Button block={true} type="text" onClick={() => addSpecial('Judged')}>
                      Judged
                    </Button>
                    <Button block={true} type="text" onClick={() => addSpecial('Marked')}>
                      Marked
                    </Button>
                    <Button block={true} type="text" onClick={() => addSpecial('Surprised')}>
                      Surprised
                    </Button>
                  </Space>
                }
        ...
        >
          <ConditionSelectModal
            immunities={props.conditions.immunities}
