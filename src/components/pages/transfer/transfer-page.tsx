...
export const TransferPage = (props: Props) => {
  ...
  const [mergeBehavior, setMergeBehavior] = useState<MergeDuplicateBehavior>(
    MergeDuplicateBehavior.Skip,
  );
  ...
  const mergeToWarehouse = () => {
    ...
    const mergedSourcebooks = SourcebookMergeLogic.merge(
      localHomebrewSourcebooks,
      remoteHomebrewSourcebooks,
      mergeBehavior,
    );
  ...
  const initializeData = () => {
    if (settings.useWarehouse) {
      ...
      localDs.getHeroes().then(heroes => {
        if (heroes !== null) {
          setLocalHeroes(heroes);
        }
      });

      localDs.getHomebrew().then(sourcebooks => {
        if (sourcebooks !== null) {
  ...
  useEffect(initializeData, [localDs, props.heroes, props.homebrewSourcebooks, settings]);
        ...
        <div className="hero-section">
          {list.map(hero => (
            <HeroPanel
              ...
              hero={hero}
              sourcebooks={SourcebookLogic.getSourcebooks(sourcebooks)}
  ...
  const getCopyLocalButton = () => {
    const localDataMissingInWarehouse =
      HeroMergeLogic.missingData(localHeroes, remoteHeroes) ||
    ...
    return (
      <Popconfirm
        ...
        cancelText="No"
      >
        <Button>Copy Warehouse data to Local</Button>
      </Popconfirm>
    );
  };
    ...
    if (!settings.useWarehouse) {
      ...
      return (
        <Space orientation="vertical" style={{ width: '100%' }}>
          ...
          />
          <Button type="primary" onClick={() => mergeToWarehouse()}>
            Merge Local data into Warehouse
          </Button>

          <HeaderText level={4}>Copy Warehouse data to Local storage</HeaderText>
          <Alert type="warning" title="This replaces the local content completely!" />
          {getCopyLocalButton()}
    ...
    <ErrorBoundary>
      <div className="transfer-page">
        <div className="transfer-page-content">
          ...
          <p>
            By default, Forge Steel uses your local browser storage to store your data. This means
            that you didn't have to register or sign up anywhere, and all of your data stays local
          ...
          <p>
            ...
            It only checks the internal ID for determining if a chunk of data is 'the same'. This
            means that copying or merging data could result in some data loss. Use at your own risk!
          </p>
          {settings.useWarehouse ? (
            <>
              <p>Once you are done, you will need to reload the app to see the updated data.</p>
              <Button type="primary" onClick={navHome}>
                Return
              </Button>
            </>
          ) : null}
