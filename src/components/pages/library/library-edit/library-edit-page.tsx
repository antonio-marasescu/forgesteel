...
export const LibraryEditPage = (props: Props) => {
  ...
  const { kind, sourcebookID, elementID } = useParams<{
      ...
      <div className="library-edit-page">
        <AppHeader subheader={getSubheader()}>
          <Button
            ...
            icon={<SaveOutlined />}
            disabled={!dirty}
            onClick={() => props.saveChanges(kind!, sourcebookID!, element)}
          >
            Save Changes
          </Button>
          <Button
            icon={<CloseOutlined />}
            onClick={() => navigation.goToLibrary(kind!, elementID!)}
          >
            Cancel
          </Button>
        </AppHeader>
        <div className="library-edit-page-content">{getEditSection()}</div>
        <AppFooter
          page="library"
