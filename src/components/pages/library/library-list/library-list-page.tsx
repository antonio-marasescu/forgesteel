...
export const LibraryListPage = (props: Props) => {
  ...
  const [previousSelectedID, setPreviousSelectedID] = useState<string | null | undefined>(
    elementID,
  );
  ...
  const [monsterFilter, setMonsterFilter] = useState<MonsterFilter>(
    FactoryLogic.createMonsterFilter(),
  );
  const [sourcebookID, setSourcebookID] = useState<string>(
    props.sourcebooks.filter(sb => sb.type === SourcebookType.Homebrew).length > 0
      ? Collections.sort(props.sourcebooks, sb => sb.name).filter(
          sb => sb.type === SourcebookType.Homebrew,
        )[0].id
      : '',
  );
  useTitle('Library');
  ...
  const getList = (type: string) => {
    ...
    switch (type) {
      ...
      case 'perk':
        ...
        break;
      case 'project':
        list = LibraryLogic.getProjects(
          getSourcebooks(),
          searchTerm,
          showProjectsFromImbuements,
          showProjectsFromItems,
        );
        break;
      case 'subclass':
  ...
  const getSidebar = () => {
            ...
            <div className="list-header">
              <Flex align="center" justify="space-between" gap={5}>
                ...
                <Button
                  ...
                  icon={
                    showMonsterFilter ? (
                      <FilterFilled style={{ color: 'rgb(22, 119, 255)' }} />
                    ) : (
                      <FilterOutlined />
                    )
                  }
        ...
        case 'culture':
          return (
            <div className="list-header">
              <Toggle
                style={{ margin: '0' }}
                label="Include cultures from ancestries"
                value={showCulturesFromAncestries}
                onChange={setShowCulturesFromAncestries}
              />
            </div>
          );
        case 'project':
          return (
            <div className="list-header">
              <Toggle
                style={{ margin: '0' }}
                label="Include projects from imbuements"
                value={showProjectsFromImbuements}
                onChange={setShowProjectsFromImbuements}
              />
              <Toggle
                style={{ margin: '0' }}
                label="Include projects from items"
                value={showProjectsFromItems}
                onChange={setShowProjectsFromItems}
              />
            </div>
          );
        case 'subclass':
          return (
            <div className="list-header">
              <Toggle
                style={{ margin: '0' }}
                label="Include subclasses from classes"
                value={showSubclassesFromClasses}
                onChange={setShowSubclassesFromClasses}
              />
            </div>
          );
      }
    ...
    const getElementListItems = () => {
      ...
      const headers = getList(category).map(item =>
        LibraryLogic.getGroupHeader(item, category, props.sourcebooks),
      );
      ...
      sortedHeaders.forEach(header => {
        ...
        const items = getList(category).filter(
          item => LibraryLogic.getGroupHeader(item, category, props.sourcebooks) === header,
        );
            ...
            <SelectorRow
              ...
              content={
                category === 'monster-group' && showMonsters ? (
                  ...
                  a.name || `Unnamed ${Format.capitalize(category.split('-').join(' '))}`
                )
              }
        ...
        if (items.length === 0) {
          listItems.push(<Empty key={`${header}-empty`} />);
        }
      });

      if (listItems.length === 0) {
        listItems.push(<Empty key="empty" />);
      }
    ...
    return (
      <div className={showSidebar ? 'selection-sidebar' : 'selection-sidebar closed'}>
        <div className="selection-toolbar">
          {showSidebar ? <SearchBox searchTerm={searchTerm} setSearchTerm={setSearchTerm} /> : null}
          {showSidebar ? <Button onClick={props.showSourcebooks}>Sourcebooks</Button> : null}
          <Button
          ...
          <div className="selection-content">
            <div className="selection-list categories">
              ...
              <SelectorRow
                ...
                content="Ancestries"
                info={getList('ancestry').length}
                onSelect={() => navigation.goToLibrary('ancestry')}
              />
              <SelectorRow
                selected={category === 'career'}
                content="Careers"
                info={getList('career').length}
                onSelect={() => navigation.goToLibrary('career')}
              />
              <SelectorRow
                selected={category === 'class'}
                content="Classes"
                info={getList('class').length}
                onSelect={() => navigation.goToLibrary('class')}
              />
              <SelectorRow
                selected={category === 'complication'}
                content="Complications"
                info={getList('complication').length}
                onSelect={() => navigation.goToLibrary('complication')}
              />
              <SelectorRow
                selected={category === 'culture'}
                content="Cultures"
                info={getList('culture').length}
                onSelect={() => navigation.goToLibrary('culture')}
              />
              <SelectorRow
                selected={category === 'domain'}
                content="Domains"
                info={getList('domain').length}
                onSelect={() => navigation.goToLibrary('domain')}
              />
              <SelectorRow
                selected={category === 'imbuement'}
                content="Imbuements"
                info={getList('imbuement').length}
                onSelect={() => navigation.goToLibrary('imbuement')}
              />
              <SelectorRow
                selected={category === 'item'}
                content="Items"
                info={getList('item').length}
                onSelect={() => navigation.goToLibrary('item')}
              />
              <SelectorRow
                selected={category === 'kit'}
                content="Kits"
                info={getList('kit').length}
                onSelect={() => navigation.goToLibrary('kit')}
              />
              <SelectorRow
                selected={category === 'perk'}
                content="Perks"
                info={getList('perk').length}
                onSelect={() => navigation.goToLibrary('perk')}
              />
              <SelectorRow
                selected={category === 'project'}
                content="Projects"
                info={getList('project').length}
                onSelect={() => navigation.goToLibrary('project')}
              />
              <SelectorRow
                selected={category === 'subclass'}
                content="Subclasses"
                info={getList('subclass').length}
                onSelect={() => navigation.goToLibrary('subclass')}
              />
              <SelectorRow
                selected={category === 'title'}
                content="Titles"
                info={getList('title').length}
                onSelect={() => navigation.goToLibrary('title')}
              />
              <div className="selection-list-group-header">
              ...
              <SelectorRow
                ...
                content="Adventures"
                info={getList('adventure').length}
                onSelect={() => navigation.goToLibrary('adventure')}
              />
              <SelectorRow
                selected={category === 'encounter'}
                content="Encounters"
                info={getList('encounter').length}
                onSelect={() => navigation.goToLibrary('encounter')}
              />
              <SelectorRow
                selected={category === 'monster-group'}
                content={showMonsters ? 'Monsters' : 'Monster Groups'}
                info={showMonsters ? getList('monster').length : getList('monster-group').length}
                onSelect={() => navigation.goToLibrary('monster-group')}
              />
              <SelectorRow
                selected={category === 'montage'}
                content="Montages"
                info={getList('montage').length}
                onSelect={() => navigation.goToLibrary('montage')}
              />
              <SelectorRow
                selected={category === 'negotiation'}
                content="Negotiations"
                info={getList('negotiation').length}
                onSelect={() => navigation.goToLibrary('negotiation')}
              />
              <SelectorRow
                selected={category === 'tactical-map'}
                content="Tactical Maps"
                info={getList('tactical-map').length}
                onSelect={() => navigation.goToLibrary('tactical-map')}
              />
              <SelectorRow
                selected={category === 'terrain'}
                content="Terrain"
                info={getList('terrain').length}
                onSelect={() => navigation.goToLibrary('terrain')}
              />
            </div>
  ...
  const getViewSelector = () => {
    ...
    switch (category) {
      ...
      case 'montage':
      case 'negotiation':
        return <ViewSelector mode="classic" value={view} onChange={setView} />;
      default:
        return (
          <ViewSelector
            mode="printable"
            ...
            onChange={value => {
              if (value === 'print') {
                const sourcebook = props.sourcebooks.find(sb =>
                  SourcebookLogic.getElements(sb)
                    .map(e => e.element.id)
                    .includes(selectedID),
                );
                if (sourcebook) {
    ...
    <ErrorBoundary>
      <div className="library-list-page">
        <AppHeader subheader="Library">
          ...
          {getViewSelector() ? <div className="divider" /> : null}
          {getViewSelector()}
        </AppHeader>
        <ErrorBoundary>
          <div className="library-list-page-content">
            ...
            <div className="element-selected">
              {selected ? getPanel(selected) : <Empty text="Nothing selected" />}
            </div>
          </div>
        </ErrorBoundary>
        <AppFooter
