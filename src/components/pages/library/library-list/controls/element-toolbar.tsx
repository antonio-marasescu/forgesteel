...
export const ElementToolbar = (props: Props) => {
  ...
  const sourcebook = LibraryLogic.getSourcebook(
    props.element,
    props.category,
    props.sourcebooks,
    props.showMonsters,
  ...
  if (!sourcebook) {
    if (props.category === 'subclass') {
      const c = SourcebookLogic.getClasses(props.sourcebooks).find(c =>
        c.subclasses.some(sc => sc.id === props.element.id),
      );
      if (c) {
        return <Button onClick={() => navigation.goToLibrary('class', c.id)}>Open {c.name}</Button>;
      }
    }
      ...
      case 'tactical-map':
        return (
          <Button
            ...
            onClick={() => props.startElement(props.category, props.element)}
          >
            Start
          </Button>
        );
        ...
        <Popover
          ...
          content={
            <Alert
              ...
              action={
                <Button style={{ marginLeft: '5px' }} onClick={() => props.setShowMonsters(false)}>
                  Switch
                </Button>
              }
            />
          ...
          content={
            ...
            >
              ...
              <Button
                type="primary"
                onClick={() =>
                  props.createElement(props.category, props.sourcebookID, props.element)
                }
              >
                Create
              </Button>
  ...
  const getEdit = () => {
    ...
    return (
      <Button
        icon={<EditOutlined />}
        onClick={() => navigation.goToLibraryEdit(props.category, sourcebook.id, props.element.id)}
      >
        Edit
      </Button>
    );
      ...
      <Popover
        ...
        content={
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', minWidth: '300px' }}>
            ...
            <Button
              type="primary"
              onClick={() => props.createElement(props.category, props.sourcebookID, props.element)}
            >
              Create a Copy
            </Button>
          </div>
    ...
    return (
      <Popover
        ...
        content={
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', minWidth: '300px' }}>
            <div>
              This currently lives in <b>{sourcebook.name || 'Unnamed Sourcebook'}</b>.
            </div>
            <DestinationSelector
              sourcebooks={props.sourcebooks}
              sourcebookID={props.sourcebookID}
            ...
            <Button
              type="primary"
              disabled={sourcebook.id === props.sourcebookID}
              onClick={() => props.moveElement(props.category, props.sourcebookID, props.element)}
            >
              Move
            </Button>
          </div>
  ...
  const getExport = () => {
    const category =
      props.category === 'monster-group' && props.showMonsters ? 'monster' : props.category;
    ...
    const imageOrPDF =
      canExportAsImage || canExportAsPDF ? (
        props.view === 'classic' ? (
          <>
            {canExportAsImage ? (
              <>
    ...
    const externalContent = LibraryLogic.getExternalContent(
      props.element,
      props.category,
      props.sourcebooks,
    );
              ...
              <Alert
                ...
                title={
                  <>
                    ...
                    <div>
                      Some homebrew content used in this {category} is located in a different
                      sourcebook.
                    </div>
                    <div>The exported file won't contain the following items:</div>
                    <ul>
                      {externalContent.map(ec => (
                        <li key={ec.element.id}>
                          <b>{ec.element.name}</b> in {ec.sourcebook.name}
                        </li>
                      ))}
                    </ul>
                  </>
      ...
      msg = (
        <>
          ...
          <ul>
            {used.map(x => (
              <li key={x.element.id}>
                <b>{x.element.name || 'Unnamed Element'}</b> is used in{' '}
                <b>{x.container.name || 'Unnamed Element'}</b>
              </li>
            ))}
          </ul>
        </>
