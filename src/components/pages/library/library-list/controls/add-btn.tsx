  ...
  const generateEncounter = () => {
    ...
    switch (difficulty) {
      case EncounterDifficulty.Easy:
        EncounterLogic.generateEncounter(
          enc,
          props.sourcebooks,
          keywords,
          budgets.maxTrivial,
        ...
        return [
            ...
            >
              {exampleEncounters.map(e => (
                <Button
                  key={e.id}
                  block={true}
                  onClick={() => props.createElement(props.category, props.sourcebookID, e)}
                >
                  {e.name}
                </Button>
              ))}
          ...
          <Expander key="random" title="Generate a random encounter">
            ...
            <Select
              ...
              options={Collections.sort(
                Collections.distinct(
                  SourcebookLogic.getMonsters(props.sourcebooks).flatMap(m => m.keywords),
                  kw => kw,
                ),
                kw => kw,
              ).map(kw => ({ value: kw, label: kw }))}
          ...
          <Expander key="image" title="Use a battlemap">
            <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              <Upload
                style={{ width: '100%' }}
                accept={
                  mapImportType === 'image' ? '.png,.webp,.gif,.jpg,.jpeg,.svg' : '.mp4,.webm'
                }
              ...
              {mapImportData ? (
                <>
                  {mapImportType === 'image' ? (
                    <img style={{ width: '100%' }} src={mapImportData} />
                  ) : (
                    <video
                      style={{ width: '100%' }}
                      src={mapImportData}
                  ...
                  <Button block={true} type="primary" onClick={createImageMap}>
                    Create
                  </Button>
                </>
              ) : null}
          ...
          <Expander key="random" title="Generate a random map">
            ...
            <NumberSpin min={1} value={mapGenerateSize} onChange={setMapGenerateSize}>
              <Field
                ...
                label={mapGenerateType === 'dungeon' ? 'Rooms' : 'Size'}
                value={mapGenerateSize}
              />
            </NumberSpin>
            <Button
              block={true}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', minWidth: '300px' }}>
          ...
          <Space orientation="vertical" style={{ width: '100%' }}>
            <Button
              ...
              block={true}
              icon={<PlusOutlined />}
              onClick={() => props.createElement(props.category, props.sourcebookID, null)}
            >
              Create
            </Button>
            <Upload
