...
export const HeroSheetPreviewPage = (props: Props) => {
  ...
  const standardAbilities = HeroLogic.getAbilities(
    FactoryLogic.createHero([]),
    [],
    AbilityData.standardAbilities.map(a => a.id),
    ...
    standardAbilityOptions.push({
      ...
      label: (
        <div className="ds-text">
          {a.name} <Tag variant="outlined">{a.actionType}</Tag>
        </div>
      ),
    });
  });
  ...
  const setDisplay = (type: 'html' | 'canvas') => {
    ...
    if (element && canvasElem) {
      ...
      switch (type) {
        ...
        case 'canvas':
          element.className = '';
          Utils.elementToImage(element, 2).then(function (image) {
  ...
  const getAllComplications = () => {
    return SourcebookLogic.getComplications(props.sourcebooks).map(
      HeroSheetBuilder.buildComplicationSheet,
    );
  };
  ...
  const getPreviewPage = () => {
    if (heroID === 'careers') {
        ...
        <main id="hero-sheet-page" className="classic-sheet">
          <div className={getPageClasses()}>
            ...
            <div className="all-careers">
              <CareerCard career={undefined} hero={fakeHero} />
              {getAllCareers().map(c => {
                return <CareerCard key={c.id} career={c} hero={fakeHero} />;
              })}
            </div>
          </div>
        </main>
        ...
        <main id="hero-sheet-page" className="classic-sheet">
          <div className={getPageClasses()}>
            ...
            <div className="all-complications">
              <ComplicationCard complication={undefined} hero={fakeHero} />
              {getAllComplications().map(c => {
                return <ComplicationCard key={c.id} complication={c} hero={fakeHero} />;
              })}
            </div>
          </div>
        </main>
      ...
      return <HeroSheetPage hero={hero} sourcebooks={props.sourcebooks} options={props.options} />;
    }
  };
