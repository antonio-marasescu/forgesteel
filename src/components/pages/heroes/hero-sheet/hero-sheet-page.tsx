import {
  CarryThreeSafelyReference,
  EdgesBanesReferenceCard,
  FallingReferenceCard,
  MainActionsReferenceCard,
  ManeuversReferenceCard,
  MarkdownReferenceCard,
  MoveActionsReferenceCard,
  MovementReferenceCard,
  RulesReferenceCard,
} from '@/components/panels/classic-sheet/reference/reference-cards';
import {
  ConsumablesCard,
  LeveledTreasureCard,
  RemainingInventoryCard,
  TrinketsCard,
} from '@/components/panels/classic-sheet/inventory-card/inventory-card';
...
export const HeroSheetPage = (props: Props) => {
  ...
  const populateExtraCards = (character: HeroSheet): ExtraCards => {
    ...
    if (character.featuresReferenceOther.length) {
      ...
      let refH = SheetFormatter.calculateFeatureReferenceSize(
        character.featuresReferenceOther,
        hero,
        lineWidth,
        1,
      );
      ...
      if (refH > 60) {
        ...
        refH = SheetFormatter.calculateFeatureReferenceSize(
          character.featuresReferenceOther,
          hero,
          lineWidth,
          2,
      ...
      required.unshift({
        element: (
          <FeatureReferenceCard character={character} columns={refW - 1} key="feature-reference" />
        ),
        width: refW,
        height: refH,
        shown: false,
    ...
    character.extraReferenceItems
      ...
      .forEach((refItem, i) => {
        ...
        lineWidth = refItem.wide
          ? (layout.perRow - 1) * layout.cardGap + layout.perRow * layout.cardLineLen
          : layout.cardLineLen;
    ...
    const numLeveledTreasures =
      character.inventory?.filter(i => ItemLogic.isLeveledTreasure(i.item)).length ?? 0;
    ...
    if (character.projects.length) {
      const h = SheetFormatter.calculateProjectsOverviewCardSize(
        character.projects,
        layout.cardLineLen,
      );
      required.push({
    ...
    addlRules.forEach(rule => {
      optional.push({
        element: (
          <RulesReferenceCard
            key={rule.label.toLocaleLowerCase().split(' ').join('-')}
            rule={rule}
          />
        ),
        width: 1,
  ...
  const getRemainingCards = (extraCards: ExtraCards) => {
    const hasRetainers = character.followers.some(f =>
      ['Retainer', 'Companion'].includes(f.classification),
    );
    ...
    if (character.followers.length) {
      character.followers
        .filter(f => f.classification !== 'Follower')
        .forEach(fs => {
          extraCards.required.unshift({
            ...
            height: Math.min(
              layoutEnd.linesY,
              SheetFormatter.calculateFollowerSize(fs, layoutEnd.cardLineLen),
            ),
            shown: false,
          });
        });
      ...
      if (followers.length) {
        extraCards.required.unshift({
          ...
          height: Math.min(
            layoutEnd.linesY,
            SheetFormatter.calculateFollowersSize(followers, layoutEnd.cardLineLen),
          ),
          shown: false,
        });
      }
    ...
    character.summons.forEach(fs => {
      extraCards.required.unshift({
        ...
        height: Math.min(
          layoutEnd.linesY,
          SheetFormatter.calculateMonsterSize(fs, layoutEnd.cardLineLen),
        ),
        shown: false,
      });
    });
    ...
    const lineLenWide =
      (layoutEnd.perRow - 1) * layoutEnd.cardGap + layoutEnd.perRow * layoutEnd.cardLineLen;
    character.extraReferenceItems
    ...
    if (artifacts.length || consumables.length > maxConsumables) {
      ...
      extraCards.required.push({
        element: (
          <RemainingInventoryCard key="inventory-overflow" character={character} items={inv} />
        ),
        width: 1,
        height: invH,
        shown: false,
      ...
      <main id="classic-sheet">
        <div className={sheetClasses.join(' ')} id={SheetFormatter.getPageId('hero', hero.id)}>
          <div
            ...
            id={SheetFormatter.getPageId('hero-sheet', hero.id)}
          >
            <HeroHeaderCard character={character} options={props.options} />
            <StatsResourcesCard character={character} options={props.options} />
            <ModifiersCard character={character} />
            <PotenciesCard character={character} />
            <ConditionsCard character={character} options={props.options} />
            <PrimaryReferenceCard character={character} options={props.options} />
            <ImmunitiesWeaknessesCard character={character} />
            <ClassFeaturesCard character={character} options={props.options} />
          </div>
          <hr className="dashed" />
          <div
            className={`page page-2 ${props.options.pageOrientation}`}
            id={SheetFormatter.getPageId('hero-sheet', hero.id, '2')}
          >
            <CultureCard character={character} />
            <div className="career-complication">
              <CareerCard career={character.career} hero={hero} />
              <ComplicationCard complication={character.complication} hero={hero} />
            </div>
            <SkillsCard character={character} />
            <PerksCard character={character} />
          </div>
          {addAbilityPages(character, extraCards)}
          <hr className="dashed" />
          <div
            className={`page page-titles-inventory-projects ${props.options.pageOrientation}`}
            id={SheetFormatter.getPageId('hero-sheet', hero.id, 'titles-inv-proj')}
          >
            <TitlesCard
              character={character}
              showLong="all"
            ...
            <TrinketsCard
              ...
              wide={props.options.pageOrientation === 'portrait'}
            />
            <LeveledTreasureCard character={character} wide={true} />
            <ConsumablesCard
              character={character}
              wide={props.options.pageOrientation !== 'portrait'}
            />
            <ProjectsCard projects={character.projects} />
          </div>
