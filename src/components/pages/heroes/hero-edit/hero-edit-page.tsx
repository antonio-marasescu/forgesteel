...
export const HeroEditPage = (props: Props) => {
  ...
  const getPageState = (page: HeroEditTab) => {
    switch (page) {
      ...
      case 'ancestry':
        if (hero.ancestry) {
          return hero.ancestry.features
            .filter(f => FeatureLogic.isChoice(f))
            .filter(f => !FeatureLogic.isChosen(f, hero)).length > 0
      ...
      case 'complication':
        if (hero.complication) {
          return hero.complication.features
            .filter(f => FeatureLogic.isChoice(f))
            .filter(f => !FeatureLogic.isChosen(f, hero)).length > 0
            ? PageState.InProgress
            : PageState.Completed;
    ...
    features.forEach(feature => {
      switch (feature.type) {
        case FeatureType.LanguageChoice:
          feature.data.selected = feature.data.selected.filter(
            language => !knownLanguages.includes(language),
          );
          break;
  ...
  const setClass = (heroClass: HeroClass | null) => {
    ...
    if (classCopy) {
      ...
      clearRedundantSelections(
        hero,
        classCopy.featuresByLevel.flatMap(byLevel => byLevel.features),
      );
    }
    const heroCopy = Utils.copy(hero);
  ...
  const setLevel = (level: number) => {
    ...
    [...HeroLogic.getCompanions(heroCopy), ...HeroLogic.getRetainers(heroCopy)].forEach(m => {
      if (m.retainer) {
  ...
  const addSubclass = (subclass: SubClass) => {
    ...
    if (heroCopy.class) {
      ...
      clearRedundantSelections(
        hero,
        selected.featuresByLevel.flatMap(byLevel => byLevel.features),
      );
    }
    setHero(heroCopy);
    setDirty(true);
  ...
  const removeSubclass = (subclassID: string) => {
    const heroCopy = Utils.copy(hero);
    if (heroCopy.class) {
      heroCopy.class.subclasses
        .filter(sc => sc.id === subclassID)
        .forEach(sc => (sc.selected = false));
    }
    setHero(heroCopy);
    setDirty(true);
  };
    ...
    switch (page) {
      ...
      case 'culture':
        setCulture(
          Collections.draw([
            CultureData.bespoke,
            ...SourcebookLogic.getCultures(props.sourcebooks, true),
          ]),
        );
        break;
      case 'career':
    ...
    return (
      <div className="page-controls">
        ...
        <Space orientation="vertical" size={4}>
          {!isSmall ? (
            <Button
              disabled={!allowRandom || !!searchTerm}
          ...
          <Button disabled={!unselect} icon={<CloseOutlined />} onClick={unselect}>
            Unselect
          </Button>
        </Space>
      </div>
