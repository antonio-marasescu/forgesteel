...
export const CultureSection = (props: CultureSectionProps) => {
  ...
  const optionsYourAncestry = cultures
    .filter(c => c.id === (props.hero.ancestry?.culture?.id || ''))
    .map(c => (
  ...
  const optionsAncestral = cultures
    .filter(c => c.type === CultureType.Ancestral)
    .map(c => (
      <SelectablePanel key={c.id} onSelect={() => props.selectCulture(c)}>
        <CulturePanel culture={c} sourcebooks={props.sourcebooks} options={props.options} />
      </SelectablePanel>
    ));
  const optionsProfessional = cultures
    .filter(c => c.type === CultureType.Professional)
    .map(c => (
      <SelectablePanel key={c.id} onSelect={() => props.selectCulture(c)}>
        <CulturePanel culture={c} sourcebooks={props.sourcebooks} options={props.options} />
      </SelectablePanel>
    ));
  const optionsBespoke = cultures
    .filter(c => c.type === CultureType.Bespoke)
    .map(c => (
      <SelectablePanel key={c.id} onSelect={() => props.selectCulture(c)}>
        <CulturePanel culture={c} sourcebooks={props.sourcebooks} options={props.options} />
      </SelectablePanel>
            ...
            {props.hero.culture.environment ? (
              <Flex className="selection-box" align="center" gap={10}>
                <Field
                  ...
                  value={
                    <Markdown text={props.hero.culture.environment.description} useSpan={true} />
                  }
                />
                <Flex vertical={true}>
                  <Button
            ...
            {props.hero.culture.organization ? (
              <Flex className="selection-box" align="center" gap={10}>
                <Field
                  ...
                  value={
                    <Markdown text={props.hero.culture.organization.description} useSpan={true} />
                  }
                />
                <Flex vertical={true}>
                  <Button
            ...
            {props.hero.culture.upbringing ? (
              <Flex className="selection-box" align="center" gap={10}>
                <Field
                  ...
                  value={
                    <Markdown text={props.hero.culture.upbringing.description} useSpan={true} />
                  }
                />
                <Flex vertical={true}>
                  <Button
              ...
              <Button
                block={true}
                className="status-warning"
                onClick={() => setShowUpbringing(true)}
              >
                Choose upbringing
              </Button>
            )}
  ...
  return (
    <ErrorBoundary>
      <div className="hero-edit-content culture-section">
        ...
        {!props.hero.culture &&
        [...optionsAncestral, ...optionsProfessional, ...optionsBespoke].length > 0 ? (
