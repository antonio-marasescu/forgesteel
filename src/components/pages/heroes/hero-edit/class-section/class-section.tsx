  ...
  const getClassOptions = (heroClass: HeroClass) => {
    ...
    options.choices.push(
      <SelectablePanel key="class-level">
        ...
        {HeroLogic.canLevelUp(props.hero, props.options) ? (
          <Button className="status-warning" onClick={() => props.setLevel(heroClass.level + 1)}>
      ...
      options.choices.push(
        <SelectablePanel key="subclass">
          ...
          <div className="ds-text">
            Choose{' '}
            {heroClass.subclassCount === 1
              ? `${Format.startsWithVowel(heroClass.subclassName || 'subclass') ? 'an' : 'a'} ${heroClass.subclassName || 'subclass'}`
              : `${heroClass.subclassCount} ${heroClass.subclassName || 'subclasse'}s`}
            .
          </div>
          {heroClass.subclasses
            ...
            .map(sc => (
              <Flex key={sc.id} align="center">
                <Field style={{ flex: '1 1 0' }} label={sc.name} value={sc.description} />
                <Flex vertical={true}>
                  <Button
                    style={{ flex: '0 0 auto' }}
                    type="text"
          ...
          {heroClass.subclasses.filter(sc => sc.selected).length < heroClass.subclassCount ? (
            <Button
              ...
              block={true}
              onClick={() => setSubclassSelectorOpen(true)}
            >
              Choose {Format.startsWithVowel(heroClass.subclassName || 'subclass') ? 'an' : 'a'}{' '}
              {heroClass.subclassName || 'subclass'}
            </Button>
          ) : null}
          <Drawer
            open={subclassSelectorOpen}
        ...
        <div className="hero-edit-content-column choices" id="class-choices">
          ...
          {choicesByLevel.map(lvl => (
            <Expander
              ...
              extra={[lvl.completed ? <CheckIcon key="completed" state="success" /> : null]}
            >
...
const Characteristics = (props: CharacteristicsProps) => {
  const getArray = () => {
    ...
    if (props.heroClass.primaryCharacteristics.length > 0) {
      const str = props.heroClass.characteristics
        ...
        .join(', ');
      currentArray =
        HeroLogic.getCharacteristicArrays(props.heroClass.primaryCharacteristics.length).find(arr =>
          Collections.getPermutations(arr)
            .map(a => a.join(', '))
            .includes(str),
        ) || null;
    }
  ...
  const [values, setValues] = useState<{ characteristic: Characteristic; value: number }[] | null>(
    null,
  );

  if (
    props.heroClass.primaryCharacteristicsOptions.length > 0 &&
    props.heroClass.primaryCharacteristics.length === 0
  ) {
    return (
      <div>
        <div className="ds-text">Your class allows you to choose your primary characteristics.</div>
        <Select
          style={{ width: '100%' }}
          status="warning"
          placeholder="Select your primary characteristics"
          options={props.heroClass.primaryCharacteristicsOptions.map(a => ({
            value: a.join(', '),
            array: a,
          }))}
          optionRender={option => <div className="ds-text">{option.data.value}</div>}
          value={
            props.heroClass.primaryCharacteristics &&
            props.heroClass.primaryCharacteristics.length > 0
  ...
  if (!array) {
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <div className="ds-text">
          You start with a 2 in <b>{props.heroClass.primaryCharacteristics.join(' and ')}</b>.
          Choose the set of values you'd like for your other characteristics.
        </div>
        {HeroLogic.getCharacteristicArrays(props.heroClass.primaryCharacteristics.length).map(
          (a, n) => (
  ...
  if (!values) {
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        <div className="ds-text">Choose your characteristics.</div>
        {HeroLogic.calculateCharacteristicArrays(array, props.heroClass.primaryCharacteristics).map(
          (a, n1) => (
            <StatsRow
              key={n1}
              onClick={() => {
