...
export const ReferenceModal = (props: Props) => {
      ...
      <>
        ...
        <Flex gap={20} style={{ flex: '1 1 0', overflowY: 'hidden' }}>
          <Space
            ...
            style={{ flex: '0 0 180px', marginTop: '20px', overflowY: 'auto' }}
          >
  ...
  const getSkillsSection = () => {
    const sourcebooks = props.hero
      ? props.hero.settingIDs.map(id => props.sourcebooks.find(s => s.id === id)).filter(s => !!s)
      : props.sourcebooks;
    const allSkills = SourcebookLogic.getSkills(sourcebooks);
    const skillNames = props.hero
      ? HeroLogic.getSkills(props.hero, sourcebooks).map(s => s.name)
      : [];
  ...
  const getLanguagesSection = () => {
    ...
    const languageNames = props.hero
      ? HeroLogic.getLanguages(props.hero, sourcebooks).map(l => l.name)
      : [];

    return (
      <div>
        {[LanguageType.Common, LanguageType.Regional, LanguageType.Cultural, LanguageType.Dead].map(
          (type, n1) => (
            <div key={n1}>
      ...
      <Tabs
        items={[
          ...
          },
          {
            ...
            children: (
              <Space orientation="vertical" style={{ paddingBottom: '20px', width: '100%' }}>
                {[AbilityData.advance, AbilityData.disengage, AbilityData.ride]
          ...
          },
          {
            ...
            children: (
              <Space orientation="vertical" style={{ paddingBottom: '20px', width: '100%' }}>
                {[AbilityData.freeStrikeMelee, AbilityData.freeStrikeRanged].map(a => (
                  <SelectablePanel key={a.id}>
                    <AbilityPanel
                      ability={a}
                      hero={props.hero || undefined}
  ...
  return (
    <Modal
      ...
      content={<div className="reference-modal">{getContent()}</div>}
