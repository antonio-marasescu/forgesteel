...
export const EncounterTurnModal = (props: Props) => {
  ...
  const getContent = () => {
    ...
    const startRound = (combatantID: string | null) => {
      ...
      copy.heroes.forEach(
        h => (h.state.encounterState = h.id === combatantID ? 'current' : 'ready'),
      );
    ...
    const endTurnAndStartNext = (currentID: string, nextID: string | null) => {
      ...
      if (nextID) {
        copy.groups
          ...
          .forEach(g => {
            const isSolo = g.slots.some(slot =>
              slot.monsters.some(m => m.role.organization === MonsterOrganizationType.Solo),
            );
        ...
        copy.groups.filter(g => g.id === nextID).forEach(g => (g.encounterState = 'current'));
        copy.heroes.filter(h => h.id === nextID).forEach(h => (h.state.encounterState = 'current'));
      } else {
    ...
    const getHeroButton = (hero: Hero, onClick: () => void) => {
      return (
        <SelectablePanel key={hero.id} onSelect={onClick}>
          <HeaderText style={{ marginTop: '-5px' }}>{hero.name}</HeaderText>
          <HeroInfo hero={hero} />
        </SelectablePanel>
      );
    };

    const activeGroups = encounter.groups.filter(g =>
      g.slots.some(s => !s.state.defeated && s.monsters.some(m => !m.state.defeated)),
    );
    ...
    if (!encounter.initiative) {
      ...
      content.push(
        <div key="initiative">
          ...
          <Flex gap={10}>
            <Button block={true} onClick={() => setEncounterInitiative('monsters')}>
              Monsters
            </Button>
            <Button block={true} onClick={() => setEncounterInitiative('heroes')}>
              Heroes
            </Button>
          </Flex>
          <SimpleRollPanel
      ...
      if ((allMonstersReady && allHeroesReady) || (allMonstersFinished && allHeroesFinished)) {
        content.push(
          <div key="start-round">
            ...
            <div className="ds-text">
              Select a {encounter.initiative === 'monsters' ? 'monster group' : 'hero'} to start the
              round and gain <b>{EncounterLogic.getMaliceGained(encounter)} malice</b>.
            </div>
            <Space orientation="vertical" style={{ width: '100%' }}>
      ...
      encounter.groups
        ...
        .forEach(group => {
          const groupButtons = groups.map(g =>
            getGroupButton(g, () => endTurnAndStartNext(group.id, g.id)),
          );
          const heroButtons = heroes.map(h =>
            getHeroButton(h, () => endTurnAndStartNext(group.id, h.id)),
          );

          content.push(
            <Space key={group.id} orientation="vertical" style={{ width: '100%' }}>
              {group.slots.map(s =>
                getConditions(s.id, EncounterLogic.getSlotName(s), s.state.conditions),
              )}
              {group.slots
                .flatMap(s => s.monsters)
      ...
      encounter.heroes
        ...
        .forEach(hero => {
          const groupButtons = groups.map(g =>
            getGroupButton(g, () => endTurnAndStartNext(hero.id, g.id)),
          );
          const heroButtons = heroes.map(h =>
            getHeroButton(h, () => endTurnAndStartNext(hero.id, h.id)),
          );
