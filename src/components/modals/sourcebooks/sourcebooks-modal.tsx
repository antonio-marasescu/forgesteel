...
export const SourcebooksModal = (props: Props) => {
  ...
  const [homebrewSourcebooks, setHomebrewSourcebooks] = useState<Sourcebook[]>(
    Utils.copy(props.homebrewSourcebooks),
  );
  const [hiddenSourcebookIDs, setHiddenSourcebookIDs] = useState<string[]>(
    Utils.copy(props.hiddenSourcebookIDs),
  );
  ...
  const getContent = () => {
    switch (page) {
      case SourcebookType.Official: {
        const officialSourcebooks = props.officialSourcebooks.filter(
          s => s.type === SourcebookType.Official,
        );
        return (
          <>
            <HeaderText level={1}>Official Sourcebooks</HeaderText>
            <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              {officialSourcebooks.length === 0 ? <Empty /> : null}
            </Space>
          </>
        );
      }
      case SourcebookType.ThirdParty: {
        const thirdPartySourcebooks = props.officialSourcebooks.filter(
          s => s.type === SourcebookType.ThirdParty,
        );
        return (
          <>
            <HeaderText
              ...
              extra={
                <Popover
                  content={
                    <Markdown
                      text={
                        "If you'd like your company's content to be featured here, [get in touch](mailto:andy.aiken@live.co.uk)."
                      }
                    />
                  }
                >
            ...
            <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              {thirdPartySourcebooks.length === 0 ? <Empty /> : null}
            </Space>
          </>
        );
      }
      case SourcebookType.Community: {
        const communitySourcebooks = props.officialSourcebooks.filter(
          s => s.type === SourcebookType.Community,
        );
        return (
          <>
            <HeaderText
              ...
              extra={
                <Popover
                  content={
                    <Markdown
                      text={
                        "If you'd like your creations to be featured here, just fill in [this form](https://forms.cloud.microsoft/r/mmxqfnFzx4)."
                      }
                    />
                  }
                >
            ...
            <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              {communitySourcebooks.length === 0 ? <Empty /> : null}
            </Space>
          </>
        );
      }
        ...
        return (
          <>
            <HeaderText
              ...
              extra={
                <Flex>
                  <Button
                    type="text"
                    title="Create a new sourcebook"
                    icon={<PlusOutlined />}
                    onClick={createSourcebook}
                  />
                  <Upload
                    style={{ width: '100%' }}
            ...
            <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              {homebrewSourcebooks.length === 0 ? <Empty /> : null}
            </Space>
          </>
        );
      }
  ...
  return (
    <Modal
      ...
      content={<div className="sourcebooks-modal">{getContent()}</div>}
