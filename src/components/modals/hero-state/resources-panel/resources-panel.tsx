...
export const ResourcesPanel = (props: Props) => {
      ...
      <>
        {HeroLogic.getHeroicResources(hero).map(hr => (
          <Space key={hr.id} orientation="vertical" style={{ width: '100%' }}>
            <HeaderText>
              {Format.capitalize(hr.type)} Resource: {hr.name}
            </HeaderText>
            <NumberSpin
              value={hr.value}
            ...
            {hr.gains.length > 0 ? (
              <>
                {hr.gains.map((g, n) => {
                  let btn = <div style={{ padding: '0 8px' }}>+{g.value}</div>;
                  ...
                  <Flex align="center" justify="space-evenly" gap={10}>
                    ...
                    >
                      <div>
                        ...
                        <div className="subtext">Victories to {hr.name || 'Heroic Resource'}</div>
                      </div>
                    </Button>
                    <Button
                      key="end-encounter"
                    ...
                    >
                      <div>
                        ...
                        <div className="subtext">+1 Victory</div>
                      </div>
                    </Button>
                  </Flex>
                ) : null}
            ...
            content={
              expression ? (
                <Space orientation="vertical" style={{ width: '100%', padding: '0 20px' }}>
                  <HeaderText level={1}>Roll</HeaderText>
      ...
      <>
        ...
        <Flex gap={20}>
          <Space orientation="vertical" style={{ flex: '1 1 0' }}>
            <NumberSpin label="Surges" value={hero.state.surges} min={0} onChange={setSurges} />
            <NumberSpin
              label="Victories"
            ...
            title={
              <>
                <div className="alert-text">
                  Spend <b>1 - 3 surges</b> to add{' '}
                  {hero.class ? Math.max(...hero.class.characteristics.map(ch => ch.value)) : 0}{' '}
                  damage per surge to one target.
                </div>
                {hero.state.surges >= 2 ? (
                  <div className="alert-text">
                    Spend <b>2 surges</b> to increase an ability’s potency by 1 for a single target.
                  </div>
                ) : null}
              </>
            }
  ...
  const getExperienceSection = () => {
    ...
    return (
      <>
        ...
        <Flex justify="center">
          <Progress
            percent={(100 * (hero.state.xp - minXP)) / props.options.xpPerLevel}
            steps={props.options.xpPerLevel}
            showInfo={false}
          />
        </Flex>
        {HeroLogic.canLevelUp(hero, props.options) ? (
          <Alert
            ...
            action={
              <Button icon={<ArrowUpOutlined />} onClick={() => setShowLevelUp(true)}>
                Level Up
              </Button>
            }
          />
  ...
  const getHeroTokenSection = () => {
    ...
    const gainStamina = () => {
      ...
      copy.state.staminaDamage = Math.max(
        copy.state.staminaDamage - HeroLogic.getRecoveryValue(copy),
        0,
      );
      setHero(copy);
      props.onChange(copy);
    ...
    return (
      <>
        ...
        <NumberSpin value={hero.state.heroTokens} min={0} onChange={setHeroTokens} />
        <Alert
          type="info"
          title={
            <Space orientation="vertical">
              <div className="alert-text">
                Hero tokens are a resource shared by your party; they typically refresh at the
              ...
              {hero.state.heroTokens >= 2 ? (
                <Flex align="center" justify="space-between" gap={10}>
                  <div className="alert-text">
                    Spend 2 hero tokens on your turn or whenever you take damage (no action
                    required) to regain Stamina equal to your recovery value before taking the
                    damage.
                  </div>
                  <div>
                    <Field
                    ...
                    />
                    <Button disabled={hero.state.staminaDamage === 0} onClick={gainStamina}>
                      +{HeroLogic.getRecoveryValue(hero)} Stamina
                    </Button>
                  </div>
                </Flex>
              ) : null}
