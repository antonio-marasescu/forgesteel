...
export const PartyModal = (props: Props) => {
  const languages = SourcebookLogic.getLanguages(props.sourcebooks)
    .map(l => l.name)
    .filter(l =>
      props.heroes.some(h => HeroLogic.getLanguages(h, props.sourcebooks).some(x => x.name === l)),
    );
  const skills = SourcebookLogic.getSkills(props.sourcebooks)
    .map(s => s.name)
    .filter(s =>
      props.heroes.some(h => HeroLogic.getSkills(h, props.sourcebooks).some(x => x.name === s)),
    );

  const itemProficiencies = Collections.distinct(
    props.heroes.flatMap(h => HeroLogic.getProficiencies(h)),
    x => x,
  ).sort();
            ...
            <table>
              <thead>
                <tr>
                  <th></th>
                  {props.heroes.map(h => (
                    <th key={h.id} className="hero-column-header">
                      {h.name}
                    </th>
                  ))}
                </tr>
                    ...
                    <tr key={n}>
                      ...
                      {props.heroes.map(h => (
                        <td key={h.id} className="row-cell">
                          {HeroLogic.getLanguages(h, props.sourcebooks).some(x => x.name === l) ? (
                ...
                {skills.map((s, n) => {
                  return (
                    <tr key={n}>
                      ...
                      {props.heroes.map(h => (
                        <td key={h.id} className="row-cell">
                          {HeroLogic.getSkills(h, props.sourcebooks).some(x => x.name === s) ? (
                            <CheckIcon state="success" />
                          ) : null}
                        </td>
                      ))}
                ...
                {itemProficiencies.map((i, n) => {
                  return (
                    <tr key={n}>
                      ...
                      {props.heroes.map(h => (
                        <td key={h.id} className="row-cell">
                          {HeroLogic.getProficiencies(h).includes(i) ? (
                            <CheckIcon state="success" />
                          ) : null}
                        </td>
                      ))}
