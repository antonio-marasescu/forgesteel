        ...
        <div className="player-view-modal">
          <Space orientation="vertical" style={{ width: '100%' }}>
            ...
            <Alert
              ...
              title={
                <div>
                  <p>Player View opens in a new window.</p>
                  <p>
                    To share it with your players, don't share the link - share the window (ie in
                    Discord or on a second screen).
                  </p>
                </div>
              }
            />
            {session.encounters.length === 0 &&
            session.montages.length === 0 &&
            session.negotiations.length === 0 &&
            session.tacticalMaps.length === 0 &&
            ...
            {session.encounters.map(e => (
              <Toggle
                key={e.id}
                label={e.name}
                value={session.playerViewID === e.id}
                onChange={value => setPlayerViewID(value ? e.id : null)}
              />
            ))}
            {session.montages.map(m => (
              <Toggle
                key={m.id}
                label={m.name}
                value={session.playerViewID === m.id}
                onChange={value => setPlayerViewID(value ? m.id : null)}
              />
            ))}
            {session.negotiations.map(n => (
              <Toggle
                key={n.id}
                label={n.name}
                value={session.playerViewID === n.id}
                onChange={value => setPlayerViewID(value ? n.id : null)}
              />
            ))}
            {session.tacticalMaps.map(tm => (
              <Toggle
                key={tm.id}
                label={tm.name}
                value={session.playerViewID === tm.id}
                onChange={value => setPlayerViewID(value ? tm.id : null)}
              />
            ))}
            {session.counters.map(c => (
              <Toggle
                key={c.id}
                label={c.name}
                value={session.playerViewID === c.id}
                onChange={value => setPlayerViewID(value ? c.id : null)}
              />
            ))}
          </Space>
        </div>
