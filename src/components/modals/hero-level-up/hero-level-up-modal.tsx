    ...
    <Modal
      toolbar={
        <>
          <Button
            type="primary"
            disabled={features.some(f => !FeatureLogic.isChosen(f, hero))}
            onClick={() => props.onAccept(hero)}
          >
            Accept Changes
          </Button>
      ...
      content={
        <div className="hero-level-up-modal">
          ...
          <div className="ds-text">
            You gain the following features at <b>level {hero.class!.level}</b>.
          </div>
          <Tabs
            items={[
              {
              ...
              },
              {
                ...
                children: (
                  <Space orientation="vertical" style={{ width: '100%' }}>
                    {features.filter(f => !FeatureLogic.isChoice(f)).map(getFeature)}
                    {features.filter(f => !FeatureLogic.isChoice(f)).length === 0 ? (
                      <Empty />
                    ) : null}
                  </Space>
                ),
