...
export const AbilityModal = (props: Props) => {
  const [hero, setHero] = useState<Hero | undefined>(
    props.hero ? Utils.copy(props.hero) : undefined,
  );
  ...
  const customization = hero
    ? hero.abilityCustomizations.find(ac => ac.abilityID === props.ability.id)
    : undefined;

  const hasCost = props.ability.cost !== 'signature';
  const hasRange = props.ability.distance.some(
    d => ![AbilityDistanceType.Self, AbilityDistanceType.Special].includes(d.type),
  );
  ...
  const getContent = () => {
    switch (page) {
      case 'Ability Card': {
        ...
        if (hero && rollSection) {
          const values = rollSection.roll.characteristic.map(ch =>
            HeroLogic.getCharacteristic(hero!, ch),
          );
              ...
              <Expander title="Power Roll">
                <Space orientation="vertical" style={{ width: '100%' }}>
                  <Select
                    ...
                    ].map(ch => ({ value: ch, label: <div className="ds-text">{ch}</div> }))}
  ...
  return (
    <Modal
      ...
      content={<div className="ability-modal">{getContent()}</div>}
