...
export const KitSelectModal = (props: Props) => {
  ...
  const kits = props.kits.filter(k => Utils.textMatches([k.name, k.description], searchTerm));
      ...
      content={
        <div className="kit-select-modal">
          <Space orientation="vertical" style={{ width: '100%' }}>
            {kits.map(k => (
              <SelectablePanel key={k.id} onSelect={() => props.onSelect(k)}>
                <KitPanel
                  kit={k}
                  hero={props.hero}
