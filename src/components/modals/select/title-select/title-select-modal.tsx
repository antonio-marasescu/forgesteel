...
export const TitleSelectModal = (props: Props) => {
  const [echelon, setEchelon] = useState<number>(
    CreatureLogic.getEchelon(props.hero.class ? props.hero.class.level : 1),
  );
  ...
  customTitle.features.push(
    FactoryLogic.feature.create({ id: Utils.guid(), name: 'Custom Title', description: 'Details' }),
  );
  ...
  const titles = sortedTitles.filter(t => Utils.textMatches([t.name, t.description], searchTerm));
          ...
          {selectedTitle === null ? (
            <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              {[customTitle, ...titles].map(t => (
                <SelectablePanel key={t.id} onSelect={() => selectTitle(t)}>
                  <TitlePanel
                    title={t}
                    hero={props.hero}
            ...
            <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              {selectedTitle.features.map(f => (
                <SelectablePanel key={f.id} onSelect={() => selectFeature(f)}>
                  <FeaturePanel
                    feature={f}
                    hero={props.hero}
                    mode={PanelMode.Full}
              ...
              {selectedTitle.features.length === 0 ? <Empty /> : null}
            </Space>
          )}
        </div>
      }
