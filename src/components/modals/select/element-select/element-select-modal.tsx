...
export const ElementSelectModal = (props: Props) => {
  ...
  const [customElement, setCustomElement] = useState<Element>({
    id: Utils.guid(),
    name: '',
    description: '',
  });
  ...
  const elements = props.elements.filter(e =>
    Utils.textMatches([e.name, e.description], searchTerm),
  );
        ...
        <div className="element-select-modal">
          ...
          <Expander title="Add a custom element">
            <Space orientation="vertical" style={{ width: '100%' }}>
              ...
              <Button
                block={true}
                disabled={!customElement.name}
                onClick={() => props.onSelect(customElement)}
              >
                Select
              </Button>
            </Space>
