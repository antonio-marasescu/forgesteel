...
export const RollModal = (props: Props) => {
  ...
  if (props.characteristics && props.hero) {
    if (
      props.characteristics.some(ch => [Characteristic.Might, Characteristic.Agility].includes(ch))
    ) {
      if (
        props.hero.state.conditions.some(c => c.type === ConditionType.Bleeding) ||
    ...
    characteristicBonus = Math.max(
      ...props.characteristics.map(ch => HeroLogic.getCharacteristic(props.hero!, ch)),
    );
  }
  ...
  return (
    <Modal
      ...
      content={<div className="roll-modal">{getContent()}</div>}
