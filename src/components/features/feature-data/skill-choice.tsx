...
export const InfoSkillChoice = (props: InfoProps) => {
  if (props.data.selected.length > 0) {
    return <Field label="Skill" value={props.data.selected.join(', ')} />;
  }
  ...
  if (!props.feature.description) {
    ...
    if (props.data.listOptions.length === 5) {
      ...
      const names = (Collections.sort(props.data.options, o => o) || [])
        .concat((Collections.sort(props.data.listOptions, o => o) || []).map(l => `the ${l} list`))
        .join(', ');
      str = count > 1 ? `Choose ${count} skills from ${names}.` : `Choose a skill from ${names}.`;
    }

    return <div className="ds-text">{str}</div>;
  }
...
export const EditSkillChoice = (props: EditProps) => {
  ...
  const skills = SourcebookLogic.getSkills(props.sourcebooks as Sourcebook[]).filter(
    skill => data.options.includes(skill.name) || data.listOptions.includes(skill.list),
  );
  ...
  return (
    <Space orientation="vertical" style={{ width: '100%' }}>
      ...
      <Select
        ...
        allowClear={true}
        mode="multiple"
        options={SourcebookLogic.getSkills(props.sourcebooks).map(option => ({
          value: option.name,
...
export const ConfigSkillChoice = (props: ConfigProps) => {
  ...
  const skills = SourcebookLogic.getSkills(props.sourcebooks as Sourcebook[])
    .filter(
      skill =>
        props.data.options.includes(skill.name) || props.data.listOptions.includes(skill.list),
    )
