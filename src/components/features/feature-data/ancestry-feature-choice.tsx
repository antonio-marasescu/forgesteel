...
export const InfoAncestryFeatureChoice = (props: InfoProps) => {
  if (!props.data.selected) {
    return <div className="ds-text">A {props.data.value}pt ancestry feature.</div>;
  }
...
export const ConfigAncestryFeatureChoice = (props: ConfigProps) => {
  ...
  if (props.data.source.customID && props.sourcebooks) {
    const a = SourcebookLogic.getAncestries(props.sourcebooks).find(
      a => a.id === props.data.source.customID,
    );
    if (a) {
      ancestries.push(a);
    }
    ...
    <Space orientation="vertical" style={{ width: '100%' }}>
      <Select
        ...
        options={sortedFeatures.map(f => ({
          ...
          disabled: currentFeatureIDs.includes(f.id),
        }))}
        optionRender={option => (
          <Field
