...
export const EditSummon = (props: EditProps) => {
  ...
  return (
    <Space orientation="vertical" style={{ width: '100%' }}>
      <HeaderText
        extra={<Button type="text" icon={<PlusOutlined />} onClick={() => addSummon(data)} />}
      >
        Options
      </HeaderText>
      {data.summons.map((summon, n) => (
        ...
        >
          ...
          <Toggle
            ...
            onChange={value => setSummonIsSignature(data, n, value)}
          />
          <NumberSpin
            min={1}
            label="Cost"
            value={summon.info.cost}
            onChange={value => setSummonCost(data, n, value)}
          />
          <NumberSpin
            min={1}
            label="Count"
            value={summon.info.count}
            onChange={value => setSummonCount(data, n, value)}
          />
          <MonsterEditPanel
      ...
      {data.summons.length === 0 ? <Empty /> : null}
    </Space>
  );
};
