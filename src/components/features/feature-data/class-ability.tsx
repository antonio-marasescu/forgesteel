...
export const InfoClassAbility = (props: InfoProps) => {
  if (!props.hero) {
    return (
      <div className="ds-text">
        Choose {props.data.count > 1 ? props.data.count : 'a'}{' '}
        {props.data.cost === 'signature' || props.data.cost === 0
          ? 'signature'
          : `${props.data.cost}pt`}{' '}
        {props.data.count > 1 ? 'abilities' : 'ability'}.
      </div>
    );
  }
  ...
  if (props.data.classID && props.sourcebooks) {
    ...
    heroClass =
      SourcebookLogic.getClasses(props.sourcebooks).find(c => c.id === props.data.classID) || null;
  }
  if (!heroClass) {
    return null;
  }
    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        {props.data.selectedIDs.map(id => {
          ...
          return <AbilityPanel key={ability.id} ability={ability} mode={PanelMode.Full} />;
        })}
      </Space>
    );
  }
    ...
    return (
      <div className="ds-text">
        ...
        {props.data.cost === 'signature' || props.data.cost === 0
          ...
          : `${props.data.cost}pt`}{' '}
        {props.data.count > 1 ? 'abilities' : 'ability'}
        {props.data.classID ? ` from the ${heroClass.name}` : ''}.
      </div>
    );
  }
    ...
    <Space orientation="vertical" style={{ width: '100%' }}>
      ...
      <Select
        ...
        options={[
          ...
          ...SourcebookLogic.getClasses(props.sourcebooks).map(o => ({
            value: o.id,
            label: `From the ${o.name}`,
          })),
        ]}
        optionRender={option => <div className="ds-text">{option.data.label}</div>}
        value={data.classID || ''}
      ...
      <Toggle
        ...
        value={data.source.fromClassAbilities}
        onChange={setClassAbilities}
      />
      <Toggle
        label="Abilities from selected subclasses"
        value={data.source.fromSelectedSubclassAbilities}
        onChange={setSelectedSubclassAbilities}
      />
      <Toggle
        label="Abilities from unselected subclasses"
        value={data.source.fromUnselectedSubclassAbilities}
        onChange={setUnselectedSubclassAbilities}
      />
      <Toggle
        label="Abilities from class levels"
        value={data.source.fromClassLevels}
        onChange={setClassLevels}
      />
      <Toggle
        label="Abilities from selected subclass levels"
        value={data.source.fromSelectedSubclassLevels}
        onChange={setSelectedSubclassLevels}
      />
      <Toggle
        label="Abilities from unselected subclass levels"
        value={data.source.fromUnselectedSubclassLevels}
        onChange={setUnselectedSubclassLevels}
      />
...
export const ConfigClassAbility = (props: ConfigProps) => {
  ...
  return (
    <Space orientation="vertical" style={{ width: '100%' }}>
      <div className="ds-text">
        Choose {props.data.count > 1 ? props.data.count : 'a'}{' '}
        {props.data.cost === 'signature' ? 'signature' : `${props.data.cost}pt`}{' '}
        {props.data.count > 1 ? 'abilities' : 'ability'}.
      </div>
      {props.data.selectedIDs.map(id => {
        const ability = abilities.find(a => a.id === id) as Ability;
