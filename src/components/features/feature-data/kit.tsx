  ...
  if (!props.feature.description) {
    return (
      <div className="ds-text">
        Choose {props.data.count > 1 ? props.data.count : 'a'} {props.data.types.join(', ')}{' '}
        {props.data.count > 1 ? 'kits' : 'kit'}.
      </div>
    );
  }
      ...
      <Select
        ...
        options={Collections.sort(
          Collections.distinct(
            SourcebookLogic.getKits(props.sourcebooks).map(k => k.type),
            x => x,
          ),
          x => x,
        ).map(type => ({ value: type, label: !type ? 'Standard' : type }))}
