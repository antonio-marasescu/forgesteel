  ...
  if (props.data.selected.length > 0) {
    return (
      <Space
        ...
        style={{ width: '100%', padding: '0 20px', borderLeft: '5px solid rgb(200 200 200)' }}
      >
      ...
      {props.data.options.map(o => (
        <div key={o.feature.id} className="container">
          <FeaturePanel
            ...
            cost={showCosts ? o.value : undefined}
            mode={PanelMode.Full}
          />
        </div>
      ))}
...
export const EditChoice = (props: EditProps) => {
  ...
  return (
    <Space orientation="vertical" style={{ width: '100%' }}>
      <HeaderText
        extra={<Button type="text" icon={<PlusOutlined />} onClick={() => addChoice(data)} />}
      >
        Options
      </HeaderText>
        ...
        >
          <Space orientation="vertical" style={{ width: '100%' }}>
            ...
            <NumberSpin
              min={1}
              value={option.value}
              onChange={value => setChoiceValue(data, n, value)}
            />
          </Space>
        </Expander>
      ))}
      {data.options.length === 0 ? <Empty /> : null}
      <HeaderText>Count</HeaderText>
      <Toggle
        label="Use ancestry points"
        value={data.count === 'ancestry'}
        onChange={value => setChoiceCount(value ? 'ancestry' : 3)}
      />
      {data.count !== 'ancestry' ? (
        <NumberSpin min={1} value={data.count} onChange={setChoiceCount} />
      ) : null}
    </Space>
  );
};
...
export const ConfigChoice = (props: ConfigProps) => {
  ...
  const pointsMax =
    props.data.count === 'ancestry' ? HeroLogic.getAncestryPoints(props.hero!) : props.data.count;
    ...
    <Space orientation="vertical" style={{ width: '100%' }}>
      ...
      {pointsLeft > 0 ? (
        sortedOptions.length === 0 ? (
          ...
          <Button
            ...
            block={true}
            onClick={() => setChoiceSelectorOpen(true)}
          >
      ...
      {pointsLeft > 0 && props.data.count === 'ancestry' ? (
        <>
          <Divider />
          <Toggle
            label="Choose a feature from a different ancestry"
            value={comprehensive}
      ...
      {comprehensive ? (
        <Alert type="warning" showIcon={true} title="This is typically against the rules." />
      ) : null}
      <Drawer
        open={choiceSelectorOpen}
