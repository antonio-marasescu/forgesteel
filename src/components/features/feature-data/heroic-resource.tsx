...
export const InfoHeroicResource = (props: InfoProps) => {
  if (props.hero) {
    const resource = HeroLogic.getHeroicResources(props.hero).find(
      hr => hr.name === props.feature.name,
    );
    if (resource) {
      return (
        <>
...
export const EditHeroicResource = (props: EditProps) => {
  ...
  return (
    <Space orientation="vertical" style={{ width: '100%' }}>
      ...
      />
      <HeaderText
        extra={<Button type="text" icon={<PlusOutlined />} onClick={() => addResourceGain(data)} />}
      >
      ...
      {data.gains.length === 0 ? <Empty /> : null}
      <Divider />
