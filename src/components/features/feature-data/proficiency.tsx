  ...
  return (
    <>
      {props.data.weapons.length > 0 ? (
        <Field label="Weapons" value={props.data.weapons.join(', ')} />
      ) : null}
      {props.data.armor.length > 0 ? (
        <Field label="Armor" value={props.data.armor.join(', ')} />
      ) : null}
    </>
  );
};
  ...
  return (
    <Space orientation="vertical" style={{ width: '100%' }}>
      ...
      <Select
        ...
        mode="multiple"
        allowClear={true}
        options={[KitArmor.Heavy, KitArmor.Light, KitArmor.Medium, KitArmor.Shield].map(option => ({
          value: option,
        }))}
        optionRender={option => <div className="ds-text">{option.data.value}</div>}
        showSearch={true}
