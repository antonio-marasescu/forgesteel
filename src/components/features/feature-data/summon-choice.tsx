    ...
    return (
      <>
        <div className="ds-text">
          ...
          {props.data.count > 1 ? 'monsters' : 'monster'}.
        </div>
        <Space orientation="vertical" style={{ width: '100%' }}>
...
export const EditSummonChoice = (props: EditProps) => {
  ...
  const setSummonChoiceIsSignature = (
    data: FeatureSummonChoiceData,
    index: number,
    value: boolean,
  ) => {
  ...
  return (
    <Space orientation="vertical" style={{ width: '100%' }}>
      <HeaderText
        extra={<Button type="text" icon={<PlusOutlined />} onClick={() => addSummonChoice(data)} />}
      >
        Options
      </HeaderText>
      {data.options.map((summon, n) => (
        ...
        >
          ...
          <Toggle
            label="Is signature"
            value={summon.info.isSignature}
            onChange={value => setSummonChoiceIsSignature(data, n, value)}
          />
          <NumberSpin
            min={1}
            label="Cost"
            value={summon.info.cost}
            onChange={value => setSummonChoiceCost(data, n, value)}
          />
          <NumberSpin
            min={1}
            label="Count"
            value={summon.info.count}
            onChange={value => setSummonChoiceCount(data, n, value)}
          />
          <MonsterEditPanel
    ...
    <Space orientation="vertical" style={{ width: '100%' }}>
      ...
      {props.data.selected.map(summon => (
        <Flex className="selection-box" align="center" gap={10}>
          <MonsterInfo style={{ flex: '1 1 0' }} monster={summon.monster} />
