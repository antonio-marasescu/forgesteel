...
export const EditDamageModifier = (props: EditProps) => {
  ...
  const setDamageModifierDamageType = (
    data: FeatureDamageModifierData,
    index: number,
    value: DamageType,
  ) => {
  ...
  const setDamageModifierType = (
    data: FeatureDamageModifierData,
    index: number,
    value: DamageModifierType,
  ) => {
    const copy = Utils.copy(data);
  ...
  const setDamageModifierValueCharacteristics = (
    data: FeatureDamageModifierData,
    index: number,
  ...
  const setDamageModifierValuePerLevel = (
    data: FeatureDamageModifierData,
    index: number,
    value: number,
  ) => {
  ...
  const setDamageModifierValuePerEchelon = (
    data: FeatureDamageModifierData,
    index: number,
    value: number,
  ) => {
        ...
        <Expander key={n} title="Damage Modifier">
          <Space orientation="vertical" style={{ width: '100%' }}>
            ...
            <NumberSpin
              ...
              min={0}
              value={mod.value}
              onChange={value => setDamageModifierValue(data, n, value)}
            />
            <Select
              style={{ width: '100%' }}
            ...
            <NumberSpin
              ...
              min={0}
              value={mod.valuePerLevel}
              onChange={value => setDamageModifierValuePerLevel(data, n, value)}
            />
            <NumberSpin
              label="Per Echelon"
              min={0}
              value={mod.valuePerEchelon}
              onChange={value => setDamageModifierValuePerEchelon(data, n, value)}
            />
            <DangerButton mode="block" onConfirm={() => deleteDamageModifier(data, n)} />
          </Space>
        </Expander>
      ))}
      {data.modifiers.length === 0 ? <Empty /> : null}
    </Space>
  );
};
