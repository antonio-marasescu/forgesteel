    ...
    return (
      <Space orientation="vertical" style={{ width: '100%' }}>
        {props.data.selected.map(f => (
          <FeaturePanel key={f.id} feature={f} options={props.options} />
        ))}
      </Space>
    );
  }
  ...
  if (!props.feature.description) {
    return (
      <div className="ds-text">
        Choose {props.data.count > 1 ? props.data.count : 'a'} '{props.data.tag}'{' '}
        {props.data.count > 1 ? 'features' : 'feature'}.
      </div>
    );
  }
    ...
    <Space orientation="vertical" style={{ width: '100%' }}>
      ...
      {sortedFeatures.length > 0 ? (
        <Select
          ...
          options={sortedFeatures.map(f => ({
            label: f.name,
            value: f.id,
            desc: f.description || f.type,
            disabled: currentTaggedFeatureIDs.includes(f.id),
          }))}
          optionRender={option => (
            <Field
