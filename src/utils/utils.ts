...
export class Utils {
  ...
  static exportImage = (elementIDs: string[], name: string) => {
    const elements = elementIDs.map(id => document.getElementById(id)).filter(element => !!element);
  ...
  static elementToImage = (element: HTMLElement, scale: number): Promise<HTMLImageElement> => {
    ...
    const fontScaleFix = (node: Node) => {
      if (node instanceof HTMLElement) {
        node.style.fontSize = node.style.fontSize.replace(
          /(\d+(\.\d+)?(e[+-]?\d+)?)/g,
          (match, number) => {
  ...
  static elementsToPdf = async (
    elementIDs: string[],
    filename: string,
    pdfPaperSize: SheetPageSize,
    ...
    return Promise.all(elements.map(e => this.elementToImage(e, scale))).then(images => {
  ...
  static savePdfPages = async (
    filename: string,
    pageCanvases: HTMLImageElement[],
    pdfPaperSize: SheetPageSize,
    dpi: number,
  ) => {
