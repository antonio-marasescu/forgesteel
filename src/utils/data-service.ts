...
export class DataService {
  ...
  constructor(settings: ConnectionSettings) {
    ...
    this.tokenHandlerHost = Utils.valueOrDefault(
      envVal,
      'https://forgesteel-warehouse-b7wsk.ondigitalocean.app',
    );
  }
  ...
  private getErrorMessage = (error: unknown) => {
    ...
    if (error instanceof AxiosError) {
      ...
      if (error.response) {
        ...
        const respMsg =
          error.response.data.message ?? error.response.data.msg ?? error.response.data;
