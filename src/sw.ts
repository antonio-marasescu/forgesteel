...
const STATIC_CACHE_URLS = ['/forgesteel/', '/forgesteel/index.html', '/forgesteel/manifest.json'];
    ...
    (async () => {
      try {
        ...
        if (
          response &&
          response.status === 200 &&
          fetchEvent.request.url.includes('/forgesteel/')
        ) {
...
self.addEventListener('push', (event: Event) => {
  ...
  if (pushEvent.data) {
    ...
    pushEvent.waitUntil(swSelf.registration.showNotification(data.title, options));
  }
});
