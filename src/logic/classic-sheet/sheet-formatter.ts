...
export class SheetFormatter {
  ...
  static convertFeatures = (
    features: { feature: Feature; display: 'short' | 'full' }[],
  ): Feature[] => {
  ...
  static isLongFeature = (f: Feature, check: number = 2): boolean => {
    return (
      [FeatureType.Text, FeatureType.Package].includes(f.type) &&
  ...
  static divideFeatures = (
    sourcedFeatures: { feature: Feature; source: string }[],
    hero: Hero | null,
  ...
  static formatAbilityTier = (
    ...
    if (ability.distance.length > 1) {
      ...
      const values = distanceTypes.map(d => {
        return {
          type: d,
          effect: SheetFormatter.cleanupText(
            AbilityLogic.getTierEffectCreature(value, tier, ability, d, creature),
          ).split('; '),
        };
      });
    ...
    return SheetFormatter.cleanupText(
      AbilityLogic.getTierEffectCreature(value, tier, ability, undefined, creature),
    );
  };
  ...
  static getAbilityIcon = (ability: Ability | AbilitySheet) => {
    ...
    if (distance?.includes('Aura') || distance?.includes('Burst')) {
      ...
      distance?.includes('Line') ||
      distance?.includes('Cube') ||
      distance?.includes('Wall')
    ) {
  ...
  static calculateMonsterSize = (
    ...
    size +=
      1.6 * Math.max(0, (monster.abilities?.length || 0) + (monster.features?.length || 0) - 1);
  ...
  static calculateProjectsOverviewCardSize = (
  ...
  static sortFeatures = (a: Feature, b: Feature): number => {
    ...
    if (aSort && bSort) {
      const typeOrder =
        this.featureTypeOrder.indexOf(a.type) - this.featureTypeOrder.indexOf(b.type);
      if (typeOrder === 0) {
        return (
          this.calculateFeatureSize(a, null, 50, false) -
          this.calculateFeatureSize(b, null, 50, false)
        );
      }
      return typeOrder;
    } else if (aSort) {
