...
export class SheetLayout {
  ...
  static getFillerCards = (
    slotsToFillInRow: number,
    availableLinesY: number,
    rowH: number,
    ...
    nextCard: while (
      availableLinesY > 0 &&
      (extraCards.required.find(c => !c.shown) || extraCards.optional.find(c => !c.shown))
    ) {
      ...
      if (spaceInRow === layout.perRow) {
        ...
        slotsToFillInRow = layout.perRow;
        if (rowH > 0) availableLinesY -= 2.5; // For vertical card gap between rows
        rowH = 0;
      }
      ...
      let card = extraCards.required
        .filter(c => !c.shown && c.width <= spaceInRow)
  ...
  static getAbilityPagesForCharacter = (
    character: HeroSheet,
    extraCards: ExtraCards,
    layout: CardPageLayout,
    options: Options,
    ...
    if (options.shownStandardAbilities.length) {
      allAbilities = allAbilities.concat(
        character.standardAbilities.filter(a => options.shownStandardAbilities.includes(a.id)),
      );
    }

    allAbilities.sort(SheetFormatter.sortAbilitiesByType);

    return this.getAbilityPages(allAbilities, extraCards, layout, p =>
      SheetFormatter.getPageId('hero-sheet', character.hero.id, `abilities-${p}`),
    );
  };

  static getAbilityPages = (
    allAbilities: AbilitySheet[],
    extraCards: ExtraCards,
    layout: CardPageLayout,
    getPageId: (s: string) => string,
  ) => {
    ...
    while (numAbilitiesPlaced < allAbilities.length) {
          ...
          if (pageH + aH <= layout.linesY) {
            ...
            if (rowSlotsFilled >= 2) {
              ...
              for (let i = rowSlotsFilled; i > 0; --i) {
                ...
                if (slot) currentRow.unshift(slot);
              }

              const slotHeights = currentRow.map(arr => arr.reduce((h, ash) => (h += ash.h), 0));
              if (
                slotHeights[slotHeights.length - 1] + slotHeights[slotHeights.length - 2] <=
                slotH
              ) {
      ...
      if (numAbilitiesPlaced === pageStart) {
        console.warn(
          `Didn't add any abilities to this page! (pg ${abilityCardPages.length}), n=${numAbilitiesPlaced}/${allAbilities.length}`,
        );
              ...
              if (a.length > 1) {
                return (
                  <div className="stacked-cards" key={`abilities-${p}-${i}`}>
                    {a.map(sa => (
                      <AbilityCard key={sa.card.id} ability={sa.card} />
                    ))}
                  </div>
                );
              } else {
                return <AbilityCard key={a[0].card.id} ability={a[0].card} />;
              }
            })}
            {refCards}
          </div>
      ...
      pages.push(
        <Fragment key={`${idPrefix}-${++i}`}>
          ...
          <div
            ...
            id={SheetFormatter.getPageId('hero-sheet', character.hero.id, `${idPrefix}-${i}`)}
          >
            {cards}
          </div>
        </Fragment>,
  ...
  static getMonsterCardPages = (
    monsterCards: FillerCard[],
    ...
    while (extraCards.required.find(c => !c.shown)) {
      ...
      pages.push(
        <Fragment key={`${idPrefix}-${++i}`}>
          ...
          <div
            className={pageClasses.join(' ')}
            id={SheetFormatter.getPageId('encounter', encounter.id, `${idPrefix}-${i}`)}
          >
            {cards}
          </div>
        </Fragment>,
