...
export class LibraryLogic {
  static getAdventures = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getAdventures(sourcebooks).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
      return [];
  ...
  static getAncestries = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getAncestries(sourcebooks).filter(item =>
        Utils.textMatches(
          [item.name, item.description, ...item.features.map(f => f.name)],
          searchTerm,
        ),
      );
    } catch (ex) {
  ...
  static getCareers = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getCareers(sourcebooks).filter(item =>
        Utils.textMatches(
          [item.name, item.description, ...item.features.map(f => f.name)],
          searchTerm,
        ),
  ...
  static getClasses = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getClasses(sourcebooks).filter(item =>
        Utils.textMatches(
          [
            item.name,
            item.description,
            ...
            ...item.subclasses.flatMap(sc =>
              sc.featuresByLevel.flatMap(lvl => lvl.features.map(f => f.name)),
            ),
          ],
          searchTerm,
        ),
  ...
  static getComplications = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getComplications(sourcebooks).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
  ...
  static getCultures = (
    ...
    searchTerm: string,
    showCulturesFromAncestries: boolean,
  ) => {
    try {
      return SourcebookLogic.getCultures(sourcebooks, showCulturesFromAncestries).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
  ...
  static getDomains = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getDomains(sourcebooks).filter(item =>
        Utils.textMatches(
          [
            item.name,
            item.description,
  ...
  static getEncounters = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getEncounters(sourcebooks).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
  ...
  static getImbuements = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getImbuements(sourcebooks).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
  ...
  static getItems = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getItems(sourcebooks).filter(item =>
        Utils.textMatches(
          [
            item.name,
            item.description,
  ...
  static getKits = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getKits(sourcebooks).filter(item =>
        Utils.textMatches(
          [item.name, item.description, ...item.features.map(f => f.name)],
          searchTerm,
        ),
  ...
  static getMonsterGroups = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getMonsterGroups(sourcebooks).filter(item =>
        Utils.textMatches(
          [item.name, item.description, ...item.monsters.map(m => m.name)],
          searchTerm,
        ),
      );
    } catch (ex) {
  ...
  static getMontages = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getMontages(sourcebooks).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
  ...
  static getNegotiations = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getNegotiations(sourcebooks).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
  ...
  static getPerks = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getPerks(sourcebooks).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
  ...
  static getProjects = (
    sourcebooks: Sourcebook[],
    searchTerm: string,
    showProjectsFromImbuements: boolean,
    showProjectsFromItems: boolean,
  ) => {
    try {
      return SourcebookLogic.getProjects(
        sourcebooks,
  ...
  static getSubclasses = (
    sourcebooks: Sourcebook[],
    searchTerm: string,
    showSubclassesFromClasses: boolean,
  ) => {
    try {
      return SourcebookLogic.getSubclasses(sourcebooks, showSubclassesFromClasses).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
  ...
  static getTacticalMaps = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getTacticalMaps(sourcebooks).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
  ...
  static getTerrainObjects = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getTerrains(sourcebooks).filter(item =>
        Utils.textMatches([item.name, item.description], searchTerm),
      );
    } catch (ex) {
      console.error(ex);
  ...
  static getTitles = (sourcebooks: Sourcebook[], searchTerm: string) => {
    try {
      return SourcebookLogic.getTitles(sourcebooks).filter(item =>
        Utils.textMatches(
          [item.name, item.description, ...item.features.map(f => f.name)],
          searchTerm,
        ),
  ...
  static getSourcebook = (
    ...
    switch (category) {
      ...
      case 'class':
        ...
        break;
      case 'complication':
        sourcebook = SourcebookLogic.getComplicationSourcebook(
          sourcebooks,
  ...
  static getGroupHeader = (
    ...
    if (category === 'subclass') {
      const c = SourcebookLogic.getClasses(sourcebooks).find(c =>
        c.subclasses.some(sc => sc.id === element.id),
      );
      if (c) {
        return c.name;
      }
      ...
      if (sourcebookData) {
        ...
        if (elementData) {
          ...
          if (elementData.type === 'encounter') {
            external.push(
              ...LibraryLogic.getExternalContent(elementData.element, 'encounter', sourcebooks),
            );
          }
        }
      }
