...
describe('getPowerRollCharacteristics', () => {
  ...
  test.each([
    ...
    'should return the Ability RollSection characteristics when no creature is passed',
    (ability: Ability, expected: Characteristic[]) => {
...
describe('getTextEffect', () => {
  ...
  it('should calculate constant dice roll effects properly when no hero is provided', () => {
    expect(AbilityLogic.getTextEffect('Value equal to 1d6 + 2', undefined)).toBe(
      'Value equal to 1d6 + 2',
    );
  });

  it('should calculate constant dice roll effects properly when a hero is provided', () => {
    const hero = {} as Hero;
    expect(AbilityLogic.getTextEffect('Value equal to 1d6 + 2', hero)).toBe(
      'Value equal to 1d6 + 2',
    );
  });
  ...
  test.each([
    ...
    'should properly swap in the correct Hero potency value',
    (text: string, potency: number, expected: string) => {
      const hero = {} as Hero;
  ...
  test.each([
    ...
    'should properly calculate references to hero level',
    (text: string, level: number, expected: string) => {
      const hero = {
        class: { level: level },
      } as Hero;
  ...
  test.each([
    ...
    'should properly calculate combinations of dice rolls and hero level',
    (text: string, level: number, expected: string) => {
      const hero = {
        class: { level: level },
      } as Hero;
  ...
  test.each([
    ...
    'should properly calculate multiplier references to characteristics',
    (text, characteristic, expected) => {
