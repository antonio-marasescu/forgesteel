...
export class SourcebookLogic {
  ...
  ): { element: Element; type: SourcebookElementKind }[] => {
    return [
      ...sourcebook.adventures.map(x => ({
        element: x,
        type: 'adventure' as SourcebookElementKind,
      })),
      ...sourcebook.ancestries.map(x => ({
        element: x,
        type: 'ancestry' as SourcebookElementKind,
      })),
      ...
      ...sourcebook.complications.map(x => ({
        element: x,
        type: 'complication' as SourcebookElementKind,
      })),
      ...
      ...sourcebook.encounters.map(x => ({
        element: x,
        type: 'encounter' as SourcebookElementKind,
      })),
      ...sourcebook.imbuements.map(x => ({
        element: x,
        type: 'imbuement' as SourcebookElementKind,
      })),
      ...
      ...sourcebook.subclasses.map(x => ({
        element: x,
        type: 'subclass' as SourcebookElementKind,
      })),
      ...sourcebook.tacticalMaps.map(x => ({
        element: x,
        type: 'tactical-map' as SourcebookElementKind,
      })),
  ...
  static getExampleContent = (sourcebook: Sourcebook) => {
    const elements = SourcebookLogic.getElements(sourcebook);
  ...
  static getAdventureSourcebook = (sourcebooks: Sourcebook[], adventure: Adventure) => {
    return sourcebooks.find(s =>
      SourcebookLogic.getAdventures([s]).some(a => a.id === adventure.id),
    );
  };

  static getAncestrySourcebook = (sourcebooks: Sourcebook[], ancestry: Ancestry) => {
    return sourcebooks.find(s =>
      SourcebookLogic.getAncestries([s]).some(a => a.id === ancestry.id),
    );
  };
  ...
  static getComplicationSourcebook = (sourcebooks: Sourcebook[], complication: Complication) => {
    return sourcebooks.find(s =>
      SourcebookLogic.getComplications([s]).some(c => c.id === complication.id),
    );
  };

  static getCultureSourcebook = (sourcebooks: Sourcebook[], culture: Culture) => {
    return sourcebooks.find(s =>
      SourcebookLogic.getCultures([s], true).some(c => c.id === culture.id),
    );
  };
  ...
  static getEncounterSourcebook = (sourcebooks: Sourcebook[], encounter: Encounter) => {
    return sourcebooks.find(s =>
      SourcebookLogic.getEncounters([s]).some(e => e.id === encounter.id),
    );
  };

  static getImbuementSourcebook = (sourcebooks: Sourcebook[], imbuement: Imbuement) => {
    return sourcebooks.find(s =>
      SourcebookLogic.getImbuements([s]).some(i => i.id === imbuement.id),
    );
  };
  ...
  static getMonsterGroupSourcebook = (sourcebooks: Sourcebook[], monsterGroup: MonsterGroup) => {
    return sourcebooks.find(s =>
      SourcebookLogic.getMonsterGroups([s]).some(mg => mg.id === monsterGroup.id),
    );
  };
  ...
  static getNegotiationSourcebook = (sourcebooks: Sourcebook[], negotiation: Negotiation) => {
    return sourcebooks.find(s =>
      SourcebookLogic.getNegotiations([s]).some(n => n.id === negotiation.id),
    );
  };
  ...
  static getProjectSourcebook = (sourcebooks: Sourcebook[], project: Project) => {
    return sourcebooks.find(s =>
      SourcebookLogic.getProjects([s], true, true).some(p => p.id === project.id),
    );
  };

  static getSubclassSourcebook = (sourcebooks: Sourcebook[], subclass: SubClass) => {
    return sourcebooks.find(s =>
      SourcebookLogic.getSubclasses([s], true).some(s => s.id === subclass.id),
    );
  };
  ...
  static getProjects = (
    ...
    includeFromImbuements: boolean,
    includeFromItems: boolean,
  ) => {
    const list: Project[] = [];
  ...
  static getAbilitiesFromClass = (
    ...
    if (unselectedSubclassAbilities) {
      abilities.push(
        ...heroClass.subclasses.filter(sc => !sc.selected).flatMap(sc => sc.abilities),
      );
    }
  ...
  static getSimilarMonsters = (sourcebooks: Sourcebook[], monster: Monster, options: Options) => {
    return sourcebooks
      ...
      .filter(
        m => !options.similarOrganization || m.role.organization === monster.role.organization,
      )
      .filter(
        m =>
          !options.similarSize ||
          (m.size.value === monster.size.value && m.size.mod === monster.size.mod),
      );
  };
  ...
  static getUsedIn = (sourcebooks: Sourcebook[], elementID: string) => {
    return [
      ...SourcebookLogic.getEncounters(sourcebooks).filter(enc =>
        EncounterLogic.getMonsterData(enc)
          .map(data => data.monsterID)
          .includes(elementID),
      ),
      ...SourcebookLogic.getAdventures(sourcebooks).filter(adv =>
        AdventureLogic.getContentIDs(adv.plot).includes(elementID),
      ),
    ] as Element[];
  };
}
