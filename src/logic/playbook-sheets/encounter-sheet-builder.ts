...
import {
  ...
  EncounterSheet,
  EncounterSlotSheet,
} from '@/models/classic-sheets/encounter-sheet';
...
export class EncounterSheetBuilder {
  static buildEncounterSheet = (
    ...
    const terrain = encounter.terrain
      .map(slot => SourcebookLogic.getTerrains(sourcebooks).find(t => t.id === slot.terrainID))
      .filter(t => !!t);
    sheet.terrain = terrain;

    const encounterMonsters = EncounterLogic.getMonsterData(encounter)
      .map(data =>
        EncounterLogic.getCustomizedMonster(data.monsterID, data.customization, sourcebooks),
      )
      .filter(m => !!m);
    ...
    sheet.malice = possibleMalice
      .map(groupMalice => {
        const maxLvl = encounterMonsters.reduce(
          (maxLvl, monster) => Math.max(maxLvl, monster.level),
          0,
        );
  ...
  static buildEncounterGroupSheet = (
    group: EncounterGroup,
    sourcebooks: Sourcebook[],
  ): EncounterGroupSheet => {
    const slots = group.slots
      .map(s => this.buildEncounterSlotSheet(s, sourcebooks))
      .filter(s => !!s);
    const adjustedSlots: EncounterSlotSheet[] = [];
    slots.forEach(slot => {
      const existingMinion = adjustedSlots
        .filter(s => s.isMinion)
        .find(s => s.monster.id === slot.monster.id);
      if (existingMinion) {
  ...
  static buildEncounterSlotSheet = (
    slot: EncounterSlot,
    sourcebooks: Sourcebook[],
  ): EncounterSlotSheet | null => {
