...
export class HeroUpdateLogic {
  ...
  static updateHeroStructure = (hero: Hero, sourcebooks: Sourcebook[]) => {
    ...
    HeroLogic.getFormerAncestries(hero)
    ...
    HeroLogic.getFeatures(hero)
      .map(f => f.feature)
      .forEach(FeatureUpdateLogic.updateFeature);
    HeroLogic.getAbilities(hero, sourcebooks, [])
      .map(a => a.ability)
    ...
    if (
      HeroLogic.getFeatures(hero)
        .map(f => f.feature)
        .filter(f => f.type === FeatureType.HeroicResource).length === 0
    ) {
      const currentClass = hero.class;
      const updatedClass = SourcebookLogic.getClasses(sourcebooks).find(
      ...
      if (currentClass && updatedClass) {
        ...
        currentClass.featuresByLevel[0].features = currentClass.featuresByLevel[0].features.filter(
          f => !heroicResourceIDs.includes(f.id) && !heroicResourceNames.includes(f.name),
        );
    ...
    try {
      switch (feature.type) {
        case FeatureType.AncestryChoice: {
          ...
          if (oFeature.data.selected) {
            ...
            feature.data.selected =
              SourcebookLogic.getAncestries(sourcebooks)
                .map(a => Utils.copy(a))
        ...
        case FeatureType.AncestryFeatureChoice: {
          ...
          feature.data.selected =
            availableOptions.find(o => oFeature.data.selected?.id === o.id) || null;
          break;
        }
        case FeatureType.Choice: {
          ...
          if (
            availableOptions.some(opt => opt.feature.type === FeatureType.AncestryFeatureChoice)
          ) {
          ...
          feature.data.selected = availableOptions
            .map(o => o.feature)
        ...
        case FeatureType.ClassAbility: {
          ...
          feature.data.selectedIDs = oFeature.data.selectedIDs.filter(id =>
        ...
        case FeatureType.Domain: {
          ...
          feature.data.selected = SourcebookLogic.getDomains(sourcebooks)
            ...
            .map(d => {
              ...
              copy.featuresByLevel = copy.featuresByLevel.filter(lvl =>
                feature.data.levels.includes(lvl.level),
              );
              [
                ...copy.defaultFeatures,
                ...copy.featuresByLevel.flatMap(lvl => lvl.features),
              ].forEach(f =>
                FeatureLogic.switchFeatureCharacteristic(
                  f,
                  Characteristic.Intuition,
                  feature.data.characteristic,
                ),
              );
              return copy;
            });
