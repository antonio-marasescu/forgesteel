...
export class HeroLogic {
  ...
  static getFeatures = (hero: Hero) => {
    ...
    return Collections.sort(features, f => f.feature.name).map(f => {
      const customization =
        hero.abilityCustomizations.find(ac => ac.abilityID === f.feature.id) || null;
      if (customization) {
  ...
  static getAbilities = (hero: Hero, sourcebooks: Sourcebook[], standardAbilityIDs: string[]) => {
    ...
    HeroLogic.getFeatures(hero)
      ...
      .forEach(f => {
        ...
        if (feature.data.classID) {
          heroClass =
            SourcebookLogic.getClasses(sourcebooks).find(c => c.id === feature.data.classID) ||
            null;
        }
        if (heroClass) {
    ...
    return abilities.map(a => {
      const customization =
        hero.abilityCustomizations.find(ac => ac.abilityID === a.ability.id) || null;
      if (customization) {
        ...
        if (customization.characteristic) {
          ability.sections
            ...
            .forEach(s => (s.roll.characteristic = [customization.characteristic!]));
        }

        if (customization.notes) {
          ability.sections.push(
            FactoryLogic.createAbilitySectionField({ name: 'Notes', effect: customization.notes }),
          );
        }
  ...
  static getSpeed = (hero: Hero) => {
    const getValue = (hero: Hero) => {
      ...
      if (
        hero.state.conditions.some(c =>
          [ConditionType.Grabbed, ConditionType.Restrained].includes(c.type),
        )
      ) {
        value = 0;
      }
  ...
  static getKitDamageBonuses = (hero: Hero) => {
    ...
    HeroLogic.getKits(hero).forEach(kit => {
      if (kit.meleeDamage) {
        kits.push({
          name: kit.name,
          type: 'melee',
    ...
    const reducedKits = uniqueKits.filter(
      k => !worse.some(wk => k.name === wk.name && k.type === wk.type),
    );
  ...
  static getFeatureDamageBonuses = (
    hero: Hero,
    ability: Ability,
    distance: AbilityDistanceType | undefined,
  ) => {
  ...
  static getDistanceBonus = (hero: Hero, ability: Ability) => {
    ...
    if (
      ability.keywords.includes(AbilityKeyword.Melee) &&
  ...
  static getProficiencies = (hero: Hero) => {
    return [...HeroLogic.getArmorProficiencies(hero), ...HeroLogic.getWeaponProficiencies(hero)];
  };
  ...
  static calculateCharacteristicArrays = (array: number[], primary: Characteristic[]) => {
    ...
    return Collections.distinct(Collections.getPermutations(array), item => item.join(', ')).map(
      arr => {
        return all.map(ch => {
          let value = 0;
  ...
  static calculateSurgeDamage = (hero: Hero) => {
    return hero.class && hero.class.characteristics.length > 0
      ? Math.max(
          ...hero.class.characteristics.map(c => this.getCharacteristic(hero, c.characteristic)),
        )
      : 0;
  };
  ...
  static createRandomHero = () => {
    ...
    const array = Collections.draw(
      HeroLogic.getCharacteristicArrays(hero.class.primaryCharacteristics.length),
    );
    hero.class.characteristics = Collections.draw(
      HeroLogic.calculateCharacteristicArrays(array, hero.class.primaryCharacteristics),
    );
      ...
      .forEach(feature => {
        switch (feature.type) {
          ...
          case FeatureType.Companion: {
            const options = SourcebookLogic.getMonsterGroups(sourcebooks).flatMap(
              mg => mg.monsters,
            );
          ...
          case FeatureType.Domain: {
            while (feature.data.selected.length < feature.data.count) {
              ...
              const options = SourcebookLogic.getDomains(sourcebooks).filter(
          ...
          case FeatureType.ItemChoice: {
            while (feature.data.selected.length < feature.data.count) {
              ...
              const options = SourcebookLogic.getItems(sourcebooks)
                ...
                .filter(
                  i => feature.data.types.length === 0 || feature.data.types.includes(i.type),
                );
              feature.data.selected.push(Collections.draw(options));
            }
            break;
          ...
          case FeatureType.Kit: {
            while (feature.data.selected.length < feature.data.count) {
              ...
              const options = SourcebookLogic.getKits(sourcebooks)
                ...
                .filter(
                  k => feature.data.types.length === 0 || feature.data.types.includes(k.type),
                );
              feature.data.selected.push(Collections.draw(options));
            }
            break;
          ...
          case FeatureType.LanguageChoice: {
            while (feature.data.selected.length < feature.data.count) {
              ...
              const options = SourcebookLogic.getLanguages(sourcebooks).filter(
                l => !current.includes(l.name),
              );
          ...
          case FeatureType.Perk: {
            while (feature.data.selected.length < feature.data.count) {
              ...
              const options = SourcebookLogic.getPerks(sourcebooks)
                ...
                .filter(
                  p => feature.data.lists.length === 0 || feature.data.lists.includes(p.list),
                );
              feature.data.selected.push(Collections.draw(options));
            }
            break;
    ...
    hero.career.incitingIncidents.selected = Utils.copy(
