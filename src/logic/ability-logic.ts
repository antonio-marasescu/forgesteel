...
export class AbilityLogic {
  ...
  static getPanelWidth = (ability: Ability) => {
    ...
    const textLength = Collections.sum(
      ability.sections.filter(s => s.type === 'text'),
      s => Math.round(s.text.split(' ').length / 10),
    );
    const fieldLength = Collections.sum(
      ability.sections.filter(s => s.type === 'field'),
  ...
  static getPowerRollBonusValue = (
    ...
    if (rollCharacteristics.length) {
      rollPowerAmount = Math.max(
        ...rollCharacteristics.map(c => CreatureLogic.getCharacteristic(creature, c)),
      );
    } else {
  ...
  static getPowerRollCharacteristics = (
    ability: Ability,
    creature: Hero | Monster | undefined,
  ): Characteristic[] => {
    const rollSections = ability.sections.filter(s => s.type === 'roll');
    ...
    return value
      ...
      .map((section, n) => {
        if (hero && n === 0 && ['damage', 'dmg'].some(s => section.toLowerCase().endsWith(s))) {
          ...
          if (distance) {
            isMelee =
              distance === AbilityDistanceType.Melee &&
              ability.keywords.includes(AbilityKeyword.Weapon);
            isRanged =
          ...
          const dmgKits = HeroLogic.getKitDamageBonuses(hero).filter(dmg => {
            switch (dmg.type) {
              case 'melee':
                return isMelee;
              case 'ranged':
        ...
        if (
          hero &&
          n === 0 &&
          ['pull', 'push', 'slide'].some(s => section.toLowerCase().includes(s))
        ) {
          let value = 0;
          let sign = '+';
          let vertical = false;
  ...
  static getTierEffectRetainer = (
    value: string,
    tier: number,
    ability: Ability,
    retainer: Monster | undefined,
  ) => {
    return value
      .split(';')
      .map(section => section.trim())
