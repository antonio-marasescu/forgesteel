...
export class TacticalMapLogic {
  ...
  static scatterCombatants(
    map: TacticalMap,
    tokens: { type: 'hero' | 'monster'; encounterID: string; id: string; size: number }[],
  ) {
  ...
  static addRoom(map: TacticalMap) {
    ...
    if (dimensions) {
      ...
      for (let x = dimensions.minX; x !== dimensions.maxX; ++x) {
        for (let y = dimensions.minY; y !== dimensions.maxY; ++y) {
          const canAdd = this.canAddTileHere(
            map,
            x,
            y,
      ...
      map.items
        ...
        .forEach(tile => {
          ...
          const maxX = Math.min(
            room.position.x + room.dimensions.width - 1,
            tile.position.x + tile.dimensions.width - 1,
          );
          const overlapX = maxX - minX + 1;
          if (overlapX >= 2) {
            const corridorTop =
              Math.min(
                room.position.y + room.dimensions.height - 1,
                tile.position.y + tile.dimensions.height - 1,
              ) + 1;
  ...
  static addWalls(map: TacticalMap, addWalls: boolean, addDoors: boolean) {
    ...
    tiles.forEach(tile => {
      ...
      tiles
        .filter(
          t =>
            t.position.y + t.dimensions.height - 1 === top - 1 &&
            t.position.x <= right &&
            t.position.x + t.dimensions.width - 1 >= left,
        )
        .forEach(t => {
          const door = addDoor(
            Math.max(left, t.position.x),
            top,
            Math.min(right + 1, t.position.x + t.dimensions.width),
            top,
          );
      ...
      tiles
        .filter(
          t =>
            t.position.x === right + 1 &&
            t.position.y <= bottom &&
            t.position.y + t.dimensions.height - 1 >= top,
        )
        .forEach(t => {
          const door = addDoor(
            right + 1,
            Math.max(top, t.position.y),
            right + 1,
            Math.min(bottom + 1, t.position.y + t.dimensions.height),
          );
      ...
      tiles
        .filter(
          t =>
            t.position.x + t.dimensions.width - 1 === left - 1 &&
            t.position.y <= bottom &&
            t.position.y + t.dimensions.height - 1 >= top,
        )
        .forEach(t => {
    ...
    Collections.distinct(walls, wall => {
