...
export class EncounterLogic {
  ...
  static getMonsterData = (encounter: Encounter) => {
    ...
    encounter.groups
      ...
      .forEach(s => {
        const key =
          s.monsterID +
          s.customization.addOnIDs.join('') +
          (s.customization.convertToSolo ? 'SOLO' : '');
  ...
  static getCustomizedMonster = (
    ...
    if (monster && monsterGroup) {
      ...
      if (customization.levelAdjustment !== 0) {
        ...
        const characteristicMod =
          suggestedAfter.highestCharacteristic - suggestedBefore.highestCharacteristic;
        const evMod = suggestedAfter.ev - suggestedBefore.ev;
        const staminaMod = suggestedAfter.stamina - suggestedBefore.stamina;
        const freeStrikeDamageMod =
          suggestedAfter.freeStrikeDamage - suggestedBefore.freeStrikeDamage;
        ...
        const max = Collections.max(
          copy.characteristics.map(c => c.value),
          v => v,
        );
        copy.characteristics
          .filter(c => c.value === max)
          .forEach(c => (c.value += characteristicMod));
      ...
      if (customization.convertToSolo) {
        ...
        copy.features.push(
          FactoryLogic.feature.create({
            ...
            name: 'Solo Turns',
            description:
              'The creature can take two turns each round. They can’t take turns consecutively.',
          }),
        );
        ...
        copy.features.push(
          FactoryLogic.feature.create({
            ...
            name: 'Solo Action',
            description:
              '(5 Malice) The creature takes an additional main action on their turn. They can use this feature even if they are dazed.',
          }),
        );
        ...
        if (!copy.features.some(f => f.name === 'End Effect')) {
          copy.features.push(
            FactoryLogic.feature.create({
              ...
              name: 'End Effect',
              description:
                'At the end of each of their turns, the creature can take 10 damage to end one effect on them that can be ended by a saving throw. This damage can’t be reduced in any way.',
            }),
          );
        }
      }
  ...
  static getAllMaliceFeatures = (
    encounter: Encounter,
    sourcebooks: Sourcebook[],
  ): { group: string; features: (FeatureMalice | FeatureMaliceAbility)[] }[] => {
  ...
  static getEncounterVictory = (encounter: Encounter) => {
    ...
    if (
      activeCombatants.every(c => c.type === 'group') &&
      inactiveCombatants.some(c => c.type === 'hero')
    ) {
      return 'monsters';
    }

    if (
      activeCombatants.every(c => c.type === 'hero') &&
      inactiveCombatants.some(c => c.type === 'group')
    ) {
      return 'heroes';
    }
