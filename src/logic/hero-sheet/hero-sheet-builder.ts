...
export class HeroSheetBuilder {
  static buildHeroSheet = (hero: Hero, sourcebooks: Sourcebook[], options: Options): HeroSheet => {
    ...
    const featureItems = allFeatures
      .map(f => f.feature)
      .filter(f => f.type === FeatureType.ItemChoice)
      .flatMap(f => f.data.selected);

    const inventory = hero.state.inventory.concat(featureItems);
    sheet.inventory = inventory.map(item =>
      ClassicSheetBuilder.buildItemSheet(item, hero, options),
    );
    coveredFeatureIds.push(
      ...inventory.flatMap(
        i =>
          FeatureLogic.getFeaturesFromItem(i, hero)
            .map(f => f.feature)
            .map(f => f.id) || [],
      ),
    );
    ...
    if (hero.class) {
      ...
      if (hero.class.name === 'Conduit') {
        ...
        sheet.subclassName = HeroLogic.getDomains(hero)
          .map(d => d.name)
          .join(', ');
      }
      ...
      if (heroicResource) {
        sheet.heroicResourceGains = [...heroicResource.gains].sort(
          SheetFormatter.sortHeroicResourceGains,
        );
    ...
    const modifiers = allFeatures
      .map(f => f.feature)
      .filter(f => f.type !== FeatureType.Choice)
      .filter(
        f =>
          f.name.match(' Augmentation') ||
          f.name.match('Ward') ||
    ...
    if (kits.length) {
      ...
      sheet.modifierWeaponImplement = kits
        .map(k => k.weapon[0])
        .filter(v => v)
        .join(', ');
      ...
      const kitFeatures = kits
        .flatMap(k => k.features)
    ...
    if (hero.class) {
      ...
      classFeatures = classFeatures.filter(f =>
        ClassicSheetLogic.includeFeature(f.feature, options),
      );

      const perkIds = classFeatures
        .map(f => f.feature)
      ...
      if (Math.max(sheet.immunities.length, sheet.weaknesses.length) > 3) {
        const extraLines = Math.ceil(
          Math.max(sheet.immunities.length, sheet.weaknesses.length) / 2,
        );
        classFeatureSpace -= 2 * extraLines;
      }
      classFeatureSpace = classFeatureSpace * numCols;
      const dividedClassFeatures = SheetFormatter.divideFeatures(
        classFeatures,
        hero,
        classFeatureSpace,
        classFeatureLineLen,
      ...
      sheet.classFeatures = SheetFormatter.enhanceFeatures(
        SheetFormatter.convertFeatures(dividedClassFeatures.displayed),
      );
    ...
    conditions
      .filter(c => [ConditionType.Custom, ConditionType.Quick].includes(c.type))
      .forEach((c, i) => {
        if (i === 0) {
    ...
    coveredFeatureIds.push(
      ...allFeatures.filter(f => f.feature.type === FeatureType.SkillChoice).map(f => f.feature.id),
    );

    // Culture
    if (hero.culture) {
      const cultureFeatures = FeatureLogic.getFeaturesFromCulture(hero.culture, hero).map(
        f => f.feature,
      );
    ...
    const divided = SheetFormatter.divideFeatures(
      combinedAncestryPerks,
      hero,
      perkSpace * 2,
      perkLineLen,
      2,
    ...
    const freeStrikes = [AbilityData.freeStrikeMelee, AbilityData.freeStrikeRanged].map(a =>
      ClassicSheetBuilder.buildAbilitySheet(a, hero, undefined, options),
    );
    sheet.abilities = abilities
      .map(a => ClassicSheetBuilder.buildAbilitySheet(a, hero, undefined, options))
      .concat(freeStrikes);

    sheet.standardAbilities = AbilityData.standardAbilities.map(a =>
      ClassicSheetBuilder.buildAbilitySheet(a, hero, undefined, options),
    );
    ...
    const retinue = allFeatures
      .filter(f =>
        ...
        ].includes(f.feature.type),
      )
      .map(f => f.feature);
    sheet.followers = retinue
      .flatMap(f => this.buildFollowerCompanionSheet(f, hero))
      .filter(s => !!s);

    const summons = retinue
      .filter(f => f.type === FeatureType.SummonChoice)
      .flatMap(f => f.data.selected)
      .concat(retinue.filter(f => f.type === FeatureType.Summon).flatMap(f => f.data.summons));
    sheet.summons = summons
      .filter(f => CreatureLogic.isSummon(f))
      .map(f => this.buildSummonSheet(f, hero))
      .filter(s => !!s);
    ...
    allFeatures
      .filter(f => !coveredFeatureIds.includes(f.feature.id))
      .forEach(f => missedFeatures.push(f));
  ...
  private static modifierFieldMapping: {
    [key: string]: (s: HeroSheet, v: string | number | undefined) => void;
  } = {
  ...
  static buildComplicationSheet = (complication: Complication): ComplicationSheet => {
    ...
    const drawbacks = complicationFeatures
      .filter(ClassicSheetLogic.isFeatureDrawback)
      .map(f => this.stripDuplicateComplicationName(complication.name, f));
    sheet.drawbacks = drawbacks;

    const benefits = complicationFeatures
      .filter(f => !ClassicSheetLogic.isFeatureDrawback(f))
      .map(f => this.stripDuplicateComplicationName(complication.name, f));
  ...
  static buildFollowerCompanionSheet = (feature: Feature, hero: Hero) => {
    if (feature.type === FeatureType.Follower) {
      ...
      (feature.type === FeatureType.Retainer || feature.type === FeatureType.Companion) &&
      ...
      return feature.data.selected
        .filter(CreatureLogic.isCompanion)
        .map(s => this.buildCompanionSheet(s, hero));
    }
  };
  ...
  static buildRetainerSheet = (follower: Monster, heroLevel: number | undefined): FollowerSheet => {
    ...
    sheet.features = MonsterLogic.getFeatures(follower).filter(f =>
      [FeatureType.Text, FeatureType.AddOn].includes(f.type),
    );
    ...
    if ((!heroLevel || heroLevel >= 4) && follower.retainer?.level4?.type === FeatureType.Ability) {
      advancement.push({
        level: 4,
        ability: ClassicSheetBuilder.buildAbilitySheet(
          follower.retainer.level4.data.ability,
          follower,
        ),
      });
    }
    if ((!heroLevel || heroLevel >= 7) && follower.retainer?.level7?.type === FeatureType.Ability) {
      advancement.push({
        level: 7,
        ability: ClassicSheetBuilder.buildAbilitySheet(
          follower.retainer.level7.data.ability,
          follower,
        ),
      });
    }
    if (
      (!heroLevel || heroLevel >= 10) &&
      follower.retainer?.level10?.type === FeatureType.Ability
    ) {
      advancement.push({
        level: 10,
        ability: ClassicSheetBuilder.buildAbilitySheet(
          follower.retainer.level10.data.ability,
          follower,
        ),
      });
    }
  ...
  static buildCompanionSheet = (companion: Summon, hero: Hero): FollowerSheet => {
    ...
    sheet.features = MonsterLogic.getFeatures(monster).filter(f =>
      [FeatureType.Text, FeatureType.AddOn].includes(f.type),
    );
  ...
  static buildSummonSheet = (summon: Summon, hero: Hero): MonsterSheet => {
    ...
    summonCost +=
      summon.info.count === 1 ? 'per minion summoned' : `for ${summon.info.count} minions`;
    sheet.cost = summonCost;
    sheet.freeStrikeDamageType =
      monster.freeStrikeType !== DamageType.Damage ? monster.freeStrikeType : '';

    const immunities = CreatureLogic.getSummonDamageModifiers(
      summon,
      hero,
      DamageModifierType.Immunity,
    );
    sheet.immunity = immunities.map(mod => `${mod.damageType} ${mod.value}`).join(', ');
    return sheet;
  };
  ...
  static buildProjectSheet = (project: Project, hero: Hero): ProjectSheet => {
    ...
    let characteristics = SheetFormatter.joinCommasOr(
