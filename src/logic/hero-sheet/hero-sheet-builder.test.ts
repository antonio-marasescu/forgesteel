...
const undeadSignatureChoices = circleOfGraves.featuresByLevel
  .find(fbl => fbl.level === 1)
  ?.features.find(f => f.id === 'summoner-4-1-4')?.data as FeatureSummonChoiceData;
const minionSummon1 = undeadSignatureChoices.options.find(
  o => o.monster.id === 'summoner-4-1-4a',
) as Summon;

const beastheartCompanionChoices = beastheart.featuresByLevel
  .find(fbl => fbl.level === 1)
  ?.features.find(f => f.id === 'beastheart-1-2a')?.data as FeatureSummonChoiceData;
const companion1 = beastheartCompanionChoices.options.find(
  o => o.monster.id === 'beastheart-1-2a-1',
) as Summon;
...
describe('buildFollowerCompanionSheet()', () => {
  test('it should call the correct builder method for Follower features', () => {
    ...
    const mockBuilderMethod = vi
      .spyOn(HeroSheetBuilder, 'buildFollowerSheet')
  ...
  test('it should call the correct builder method for Companion features', () => {
    ...
    const mockBuilderMethod = vi
      .spyOn(HeroSheetBuilder, 'buildRetainerSheet')
      .mockReturnValueOnce(mockResult);
  ...
  test('it should call the correct builder method for Summoner selection features', () => {
    ...
    const mockBuilderMethod = vi
      .spyOn(HeroSheetBuilder, 'buildCompanionSheet')
