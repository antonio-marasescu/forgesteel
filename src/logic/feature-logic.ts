import {
  ...
  FeatureLanguageData,
  FeatureMaliceAbilityData,
  FeatureMaliceData,
  FeatureMovementModeData,
  FeatureMultipleData,
  FeaturePackageContentData,
  FeaturePackageData,
  FeaturePerkData,
  FeatureProficiencyData,
  FeatureRetainerData,
  FeatureSaveThresholdData,
  FeatureSizeData,
  FeatureSkillChoiceData,
  FeatureSpeedData,
  FeatureSummonChoiceData,
  FeatureSummonData,
  FeatureTaggedFeatureChoiceData,
  FeatureTaggedFeatureData,
...
export class FeatureLogic {
  ...
  static getFeaturesFromClass = (heroClass: HeroClass, hero: Hero) => {
    ...
    heroClass.featuresByLevel.forEach(lvl => {
      if (lvl.level <= classLevel) {
        features.push(
          ...lvl.features.map(f => ({ feature: f, source: heroClass.name, level: lvl.level })),
        );
      }
    });
    ...
    heroClass.subclasses
      ...
      .forEach(sc => {
        sc.featuresByLevel.forEach(lvl => {
          if (lvl.level <= classLevel) {
            features.push(
              ...lvl.features.map(f => ({ feature: f, source: sc.name, level: lvl.level })),
            );
          }
        });
      });
  ...
  static simplifyFeatures = (
    features: { feature: Feature; source: string; level?: number }[],
    hero: Hero,
  ) => {
      ...
      switch (feature.type) {
        ...
        case FeatureType.ItemChoice:
          feature.data.selected.forEach(item =>
            FeatureLogic.getFeaturesFromItem(item, hero).forEach(f =>
        ...
        case FeatureType.Kit:
          feature.data.selected.forEach(kit =>
            kit.features.forEach(f => addFeature(f, kit.name, level)),
          );
          break;
