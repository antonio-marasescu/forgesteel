...
export class FormatLogic {
  ...
  static getModifier = (mod: Modifier) => {
    ...
    if (mod.value && mod.valuePerLevel && mod.value === mod.valuePerLevel) {
      ...
      if (mod.valuePerLevel) {
        sections.push(
          `${mod.valuePerLevel >= 0 ? '+' : ''}${mod.valuePerLevel} per level after 1st`,
        );
      }
    }
  ...
  static getMultiplier = (text: string) => {
    ...
    const x: { n: number; words: string[] }[] = [
      ...
      {
        n: 3,
        words: ['thrice', 'three times', '3x', '3 x', '3×', '3 ×', '3 times'],
      },
      {
        n: 4,
        words: ['four times', '4x', '4 x', '4×', '4 ×', '4 times'],
      },
      {
        n: 5,
      ...
      {
        n: 6,
        words: ['six times', '6x', '6 x', '6×', '6 ×', '6 times'],
      },
      {
        n: 7,
        words: ['seven times', '7x', '7 x', '7×', '7 ×', '7 times'],
      },
      {
        n: 8,
        words: ['eight times', '8x', '8 x', '8×', '8 ×', '8 times'],
      },
      {
        n: 9,
        words: ['nine times', '9x', '9 x', '9×', '9 ×', '9 times'],
      },
      {
        n: 10,
        words: ['ten times', '10x', '10 x', '10×', '10 ×', '10 times'],
      },
    ];
    x.forEach(set => {
