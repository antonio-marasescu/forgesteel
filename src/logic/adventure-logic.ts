...
export class AdventureLogic {
  static getVictories = (adventure: Adventure, sourcebooks: Sourcebook[]) => {
    return AdventureLogic.getVictoriesForPlot(
      adventure.plot,
  ...
  static getVictoriesForPlot = (
    ...
    plot.content
      ...
      .forEach(pc => {
        switch (pc.type) {
          case 'adventure': {
            const adventure = SourcebookLogic.getAdventures(sourcebooks).find(
              a => a.id === pc.contentID,
            );
            if (adventure) {
          ...
          case 'encounter': {
            const encounter = SourcebookLogic.getEncounters(sourcebooks).find(
              e => e.id === pc.contentID,
            );
            if (encounter) {
              const strength = EncounterDifficultyLogic.getStrength(encounter, sourcebooks);
              const difficulty = EncounterDifficultyLogic.getDifficultyForParty(
                strength,
                heroCount,
                heroLevel,
                0,
          ...
          case 'montage': {
            const montage = SourcebookLogic.getMontages(sourcebooks).find(
              m => m.id === pc.contentID,
            );
            if (montage) {
          ...
          case 'negotiation': {
            const negotiation = SourcebookLogic.getNegotiations(sourcebooks).find(
              n => n.id === pc.contentID,
            );
            if (negotiation) {
              victories += 1;
  ...
  static getContentIDs = (plot: Plot) => {
    return AdventureLogic.getAllPlotPoints(plot)
      .flatMap(p => p.content)
  ...
  static getPlotPointParent = (plot: Plot, plotPointID: string) => {
    return AdventureLogic.getAllPlotPoints(plot).find(p =>
      p.plots.map(pp => pp.id).includes(plotPointID),
    );
  };
  ...
  static getContentOptions = (session: Session) => {
    ...
    session.encounters.forEach(e =>
      options.push({ type: 'encounter', id: e.id, name: e.name || 'Unnamed Encounter' }),
    );
    session.montages.forEach(m =>
      options.push({ type: 'montage', id: m.id, name: m.name || 'Unnamed Montage' }),
    );
    session.negotiations.forEach(n =>
      options.push({ type: 'negotiation', id: n.id, name: n.name || 'Unnamed Negotiation' }),
    );
    session.tacticalMaps.forEach(tm =>
      options.push({ type: 'map', id: tm.id, name: tm.name || 'Unnamed Map' }),
    );
    session.counters.forEach(c =>
      options.push({ type: 'counter', id: c.id, name: c.name || 'Unnamed Counter' }),
    );
