...
export class EncounterDifficultyLogic {
  static getStrength = (encounter: Encounter, sourcebooks: Sourcebook[]) => {
    const monsters = Collections.sum(encounter.groups, g =>
      EncounterDifficultyLogic.getGroupStrength(g, sourcebooks),
    );
    const terrain = Collections.sum(encounter.terrain, t =>
      EncounterDifficultyLogic.getTerrainStrength(t, sourcebooks),
    );
  ...
  static getGroupStrength = (group: EncounterGroup, sourcebooks: Sourcebook[]) => {
    return Collections.sum(group.slots, slot => {
      ...
      const addOns = group
        ? group.addOns.filter(a => slot.customization.addOnIDs.includes(a.id))
        : [];
  ...
  static getDifficultyForParty = (
    encounterStrength: number,
    heroCount: number,
    heroLevel: number,
    heroVictories: number,
  ) => {
    const budgets = EncounterDifficultyLogic.getBudgetsForParty(
      heroCount,
      heroLevel,
      heroVictories,
    );
