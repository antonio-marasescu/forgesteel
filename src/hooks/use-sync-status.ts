...
export const useSyncStatus = () => {
  ...
  const updateStatusMessage = (isOnline: boolean, isSynced: boolean, isSyncing: boolean) => {
    if (!isOnline) {
  ...
  const checkSyncStatus = useCallback(async () => {
    if ('serviceWorker' in navigator && 'caches' in window) {
      try {
        ...
        const hasIndex = keys.some(request => request.url.includes('/index.html'));
        const hasManifest = keys.some(request => request.url.includes('/manifest.json'));
        const hasAssets = keys.some(request => request.url.includes('/assets/'));
  ...
  const syncForOffline = useCallback(async () => {
    if ('serviceWorker' in navigator && 'caches' in window) {
      ...
      try {
        ...
        const urlsToCache = ['/forgesteel/', '/forgesteel/index.html', '/forgesteel/manifest.json'];
        ...
        setSyncStatus(prev => ({
          ...prev,
          isSyncing: false,
          statusMessage: updateStatusMessage(prev.isOnline, prev.isSynced, false),
        }));
      }
    }
  }, []);
